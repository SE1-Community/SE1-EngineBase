...
inline void CClipMove::GetPositionsOfEntity(CEntity *pen, FLOAT3D &v0, FLOATmatrix3D &m0, FLOAT3D &v1, FLOATmatrix3D &m1) {
...
CClipMove::CClipMove(CMovableEntity *penEntity) {
  ...
  if ((cm_penMoving->en_ulFlags & ENF_DELETED) || !(cm_penMoving->en_ulCollisionFlags & ECF_TESTMASK)
  ...
  // if entity is model
  if (penEntity->en_RenderType == CEntity::RT_MODEL || penEntity->en_RenderType == CEntity::RT_EDITORMODEL
      || penEntity->en_RenderType == CEntity::RT_SKAMODEL || penEntity->en_RenderType == CEntity::RT_SKAEDITORMODEL) {
 ...
 */
inline void CClipMove::ClipMovingPointToSphere(const FLOAT3D &vStart, const FLOAT3D &vEnd, const FLOAT3D &vSphereCenter,
  ...
  const FLOAT fQ
    = (((vSphereCenterToStart % vSphereCenterToStart) - (fSphereRadius * fSphereRadius)) / (vStartToEnd % vStartToEnd));
 ...
 */
inline void CClipMove::ClipMovingPointToCylinder(const FLOAT3D &vStart, const FLOAT3D &vEnd, const FLOAT3D &vCylinderBottomCenter,
  ...
  if (0.0f <= fMinLambda && fMinLambda < cm_fMovementFraction) {
    ...
    if (0 <= fCollisionToBottomPlaneDistance && fCollisionToBottomPlaneDistance < fCylinderBottomToTopLength) {
      // if cannot pass
      if (!SendPassEvent(cm_penTested)) {
        // mark this as the new closest found collision point
        cm_fMovementFraction = fMinLambda;
        cm_vClippedLine = (vStartToEnd * (1.0f - fMinLambda)) * cm_mBToAbsolute;
        ASSERT(cm_vClippedLine.Length() < 100.0f);
        FLOAT3D vCollisionNormal = plCylinderBottom.ProjectPoint(vCollisionPoint) - vCylinderBottomCenter;
        FLOATplane3D plClippedPlane(vCollisionNormal, vCollisionPoint);
        // project the collision plane from space B to absolute space
        cm_plClippedPlane = plClippedPlane * cm_mBToAbsolute + cm_vBToAbsolute;
        // remember hit entity
 ...
 */
void CClipMove::ClipMovingSphereToSphere(const CMovingSphere &msMoving, const CMovingSphere &msStanding) {
  ...
  ClipMovingPointToSphere(msMoving.ms_vRelativeCenter0,     // start
 ...
 */
void CClipMove::ClipMovingSphereToBrushPolygon(const CMovingSphere &msMoving, CBrushPolygon *pbpoPolygon) {
  ...
  FOREACHINSTATICARRAY(pbpoPolygon->bpo_abpePolygonEdges, CBrushPolygonEdge, itbpe) {
    ...
    ClipMovingPointToCylinder(msMoving.ms_vRelativeCenter0, // start,
                              msMoving.ms_vRelativeCenter1, // end,
...
void CClipMove::ClipMovingSphereToTerrainPolygon(const CMovingSphere &msMoving, const FLOAT3D &v0, const FLOAT3D &v1,
                                                 const FLOAT3D &v2) {
  _pfPhysicsProfile.IncrementCounter(CPhysicsProfile::PCI_SPHERETOPOLYGONTESTS);
  cm_pbpoTested = NULL;
...
void CClipMove::ClipModelMoveToModel(void) {
  ...
  FOREACHINSTATICARRAY(*cm_pamsA, CMovingSphere, itmsA) {
    ...
    FOREACHINSTATICARRAY(*cm_pamsB, CMovingSphere, itmsB) {
      ...
      if ((boxMovingSphere.Min()(1) > msB.ms_vCenter(1) + fRB) || (boxMovingSphere.Max()(1) < msB.ms_vCenter(1) - fRB)
          || (boxMovingSphere.Min()(2) > msB.ms_vCenter(2) + fRB) || (boxMovingSphere.Max()(2) < msB.ms_vCenter(2) - fRB)
          || (boxMovingSphere.Min()(3) > msB.ms_vCenter(3) + fRB) || (boxMovingSphere.Max()(3) < msB.ms_vCenter(3) - fRB)) {
        // skip it
        continue;
      }
...
void CClipMove::ClipBrushMoveToModel(void) {
  ...
  FOREACHINDYNAMICARRAY(pbmMip->bm_abscSectors, CBrushSector, itbsc) {
    ...
    FOREACHINSTATICARRAY(itbsc->bsc_abpoPolygons, CBrushPolygon, itbpo) {
      ...
      if ((itbpo->bpo_ulFlags & BPOF_PASSABLE) || !itbpo->bpo_boxBoundingBox.HasContactWith(cm_boxMovementPathAbsoluteA, 0.01f)) {
        // skip it
        continue;
      }
...
void CClipMove::ClipMoveToModel(CEntity *penModel) {
  ...
  if ((cm_boxMovementPath.Min()(1) > boxModel.Max()(1)) || (cm_boxMovementPath.Max()(1) < boxModel.Min()(1))
      || (cm_boxMovementPath.Min()(2) > boxModel.Max()(2)) || (cm_boxMovementPath.Max()(2) < boxModel.Min()(2))
      || (cm_boxMovementPath.Min()(3) > boxModel.Max()(3)) || (cm_boxMovementPath.Max()(3) < boxModel.Min()(3))) {
    // do nothing
...
void CClipMove::CacheNearPolygons(void) {
  ...
  _pfPhysicsProfile.IncrementTimerAveragingCounter(CPhysicsProfile::PTI_CACHENEARPOLYGONS, 1);
  ...
  FOREACHINLIST(CBrushSector, bsc_lnInActiveSectors, cm_lhActiveSectors, itbsc) {
    _pfPhysicsProfile.IncrementTimerAveragingCounter(CPhysicsProfile::PTI_CACHENEARPOLYGONS_MAINLOOP, 1);
    // for each polygon in the sector
    FOREACHINSTATICARRAY(itbsc->bsc_abpoPolygons, CBrushPolygon, itbpo) {
      ...
      _pfPhysicsProfile.IncrementTimerAveragingCounter(CPhysicsProfile::PTI_CACHENEARPOLYGONS_MAINLOOPFOUND, 1);
      // add it to cache
      apbpo.Push() = pbpo;
      // if it is passable
...
void CClipMove::ClipToNonZoningSector(CBrushSector *pbsc) {
  ...
  _pfPhysicsProfile.IncrementTimerAveragingCounter(CPhysicsProfile::PTI_CLIPTONONZONINGSECTOR, pbsc->bsc_abpoPolygons.Count());
  ...
  FOREACHINSTATICARRAY(pbsc->bsc_abpoPolygons, CBrushPolygon, itbpo) {
    ...
    if (!itbpo->bpo_boxBoundingBox.HasContactWith(cm_boxMovementPath) || (itbpo->bpo_ulFlags & BPOF_PASSABLE)) {
      // skip it
      continue;
    }
    // clip movement to the polygon
...
void CClipMove::ClipToZoningSector(CBrushSector *pbsc) {
  ...
  _pfPhysicsProfile.IncrementTimerAveragingCounter(CPhysicsProfile::PTI_CLIPTOZONINGSECTOR, apbpo.Count());
  ...
  for (INDEX iPolygon = 0; iPolygon < apbpo.Count(); iPolygon++) {
    ...
    if (pbpo->bpo_pbscSector != pbsc || !pbpo->bpo_boxBoundingBox.HasContactWith(cm_boxMovementPath)) {
      // skip it
      continue;
    }
...
void CClipMove::ClipMoveToBrushes(void) {
  ...
  // for each active sector
  FOREACHINLIST(CBrushSector, bsc_lnInActiveSectors, cm_lhActiveSectors, itbsc) {
    _pfPhysicsProfile.IncrementTimerAveragingCounter(CPhysicsProfile::PTI_CLIPMOVETOBRUSHES_MAINLOOP, 1);
  ...
  // for each of the found entities
  {
    for (INDEX ienFound = 0; ienFound < apenNearEntities.Count(); ienFound++) {
      ...
      if (MustTest(&enToCollide)) {
        ...
        if (enToCollide.en_RenderType == CEntity::RT_MODEL || enToCollide.en_RenderType == CEntity::RT_EDITORMODEL
            || enToCollide.en_RenderType == CEntity::RT_SKAMODEL || enToCollide.en_RenderType == CEntity::RT_SKAEDITORMODEL) {
...
void CClipMove::ClipMoveToWorld(class CWorld *pwoWorld) {
  ...
  if (/*!cm_bMovingBrush&&(cm_vA0 == cm_vA1 && cm_mA0 == cm_mA1)
    ...
    ||*/
      (cm_penMoving->en_ulFlags & ENF_DELETED) || !(cm_penMoving->en_ulCollisionFlags & ECF_TESTMASK)) {
    // skip clipping
