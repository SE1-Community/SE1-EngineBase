...
class CClipMove {
  public:
    ...
    inline BOOL MustTest(CEntity *pen) {
      return (pen->en_ulCollisionFlags & cm_ulTestMask1) && (pen->en_ulCollisionFlags & cm_ulTestMask2);
    };
    ...
    FLOAT3D cm_vA0;
    FLOATmatrix3D cm_mA0; // at the start of movement
    FLOAT3D cm_vA1;
    FLOATmatrix3D cm_mA1; // at the end of movement

    // Placement of entity B
    FLOAT3D cm_vB0;
    FLOATmatrix3D cm_mB0; // at the start of movement
    FLOAT3D cm_vB1;
    FLOATmatrix3D cm_mB1; // at the end of movement
    ...
    FLOAT3D cm_vAToB0;
    FLOATmatrix3D cm_mAToB0; // at the start of movement
    FLOAT3D cm_vAToB1;
    FLOATmatrix3D cm_mAToB1; // at the end of movement

    // For converting from space of entity B to absolute space
    FLOAT3D cm_vBToAbsolute;
    ...
    inline void GetPositionsOfEntity(CEntity *pen, FLOAT3D &v0, FLOATmatrix3D &m0, FLOAT3D &v1, FLOATmatrix3D &m1);
    ...
    inline void ClipMovingPointToSphere(const FLOAT3D &vStart, const FLOAT3D &vEnd, const FLOAT3D &vSphereCenter,
                                        const FLOAT fSphereRadius);

    // Clip a moving point to a cylinder, update collision data
    inline void ClipMovingPointToCylinder(const FLOAT3D &vStart, const FLOAT3D &vEnd, const FLOAT3D &vCylinderBottomCenter,
    ...
    void ClipMovingSphereToSphere(const CMovingSphere &msMoving, const CMovingSphere &msStanding);
    // Clip a moving sphere to a brush polygon, update collision data
    void ClipMovingSphereToBrushPolygon(const CMovingSphere &msMoving, CBrushPolygon *pbpoPolygon);
    // Clip a moving sphere to a terrain polygon, update collision data
    void ClipMovingSphereToTerrainPolygon(const CMovingSphere &msMoving, const FLOAT3D &v0, const FLOAT3D &v1, const FLOAT3D &v2);
