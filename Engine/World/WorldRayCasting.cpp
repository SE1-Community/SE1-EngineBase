 ...
 */
inline static BOOL RayHitsSphere(const FLOAT3D &vStart, const FLOAT3D &vEnd, const FLOAT3D &vSphereCenter,
                                 const FLOAT fSphereRadius, FLOAT &fDistance) {
...
void CCastRay::TestModelSimple(CEntity *penModel, CModelObject &mo) {
  ...
  if (!RayHitsSphere(cr_vOrigin, cr_vTarget, vSphereCenter, fSphereRadius + cr_fTestR, fSphereHitDistance)) {
    // ignore
    return;
  }
...
void CCastRay::TestModelCollisionBox(CEntity *penModel) {
  ...
  FOREACHINSTATICARRAY(ams, CMovingSphere, itms) {
    ...
    if (RayHitsSphere(cr_vOrigin, cr_vTarget, vCenter, itms->ms_fR + cr_fTestR, fOneSphereHitDistance)
...
void CCastRay::TestModelFull(CEntity *penModel, CModelObject &mo) {
  ...
  if (mo.PolygonHit(cl_plRay, penModel->en_plPlacement, 0 /*iCurrentMip*/, fHitDistance) != NULL
...
void CCastRay::TestSkaModelFull(CEntity *penModel, CModelInstance &mi) {
  ...
  if (fSphereHitDistance < cr_fHitDistance && fSphereHitDistance > 0.0f) {
    ...
    INDEX iBoneID = -1;
    if (cr_bFindBone) {
      fTriangleHitDistance = RM_TestRayCastHit(mi, penModel->en_mRotation, penModel->en_plPlacement.pl_PositionVector, cr_vOrigin,
                                               cr_vTarget, cr_fHitDistance, &iBoneID);
    } else {
      fTriangleHitDistance = RM_TestRayCastHit(mi, penModel->en_mRotation, penModel->en_plPlacement.pl_PositionVector, cr_vOrigin,
                                               cr_vTarget, cr_fHitDistance, NULL);
    }
...
void CCastRay::TestBrushSector(CBrushSector *pbscSector) {
  ...
  FOREACHINSTATICARRAY(pbscSector->bsc_abpoPolygons, CBrushPolygon, itpoPolygon) {
    ...
    if (fDistance0 >= 0 && fDistance0 >= fDistance1) {
      ...
      FOREACHINSTATICARRAY(bpoPolygon.bpo_abpePolygonEdges, CBrushPolygonEdge, itbpePolygonEdge) {
...
inline void CCastRay::AddSector(CBrushSector *pbsc) {
  ...
  if (pbsc->bsc_pbmBrushMip->IsFirstMip() && !(pbsc->bsc_ulFlags & BSCF_RAYTESTED)) {
...
void CCastRay::AddAllSectorsOfBrush(CBrush3D *pbr) {
  ...
  CBrushMip *pbmMip = pbr->GetBrushMipByDistance(_wrpWorldRenderPrefs.GetManualMipBrushingFactor());
  ...
  for (INDEX ias = 0; ias < _aas.Count(); ias++) {
    ...
    {FOREACHDSTOFSRC(pbsc->bsc_rsEntities, CEntity, en_rdSectors, pen)
      ...
      if (((pen->en_RenderType == CEntity::RT_MODEL
            || (pen->en_RenderType == CEntity::RT_EDITORMODEL && _wrpWorldRenderPrefs.IsEditorModelsOn()))
           && cr_ttHitModels != TT_NONE)
          //  and if cast type is TT_FULL_SEETROUGH then model is not
          //  ENF_SEETROUGH
          && !((cr_ttHitModels == TT_FULLSEETHROUGH || cr_ttHitModels == TT_COLLISIONBOX)
        ...
        // if is is a ska model
                   ...
                   || (pen->en_RenderType == CEntity::RT_SKAEDITORMODEL && _wrpWorldRenderPrefs.IsEditorModelsOn()))
                  && cr_ttHitModels != TT_NONE)
                 //  and if cast type is TT_FULL_SEETROUGH then model is not
                 //  ENF_SEETROUGH
                 && !((cr_ttHitModels == TT_FULLSEETHROUGH || cr_ttHitModels == TT_COLLISIONBOX)
      ...
      } else if ((pen->en_RenderType == CEntity::RT_BRUSH) && !(pen->en_ulFlags & ENF_HIDDEN)) {
        // get its brush
