...
static inline void BoxToGrid(const FLOATaabbox3D &boxEntity, INDEX &iMinX, INDEX &iMaxX, INDEX &iMinZ, INDEX &iMaxZ) {
...
static inline INDEX MakeKey(INDEX iX, INDEX iZ) {
  ...
  INDEX iZ2 = abs(iZ);
  INDEX iKey = (iZ2 >> (GRID_HASHTABLESIZE_LOG2 / 2)) | ((iZ2 & (GRID_HASHTABLESIZE / 2 - 1)) << (GRID_HASHTABLESIZE_LOG2 / 2));
  iKey = iKey ^ abs(iX);
...
void CWorld::MoveEntityInCollisionGrid(CEntity *pen, const FLOATaabbox3D &boxOld, const FLOATaabbox3D &boxNew) {
  ...
  // for each cell spanned by the entity before moving but not after moving
  {
    for (INDEX iX = iOldMinX; iX <= iOldMaxX; iX++) {
      for (INDEX iZ = iOldMinZ; iZ <= iOldMaxZ; iZ++) {
        if (iX >= iNewMinX && iX <= iNewMaxX && iZ >= iNewMinZ && iZ <= iNewMaxZ) {
          continue;
        }
        // find that cell
        INDEX igc = wo_pcgCollisionGrid->FindCell(iX, iZ, FALSE);
  ...
  // for each cell spanned by the entity after moving but not before moving
  {
    for (INDEX iX = iNewMinX; iX <= iNewMaxX; iX++) {
      for (INDEX iZ = iNewMinZ; iZ <= iNewMaxZ; iZ++) {
        if (iX >= iOldMinX && iX <= iOldMaxX && iZ >= iOldMinZ && iZ <= iOldMaxZ) {
          continue;
        }
        // find that cell
        INDEX igc = wo_pcgCollisionGrid->FindCell(iX, iZ, TRUE);
...
void CWorld::FindEntitiesNearBox(const FLOATaabbox3D &boxNear, CStaticStackArray<CEntity *> &apenNearEntities) {
  ...
  _pfPhysicsProfile.IncrementCounter(CPhysicsProfile::PCI_NEARENTITIESFOUND, apenNearEntities.Count());
