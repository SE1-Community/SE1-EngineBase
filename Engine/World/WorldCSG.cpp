...
CBrushMip *CWorld::GetBrushMip(CEntity &enBrush) {
  ...
  ASSERT(enBrush.en_RenderType == CEntity::RT_BRUSH || enBrush.en_RenderType == CEntity::RT_FIELDBRUSH);
 ...
 */
void CWorld::CopySourceBrushSectorsToObject(CEntity &enBrush, CBrushSectorSelectionForCSG &bscselSectors,
 ...
 */
void CWorld::MoveTargetBrushPartToObject(CEntity &enBrush, DOUBLEaabbox3D &boxAffected, CObject3D &obObject) {
  ASSERT(GetFPUPrecision() == FPT_53BIT);
  // get the brush mip from the entity
  CBrushMip &bmBrushMip = *GetBrushMip(enBrush);
 ...
 */
void CWorld::DoCSGOperation(CEntity &enThis, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther,
  ...
  woOther.CopySourceBrushSectorsToObject(enOther, selbscOtherOpen, plOther, obOtherOpen, enThis.en_plPlacement, boxOtherOpen);
  ...
  woOther.CopySourceBrushSectorsToObject(enOther, selbscOtherClosed, plOther, obOtherClosed, enThis.en_plPlacement,
                                         boxOtherClosed);
 ...
 */
void CWorld::CSGAdd(CEntity &enThis, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther) {
  ...
  DoCSGOperation(enThis, woOther, enOther, plOther, &CObject3D::CSGAddMaterial, &CObject3D::CSGAddRooms);
 ...
 * Add a brush from another world to a brush in this world, other entities get copied.
 */
void CWorld::CSGAddReverse(CEntity &enThis, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther) {
  _pfWorldEditingProfile.StartTimer(CWorldEditingProfile::PTI_CSGTOTAL);
  _pfWorldEditingProfile.IncrementAveragingCounter();
  // do add material reverse for open sectors and then do add rooms for closed sectors
  DoCSGOperation(enThis, woOther, enOther, plOther, &CObject3D::CSGAddMaterialReverse, &CObject3D::CSGAddRooms);
 ...
 */
void CWorld::CSGRemove(CEntity &enThis, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther) {
  _pfWorldEditingProfile.StartTimer(CWorldEditingProfile::PTI_CSGTOTAL);
  _pfWorldEditingProfile.IncrementAveragingCounter();
  // assure that floating point precision is 53 bits
  AssureFPT_53();
  ...
  DoCSGOperation(enThis, woOther, enOther, plOther, &CObject3D::CSGRemoveMaterial, &CObject3D::CSGRemoveMaterial);
 ...
 */
void CWorld::SplitSectors(CEntity &enThis, CBrushSectorSelection &selbscSectorsToSplit, CWorld &woOther, CEntity &enOther,
  ...
  woOther.CopySourceBrushSectorsToObject(enOther, selbscOther, plOther, obOther, enThis.en_plPlacement, boxOther);
...
CBrushSector *CWorld::JoinTwoSectors(CBrushSector &bscA, CBrushSector &bscB) {
  ...
  if (obResult.ob_aoscSectors.Count() == 0)
    return NULL;
...
BOOL CWorld::CanJoinSectors(CBrushSectorSelection &selbscSectorsToJoin) {
  if (selbscSectorsToJoin.Count() < 2)
    return FALSE;
 ...
 * (other brush must have only one sector.)
 */
void CWorld::SplitPolygons(CEntity &enThis, CBrushPolygonSelection &selbpoPolygonsToSplit, CWorld &woOther, CEntity &enOther,
                           const CPlacement3D &plOther) {
  _pfWorldEditingProfile.StartTimer(CWorldEditingProfile::PTI_CSGTOTAL);
  _pfWorldEditingProfile.IncrementAveragingCounter();
...
void CWorld::JoinAllPossiblePolygons(CBrushPolygonSelection &selbpoPolygonsToJoin, BOOL bPreserveTextures, INDEX iTexture) {
  _pfWorldEditingProfile.StartTimer(CWorldEditingProfile::PTI_CSGTOTAL);
  _pfWorldEditingProfile.IncrementAveragingCounter();
...
BOOL CWorld::CanCopySectors(CBrushSectorSelection &selbscSectorsToCopy) {
  if (selbscSectorsToCopy.Count() < 1)
    return FALSE;
  // get the brush mip from first sector in selection
...
void CWorld::CopySectors(CBrushSectorSelection &selbscSectorsToCopy, CEntity *penTarget, BOOL bWithEntities) {
...
void CWorld::CopyPolygonInWorld(CBrushPolygon &bpoSrc, CBrushSector &bscDst, INDEX iPol, INDEX &iEdg, INDEX &iVtx) {
  ...
  GetMajorAxesForPlane(plDst.bpl_plAbsolute, plDst.bpl_iPlaneMajorAxis1, plDst.bpl_iPlaneMajorAxis2);
...
void CWorld::CreatePolygon(CBrushVertexSelection &selVtx) {
  ...
  bplNew.bpl_pldPreciseRelative = DOUBLEplane3D(bv0.bvx_vdPreciseRelative, bv1.bvx_vdPreciseRelative, bv2.bvx_vdPreciseRelative);
  ...
  GetMajorAxesForPlane(bplNew.bpl_plAbsolute, bplNew.bpl_iPlaneMajorAxis1, bplNew.bpl_iPlaneMajorAxis2);
