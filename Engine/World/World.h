...
class ENGINE_API CWorld {
  public:
    CStaticArray<CTextureTransformation> wo_attTextureTransformations; // current transformations for animating texture mapping
    CStaticArray<CTextureBlending> wo_atbTextureBlendings; // current blendings for animating texture blending
    CStaticArray<CSurfaceType> wo_astSurfaceTypes; // polygon surface types
    CStaticArray<CContentType> wo_actContentTypes; // sector content types
    CStaticArray<CEnvironmentType> wo_aetEnvironmentTypes; // sector environment types
    CStaticArray<CIlluminationType> wo_aitIlluminationTypes; // illumination types (0 is not used)
    ...
    void MoveEntityInCollisionGrid(CEntity *pen, const FLOATaabbox3D &boxOld, const FLOATaabbox3D &boxNew);

    // Find all entities in collision grid near given box
    void FindEntitiesNearBox(const FLOATaabbox3D &boxNear, CStaticStackArray<CEntity *> &apenNearEntities);
    ...
    void CopySourceBrushSectorsToObject(CEntity &enBrush, CBrushSectorSelectionForCSG &bscselSectors,
                                        const CPlacement3D &plSourcePlacement, CObject3D &obObject,
                                        const CPlacement3D &plTargetPlacement, DOUBLEaabbox3D &boxSourceAbsolute);

    // Move sectors of a target brush that are affected, to a 3D object
    void MoveTargetBrushPartToObject(CEntity &enBrush, DOUBLEaabbox3D &boxAffected, CObject3D &obObject);
    ...
    void DoCSGOperation(CEntity &enThis, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther,
    ...
    void CopyAllEntitiesExceptOne(CWorld &woOther, CEntity &enExcepted, const CPlacement3D &plOtherSystem);
    ...
    // Write and read world information (description, name, flags...)
  ...
  public:
    ...
    // Create a new entity of given class
    CEntity *CreateEntity_t(const CPlacement3D &plPlacement, const CTFileName &fnmClass); // throw char *
    ...
    void CopyEntities(CWorld &woOther, CDynamicContainer<CEntity> &cenToCopy, CEntitySelection &senCopied, const CPlacement3D &plOtherSystem);

    // Copy entity in world
    CEntity *CopyEntityInWorld(CEntity &enOriginal, const CPlacement3D &plOtherEntity, BOOL bWithDescendants = TRUE);
    ...
    void CSGRemove(CEntity &enThis, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther);

    // Split selected sectors in a brush in this world with one brush in other world (other brush must have only one sector)
    void SplitSectors(CEntity &enThis, CBrushSectorSelection &selbscSectorsToSplit, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther);
    ...
    // Split selected polygons in a brush in this world with one brush in other world (other brush must have only one sector)
    void SplitPolygons(CEntity &enThis, CBrushPolygonSelection &selbpoPolygonsToSplit, CWorld &woOther, CEntity &enOther, const CPlacement3D &plOther);
    ...
    void JoinAllPossiblePolygons(CBrushPolygonSelection &selbpoPolygonsToJoin, BOOL bPreserveTextures, INDEX iTexture);
