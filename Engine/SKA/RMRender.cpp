...
void RM_GetModelVertices(CModelInstance &mi, CStaticStackArray<FLOAT3D> &avVertices, FLOATmatrix3D &mRotation, FLOAT3D &vPosition,
      ...
      for (int isurf = 0; isurf < ctsurf; isurf++) {
        ...
        for (int itri = 0; itri < cttri; itri++) {
          ...
          if (fDistance0 >= 0 && fDistance0 >= fDistance1) {
            ...
            isIntersector.AddEdge(vVertex1(iMajorAxis1), vVertex1(iMajorAxis2), vVertex2(iMajorAxis1), vVertex2(iMajorAxis2));
            isIntersector.AddEdge(vVertex2(iMajorAxis1), vVertex2(iMajorAxis2), vVertex0(iMajorAxis1), vVertex0(iMajorAxis2));
...
void RM_AddSimpleShadow_View(CModelInstance &mi, const FLOAT fIntensity, const FLOATplane3D &plShadowPlane) {
  ...
  if (_ulRenFlags & SRMF_FOG) { // for each vertex in shadow quad
    GFXTexCoord tex;
    for (INDEX i = 0; i < 4; i++) {
  ...
  if (_ulRenFlags & SRMF_HAZE) { // for each vertex in shadow quad
    for (INDEX i = 0; i < 4; i++) {
...
void RM_SetShaderParamsAdjustCallback(void (*pAdjustShaderParams)(void *pData, INDEX iSurfaceID, CShader *pShader,
...
static void RenderBox(FLOAT3D vMinVtx, FLOAT3D vMaxVtx, COLOR col) {
  ...
  vBoxVtxs[0].x = vMinVtx(1);
  vBoxVtxs[0].y = vMaxVtx(2);
  vBoxVtxs[0].z = vMinVtx(3);
  vBoxVtxs[1].x = vMinVtx(1);
  vBoxVtxs[1].y = vMaxVtx(2);
  vBoxVtxs[1].z = vMaxVtx(3);
  vBoxVtxs[2].x = vMaxVtx(1);
  vBoxVtxs[2].y = vMaxVtx(2);
  vBoxVtxs[2].z = vMinVtx(3);
  vBoxVtxs[3].x = vMaxVtx(1);
  vBoxVtxs[3].y = vMaxVtx(2);
  vBoxVtxs[3].z = vMaxVtx(3);

  vBoxVtxs[4].x = vMinVtx(1);
  vBoxVtxs[4].y = vMinVtx(2);
  vBoxVtxs[4].z = vMinVtx(3);
  vBoxVtxs[5].x = vMinVtx(1);
  vBoxVtxs[5].y = vMinVtx(2);
  vBoxVtxs[5].z = vMaxVtx(3);
  vBoxVtxs[6].x = vMaxVtx(1);
  vBoxVtxs[6].y = vMinVtx(2);
  vBoxVtxs[6].z = vMinVtx(3);
  vBoxVtxs[7].x = vMaxVtx(1);
  vBoxVtxs[7].y = vMinVtx(2);
  vBoxVtxs[7].z = vMaxVtx(3);
...
static INDEX BuildHierarchy(CModelInstance *pmiModel, INDEX irmParent) {
  ...
  if (pmiModel->mi_iParentBoneID == (-1)) {
    ...
    if (rmParent.rm_pmiModel->mi_psklSkeleton != NULL && rmParent.rm_iSkeletonLODIndex >= 0) {
      ...
      iParentBoneIndex
        = rmParent.rm_pmiModel->mi_psklSkeleton->FindBoneInLOD(pmiModel->mi_iParentBoneID, rmParent.rm_iSkeletonLODIndex);
  ...
  for (INDEX im = 0; im < ctm; im++) {
    ...
    if (vStretch != FLOAT3D(1.0f, 1.0f, 1.0f)) {
      // calculate new distance factor
      fDistFactor
        = fDistFactor / Max(vStretch(1), Max(vStretch(2), vStretch(3))); // Log2( Max(vStretch(1),Max(vStretch(2),vStretch(3))));
    }
      ...
      if (rm.rm_pmiModel->FindAnimationByID(pa.pa_iAnimID, &iAnimSetIndex, &iAnimIndex)) {
        ...
        for (INDEX im = 0; im < an.an_ameMorphs.Count(); im++) {
          ...
          if (FindRenMorph(rm, an.an_ameMorphs[im].me_iMorphMapID, &iMorphIndex)) {
            ...
            _aRenMorph[iMorphIndex].rmp_fFactor
              = Lerp(_aRenMorph[iMorphIndex].rmp_fFactor, fFactor, fFadeFactor * pa.pa_Strength);
          }
        }
      }
    }
