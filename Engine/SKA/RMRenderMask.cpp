...
extern void InternalShader_Mask(void) {
  ...
  for (; iVtx < ctVtx; iVtx++) {
    ...
    if (ptex != NULL) {
      ...
      tvd.tvd_fU = tvd.tvd_fV = 0;

    // check clipping against horizontal screen boundaries and near clip plane
    if (tvd.tvd_pv2.pv2_fI < 0 || tvd.tvd_pv2.pv2_fI >= _slMaskWidth || tvd.tvd_fZ > fFrontClipDistance
  ...
  for (INDEX iIdx = 0; iIdx < ctIdx; iIdx += 3) {
    ...
    // not clipped - just draw!
      ...
      DrawTriangle_Mask(_pubMask, _slMaskWidth, _slMaskHeight, &tvd[0].tvd_pv2, &tvd[1].tvd_pv2, &tvd[2].tvd_pv2, TRUE);
    }
  }
}
