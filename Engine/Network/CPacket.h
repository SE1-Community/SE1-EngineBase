...
#define MAX_HEADER_SIZE (sizeof(UBYTE) + sizeof(ULONG) + sizeof(UWORD) \
                       + sizeof(ULONG)) // pa_bReliable + pa_ulPacketSequence + pa_uwID + pa_ulTransferSize
...
class CPacket {
  public:
    ...
    CTimerValue pa_tvSendWhen; // When to try sending this packet (includes latency bandwidth limitations as well as retry intervals)
...
class CPacketBuffer {
  public:
    ...
    // Add a packet to the end of the packet buffer
    BOOL AppendPacket(CPacket& paPacket, BOOL bDelay);

    // Insert the packet in the buffer, according to it's sequence number
    ...
    // Read the data from the first packet in the bufffer, but does not remove it
    CPacket* PeekFirstPacket();

    // Read the first packet in the bufffer
    CPacket* GetFirstPacket();

    // Read the data from the packet with the requested sequence, but does not remove it
    CPacket* PeekPacket(ULONG ulSequence);

    // Read the packet with the requested sequence
