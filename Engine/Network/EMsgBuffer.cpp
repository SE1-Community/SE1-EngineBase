...
void CEntityMessage::WriteEntityCreate(ULONG &ulEntityID, CPlacement3D &plPlacement, UWORD &uwEntityClassID, UWORD &uwEventCode,
                                       void *pvEventData, UWORD &uwDataSize) {
  UBYTE *pubMarker;
  ULONG ulShrunkAngle;
  SWORD swH, swP, swB;
...
void CEntityMessage::ReadEntityCreate(ULONG &ulEntityID, CPlacement3D &plPlacement, UWORD &uwEntityClassID, UWORD &uwEventCode,
                                      void *pvEventData, UWORD &uwDataSize) {
...
void CEntityMessage::WriteEntityCopy(ULONG &ulSourceEntityID, ULONG &ulTargetEntityID, CPlacement3D &plPlacement,
                                     UWORD &uwEventCode, void *pvEventData, UWORD &uwDataSize) {
  UBYTE *pubMarker;
  ULONG ulShrunkAngle;
  SWORD swH, swP, swB;
...
void CEntityMessage::ReadEntityCopy(ULONG &ulSourceEntityID, ULONG &ulTargetEntityID, CPlacement3D &plPlacement,
                                    UWORD &uwEventCode, void *pvEventData, UWORD &uwDataSize) {
...
int CEMsgBuffer::ReadTick(float fTickTime, const void *pv, SLONG &slSize) {
  ...
  if (slTickSize > 0) {
    ...
    if (slSize
        < (bu_slSize - emb_atmTickMarkers[iTickIndex].tm_slTickOffset) + emb_atmTickMarkers[iNextTickIndex].tm_slTickOffset - 1) {
      return EMB_ERR_BUFFER_TOO_SMALL;
    }
    ...
    memcpy((UBYTE *)pv, bu_pubBuffer + emb_atmTickMarkers[iTickIndex].tm_slTickOffset,
           bu_slSize - emb_atmTickMarkers[iTickIndex].tm_slTickOffset);
    // copy datafrom the start of the buffer to the start of next tick
    memcpy(((UBYTE *)pv) + bu_slSize - emb_atmTickMarkers[iTickIndex].tm_slTickOffset, bu_pubBuffer,
           emb_atmTickMarkers[iNextTickIndex].tm_slTickOffset);
  }
