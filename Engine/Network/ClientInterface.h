...
class CClientInterface {
  public:
    ...
    // Set the client to be local and optionally connects to another local client
    ...
    // Read the expected size of realiable message in the reliable input buffer
    SLONG GetExpectedReliableSize(void);

    // Read the current size of realiable message in the reliable input buffer
