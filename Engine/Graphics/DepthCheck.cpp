...
static void UpdateDepthPointsVisibility(const CDrawPort *pdp, const INDEX iMirrorLevel, DepthInfo *pdi, const INDEX ctCount) {
  ...
  if (eAPI == GAT_D3D) {
    ...
    for (idi = 0; idi < ctCount; idi++) {
      ...
      if (hr != D3D_OK) {
        continue; // skip if lock didn't make it
      }
    ...
    for (idi = 0; idi < ctCount; idi++) {
      ...
      if (iMirrorLevel != di.di_iMirrorLevel || _iCheckIteration != di.di_iSwapLastRequest || col == 0) {
        continue;
      }
...
extern BOOL CheckDepthPoint(const CDrawPort *pdp, PIX pixI, PIX pixJ, FLOAT fOoK, INDEX iID, INDEX iMirrorLevel) {
  // no raster?
  const CRaster *pra = pdp->dp_Raster;
  ...
  if (pixI < 1 || pixJ < 1 || pixI > pra->ra_Width - 2 || pixJ > pra->ra_Height - 2) {
    return FALSE;
  }
...
extern void CheckDelayedDepthPoints(const CDrawPort *pdp, INDEX iMirrorLevel) {
  // skip if not delayed or mirror level is to high
  gap_iOptimizeDepthReads = Clamp(gap_iOptimizeDepthReads, 0L, 2L);

  if (gap_iOptimizeDepthReads == 0 || iMirrorLevel > 7) {
    return;
  }
  ...
  if (gap_iOptimizeDepthReads == 2 && tmDelta < 0.1f) {
    return;
  }
  ...
  if (ctPoints == 0) {
    return; // done if no points in queue
  }
  ...
  if (ctPoints == 0) {
    ...
    _adiDelayed.PopUntil(ctPoints - 1);
  }
  ...
  if (ctPoints > 0) {
    UpdateDepthPointsVisibility(pdp, iMirrorLevel, &_adiDelayed[0], ctPoints);
  }
