...
static BOOL ClipToDrawPort(const CDrawPort *pdp, PIX &pixI, PIX &pixJ, PIX &pixW, PIX &pixH) {
  ...
  if ((pixI + pixW) > pdp->dp_Width) {
    pixW = pdp->dp_Width - pixI;
  }

  if ((pixJ + pixH) > pdp->dp_Height) {
    pixH = pdp->dp_Height - pixJ;
  }
...
void CDrawPort::InitCloned(CDrawPort *pdpBase, DOUBLE rMinI, DOUBLE rMinJ, DOUBLE rSizeI, DOUBLE rSizeJ) {
  ...
  dp_MinIOverRasterSizeI = rMinI * pdpBase->dp_SizeIOverRasterSizeI + pdpBase->dp_MinIOverRasterSizeI;
  dp_MinJOverRasterSizeJ = rMinJ * pdpBase->dp_SizeJOverRasterSizeJ + pdpBase->dp_MinJOverRasterSizeJ;
  ...
  if (dp_ScissorMinI > dp_ScissorMaxI) {
    dp_ScissorMinI = dp_ScissorMaxI = 0;
  }

  if (dp_ScissorMinJ > dp_ScissorMaxJ) {
    dp_ScissorMinJ = dp_ScissorMaxJ = 0;
  }
...
// Clone a drawport
CDrawPort::CDrawPort(CDrawPort *pdpBase, DOUBLE rMinI, DOUBLE rMinJ, DOUBLE rSizeI, DOUBLE rSizeJ) {
...
// Return unique drawports number
...
void CDrawPort::SetOrtho(void) const {
  ...
  // [Cecil] 2021-02-20: Get APIs before checking
  const BOOL bFinishOGL = (ogl_iFinish == 3 && _pGfx->gl_eCurrentAPI == GAT_OGL);
  #ifdef SE1_D3D
  const BOOL bFinishD3D = (d3d_iFinish == 3 && _pGfx->gl_eCurrentAPI == GAT_D3D);
  #else
...
// Draw one point
void CDrawPort::DrawPoint(PIX pixI, PIX pixJ, COLOR col, PIX pixRadius) const {
...
void CDrawPort::DrawPoint3D(FLOAT3D v, COLOR col, FLOAT fRadius) const {
  // check API and radius
  const GfxAPIType eAPI = _pGfx->gl_eCurrentAPI;
...
void CDrawPort::DrawLine(PIX pixI0, PIX pixJ0, PIX pixI1, PIX pixJ1, COLOR col, ULONG typ) const {
  ...
  // OpenGL
  if (eAPI == GAT_OGL) {
    const FLOAT fI0 = pixI0 + 0.5f;
    const FLOAT fJ0 = pixJ0 + 0.5f;
    const FLOAT fI1 = pixI1 + 0.5f;
    const FLOAT fJ1 = pixJ1 + 0.5f;
    ...
    HRESULT hr;
    const FLOAT fI0 = pixI0 + 0.75f;
    const FLOAT fJ0 = pixJ0 + 0.75f;
    const FLOAT fI1 = pixI1 + 0.75f;
    const FLOAT fJ1 = pixJ1 + 0.75f;
    const ULONG d3dColor = rgba2argb(col);

    CTVERTEX avtxLine[2] = {{fI0, fJ0, 0, d3dColor, 0, 0}, {fI1, fJ1, 0, d3dColor, fD, 0}};
...
void CDrawPort::DrawLine3D(FLOAT3D v0, FLOAT3D v1, COLOR col) const {
  ...
  if (eAPI == GAT_OGL) {
    ...
    CTVERTEX avtxLine[2] = {{v0(1), v0(2), v0(3), d3dColor, 0, 0}, {v1(1), v1(2), v1(3), d3dColor, 0, 0}};
...
void CDrawPort::DrawBorder(PIX pixI, PIX pixJ, PIX pixWidth, PIX pixHeight, COLOR col, ULONG typ) const {
  ...
  if (eAPI == GAT_OGL) {
    ...
    pglVertex2f(fI0, fJ0);
    pglTexCoord2f(fD, 0);
    pglVertex2f(fI1, fJ0); // up
    pglTexCoord2f(0, 0);
    pglVertex2f(fI1, fJ0);
    pglTexCoord2f(fD, 0);
    pglVertex2f(fI1, fJ1); // right
    pglTexCoord2f(0, 0);
    pglVertex2f(fI0, fJ1);
    pglTexCoord2f(fD, 0);
    pglVertex2f(fI1 + 1, fJ1); // down
    pglTexCoord2f(0, 0);
    pglVertex2f(fI0, fJ0 + 1);
    pglTexCoord2f(fD, 0);
    pglVertex2f(fI0, fJ1); // left
    pglEnd();
    OGL_CHECKERROR;
...
void CDrawPort::Fill(PIX pixI, PIX pixJ, PIX pixWidth, PIX pixHeight, COLOR col) const {
  ...
  if (((col & CT_AMASK) >> CT_ASHIFT) != CT_OPAQUE) { // draw thru polygon
...
void CDrawPort::Fill(PIX pixI, PIX pixJ, PIX pixWidth, PIX pixHeight, COLOR colUL, COLOR colUR, COLOR colDL, COLOR colDR) const {
  ...
  const FLOAT fI0 = pixI;
  const FLOAT fI1 = pixI + pixWidth;
  const FLOAT fJ0 = pixJ;
  const FLOAT fJ1 = pixJ + pixHeight;
  ...
  if (eAPI == GAT_OGL) {
    ...
    CTVERTEX avtxTris[6] = {{fI0, fJ0, 0, d3dColUL, 0, 0}, {fI0, fJ1, 0, d3dColDL, 0, 1}, {fI1, fJ1, 0, d3dColDR, 1, 1},
...
// Grab screen
void CDrawPort::GrabScreen(class CImageInfo &iiGrabbedImage, INDEX iGrabZBuffer) const {
  // check API
  const GfxAPIType eAPI = _pGfx->gl_eCurrentAPI;
  ...
  if (eAPI == GAT_OGL) {
    ...
    FlipBitmap(iiGrabbedImage.ii_Picture, iiGrabbedImage.ii_Picture, iiGrabbedImage.ii_Width,
               iiGrabbedImage.ii_Height, 1, iiGrabbedImage.ii_BitsPerPixel == 32);
  }
  // Direct3D
  #ifdef SE1_D3D
  else if (eAPI == GAT_D3D) {
...
BOOL CDrawPort::IsPointVisible(PIX pixI, PIX pixJ, FLOAT fOoK, INDEX iID, INDEX iMirrorLevel) const {
  // must have raster!
  if (dp_Raster == NULL) {
    ASSERT(FALSE);
    return FALSE;
  ...
  if (pixI < 1 || pixI > dp_Width - 2 || pixJ < 1 || pixJ > dp_Height - 2) {
    return FALSE;
  }
...
void CDrawPort::RenderLensFlare(CTextureObject *pto, FLOAT fI, FLOAT fJ, FLOAT fSizeI, FLOAT fSizeJ,
                                ANGLE aRotation, COLOR colLight) const {
  // check API
  const GfxAPIType eAPI = _pGfx->gl_eCurrentAPI;
  ...
  pvtx[0].x = fI - fRICosA + fRJSinA;
  pvtx[0].y = fJ - fRISinA + fRJCosA;
  pvtx[0].z = 0.01f;

  pvtx[1].x = fI - fRICosA - fRJSinA;
  pvtx[1].y = fJ - fRISinA - fRJCosA;
  pvtx[1].z = 0.01f;

  pvtx[2].x = fI + fRICosA - fRJSinA;
  pvtx[2].y = fJ + fRISinA - fRJCosA;
  pvtx[2].z = 0.01f;

  pvtx[3].x = fI + fRICosA + fRJSinA;
  pvtx[3].y = fJ + fRISinA + fRJCosA;
  pvtx[3].z = 0.01f;
...
// Set font to be used to printout some text on this drawport
...
// Return width of the longest line in text string
...
// Write text string on drawport (left aligned if not forced otherwise)
void CDrawPort::PutText(const CTString &strText, PIX pixX0, PIX pixY0, const COLOR colBlend) const {
  ...
  if (pixY0 > dp_Height || pixX0 > dp_Width) {
    return;
  }
  ...
  ULONG ulAlphaDefault = (colBlend & CT_AMASK) >> CT_ASHIFT;
  ...
  for (INDEX iChar = 0; iChar < ctMaxChars; iChar++) {
    ...
    // special char encountered and allowed?
      ...
      switch (chrCurrent) {
        ...
        // allow console override settings?
        case 'o':
          bParse = bParse && gfx_bDecoratedText;
          continue;
    ...
    const FLOAT fX0 = pixXA;
    const FLOAT fX1 = fX0 + pixScaledWidth;
    const FLOAT fY0 = pixY0;
    const FLOAT fY1 = fY0 + pixScaledHeight;
    const FLOAT fU0 = pixCharX * fCorrectionU;
    const FLOAT fU1 = (pixCharX + pixCellWidth) * fCorrectionU;
    const FLOAT fV0 = pixCharY * fCorrectionV;
    const FLOAT fV1 = (pixCharY + pixCharHeight) * fCorrectionV;
    ...
    if (bBold) {
      ...
      pvtx[0].x = pvtx[0 - 4].x + fAdjustX;
      pvtx[0].y = fY0;
      pvtx[0].z = 0;

      pvtx[1].x = pvtx[1 - 4].x + fAdjustX;
      pvtx[1].y = fY1;
      pvtx[1].z = 0;

      pvtx[2].x = pvtx[2 - 4].x + fAdjustX;
      pvtx[2].y = fY1;
      pvtx[2].z = 0;

      pvtx[3].x = pvtx[3 - 4].x + fAdjustX;
      pvtx[3].y = fY0;
      pvtx[3].z = 0;
...
// Write text string on drawport (centered arround X)
...
// Write text string on drawport (centered arround X and Y)
...
// Write text string on drawport (right-aligned)
...
void CDrawPort::PutTexture(class CTextureObject *pTO, const PIXaabbox2D &boxScreen, const COLOR colBlend) const {
...
void CDrawPort::PutTexture(class CTextureObject *pTO, const PIXaabbox2D &boxScreen, const MEXaabbox2D &boxTexture,
  ...
  const PIX pixI0 = boxScreen.Min()(1);
  const PIX pixI1 = boxScreen.Max()(1);
  const PIX pixJ0 = boxScreen.Min()(2);
  const PIX pixJ1 = boxScreen.Max()(2);
...
// Prepare texture and rendering arrays
void CDrawPort::InitTexture(class CTextureObject *pTO, const BOOL bClamp) const {
  // prepare
  if (pTO != NULL) {
    // has texture
    CTextureData *ptd = (CTextureData *)pTO->GetData();
...
// Add one full texture to rendering queue
...
// Add one part of texture to rendering queue
void CDrawPort::AddTexture(const FLOAT fI0, const FLOAT fJ0, const FLOAT fI1, const FLOAT fJ1, const FLOAT fU0, const FLOAT fV0,
                           const FLOAT fU1, const FLOAT fV1, const COLOR col) const {
  const GFXColor glCol(AdjustColor(col, _slTexHueShift, _slTexSaturation));
  const INDEX iStart = _avtxCommon.Count();
...
// Add one triangle to rendering queue
void CDrawPort::AddTriangle(const FLOAT fI0, const FLOAT fJ0, const FLOAT fI1, const FLOAT fJ1, const FLOAT fI2, const FLOAT fJ2,
...
// Add one textured quad (up-left start, counter-clockwise)
void CDrawPort::AddTexture(const FLOAT fI0, const FLOAT fJ0, const FLOAT fU0, const FLOAT fV0, const COLOR col0, const FLOAT fI1,
                           const FLOAT fJ1, const FLOAT fU1, const FLOAT fV1, const COLOR col1, const FLOAT fI2, const FLOAT fJ2,
                           const FLOAT fU2, const FLOAT fV2, const COLOR col2, const FLOAT fI3, const FLOAT fJ3, const FLOAT fU3,
                           const FLOAT fV3, const COLOR col3) const {
...
// Render all textures from rendering queue and flushed rendering arrays
void CDrawPort::FlushRenderingQueue(void) const {
  gfxFlushElements();
  gfxResetArrays();
}

// Blend screen with accumulation color
