...
void CDrawPort::GrabScreen(class CImageInfo &iiGrabbedImage, INDEX iGrabZBuffer /*=0*/) const {
  ...
  if (eAPI == GAT_OGL) {
    ...
    FlipBitmap(iiGrabbedImage.ii_Picture, iiGrabbedImage.ii_Picture, iiGrabbedImage.ii_Width, iiGrabbedImage.ii_Height, 1,
               iiGrabbedImage.ii_BitsPerPixel == 32);
  }
  // Direct3D
  #ifdef SE1_D3D
  else if (eAPI == GAT_D3D) {
...
void CDrawPort::RenderLensFlare(CTextureObject *pto, FLOAT fI, FLOAT fJ, FLOAT fSizeI, FLOAT fSizeJ, ANGLE aRotation,
                                COLOR colLight) const {
  // check API
  const GfxAPIType eAPI = _pGfx->gl_eCurrentAPI;
...
ULONG CDrawPort::GetTextWidth(const CTString &strText) const {
  ...
  for (INDEX i = 0; i < (INDEX)strlen(strText); i++) { // get current letter
...
void CDrawPort::PutTexture(class CTextureObject *pTO, const PIXaabbox2D &boxScreen, const COLOR colBlend /*=0xFFFFFFFF*/) const {
...
void CDrawPort::PutTexture(class CTextureObject *pTO, const PIXaabbox2D &boxScreen, const COLOR colUL, const COLOR colUR,
                           const COLOR colDL, const COLOR colDR) const {
