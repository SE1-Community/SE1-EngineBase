...
void Particle_PrepareSystem(CDrawPort *pdpDrawPort, CAnyProjection3D &prProjection) {
  ...
  if (prProjection.IsPerspective()) {
    _fPerspectiveFactor = ((CPerspectiveProjection3D *)&*prProjection)->ppr_PerspectiveRatios(1);
  }
...
void Particle_EndSystem(BOOL bRestoreOrtho) {
...
void Particle_RenderSquare(const FLOAT3D &vPos, FLOAT fSize, ANGLE aRotation, COLOR col, FLOAT fYRatio) {
  // trivial rejection
  ...
  // if haze is active
  if (_Particle_bHasHaze) { // get haze strength at particle position
  ...
  // if fog is active
  if (_Particle_bHasFog) { // get fog strength at particle position
  ...
  if (aRotation == 0) {
    const FLOAT fIBeg = fI0 - fRX;
    const FLOAT fIEnd = fI0 + fRX;
    const FLOAT fJBeg = fJ0 - fRY;
    const FLOAT fJEnd = fJ0 + fRY;
    ...
    pvtx[0].x = fI0 - fSinPCos;
    pvtx[0].y = fJ0 - fSinMCos;
    pvtx[0].z = fOoK;

    pvtx[1].x = fI0 + fSinMCos;
    pvtx[1].y = fJ0 - fSinPCos;
    pvtx[1].z = fOoK;

    pvtx[2].x = fI0 + fSinPCos;
    pvtx[2].y = fJ0 + fSinMCos;
    pvtx[2].z = fOoK;

    pvtx[3].x = fI0 - fSinMCos;
...
void Particle_RenderLine(const FLOAT3D &vPos0, const FLOAT3D &vPos1, FLOAT fWidth, COLOR col) {
  // trivial rejection
  if (fWidth < 0 || ((col & CT_AMASK) >> CT_ASHIFT) < 2) {
    return;
  }
  ...
  // skip if not in screen
  if (vProjected0(3) > _fNearClipDistance || vProjected1(3) > _fNearClipDistance) {
    return;
  }
  ...
  // if haze is active
  if (_Particle_bHasHaze) { // get haze strength at particle positions
    ...
    if (_colAttMask) { // apply haze color (if not transparent)
      COLOR colH;
      colH = _colAttMask | RGBAToColor(ulH0, ulH0, ulH0, ulH0);
      col0 = MulColors(col0, colH);
      colH = _colAttMask | RGBAToColor(ulH1, ulH1, ulH1, ulH1);
      col1 = MulColors(col1, colH);

    } else {
      ptexFogHaze[0].t = ptexFogHaze[1].t = 0;
    }
  }
  ...
  if (_Particle_bHasFog) {
    ...
    if (_colAttMask) {
      // apply fog color (if not transparent)
      COLOR colF;
      colF = _colAttMask | RGBAToColor(ulF0, ulF0, ulF0, ulF0);
      col0 = MulColors(col0, colF);
      colF = _colAttMask | RGBAToColor(ulF1, ulF1, ulF1, ulF1);
      col1 = MulColors(col1, colF);
    }
  }
  ...
  // lets draw
  const FLOAT fI0 = vProjected0(1);
  const FLOAT fI1 = vProjected1(1);
  const FLOAT fJ0 = vProjected0(2);
  const FLOAT fJ1 = vProjected1(2);
  const FLOAT fOoK0 = vProjected0(3);
  const FLOAT fOoK1 = vProjected1(3);
...
void Particle_RenderQuad3D(const FLOAT3D &vPos0, const FLOAT3D &vPos1, const FLOAT3D &vPos2, const FLOAT3D &vPos3, COLOR col) {
  // trivial rejection
  ...
  FLOAT3D vNearest = vProjected0;
  
  // find nearest-Z vertex
  if (vNearest(3) > vProjected1(3)) {
    vNearest = vProjected1;
  }

  if (vNearest(3) > vProjected2(3)) {
    vNearest = vProjected2;
  }

  if (vNearest(3) > vProjected3(3)) {
    vNearest = vProjected3;
  }
  ...
  if (_Particle_bHasHaze) {
    ...
    if (_colAttMask) {
      ...
      colH = _colAttMask | RGBAToColor(ulH2, ulH2, ulH2, ulH2);
      col2 = MulColors(col2, colH);

      colH = _colAttMask | RGBAToColor(ulH3, ulH3, ulH3, ulH3);
      col3 = MulColors(col3, colH);

    } else {
      ptexFogHaze[0].t = ptexFogHaze[1].t = ptexFogHaze[2].t = ptexFogHaze[3].t = 0;
    }
  }
  ...
  if (_Particle_bHasFog) {
    ...
    if (_colAttMask) {
      ...
      colF = _colAttMask | RGBAToColor(ulF2, ulF2, ulF2, ulF2);
      col2 = MulColors(col2, colF);

      colF = _colAttMask | RGBAToColor(ulF3, ulF3, ulF3, ulF3);
      col3 = MulColors(col3, colF);
    }
  }
  ...
  // prepare vertices
  pvtx[0].x = vProjected0(1);
  pvtx[0].y = vProjected0(2);
  pvtx[0].z = vProjected0(3);

  pvtx[1].x = vProjected1(1);
  pvtx[1].y = vProjected1(2);
  pvtx[1].z = vProjected1(3);

  pvtx[2].x = vProjected2(1);
  pvtx[2].y = vProjected2(2);
  pvtx[2].z = vProjected2(3);

  pvtx[3].x = vProjected3(1);
  pvtx[3].y = vProjected3(2);
  pvtx[3].z = vProjected3(3);
...
// Flush particle rendering queue (i.e. render particle on screen)
void Particle_Flush(void) {
  ...
  if (_bTransFogHaze) { // setup haze/fog color and texture
    GFXColor glcolFH;
...
void Particle_Sort(BOOL b3D) {
  ...
  if (b3D) {
    qsort(&aiIndices[0], ctParticles, sizeof(INDEX), qsort_CompareZ3D);
  } else {
    qsort(&aiIndices[0], ctParticles, sizeof(INDEX), qsort_CompareZ);
  }
