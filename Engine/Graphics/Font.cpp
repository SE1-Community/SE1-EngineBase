 ...
 */
void CFontData::Make_t(const CTFileName &fnTexture, PIX pixCharWidth, PIX pixCharHeight, CTFileName &fnOrderFile,
                       BOOL bUseAlpha) {
  ...
  while ((strLettersOrder[iLettersInLine] != '\n') && iLettersInLine < strlen(strLettersOrder))
    iLettersInLine++;
  if (iLettersInLine <= 0)
    FatalError("Invalid font definition ASCII file.");
  ...
  while (iLetter < strlen(strLettersOrder)) { // for letters in one line
    for (INDEX iCurrentLetterColumn = 0; iCurrentLetterColumn < iLettersInLine;
         iCurrentLetterColumn++) { // test if we at the end of whole array
      if (iLetter >= strlen(strLettersOrder))
        break;
      // get char params
      ...
      for (INDEX iPixelLine = 0; iPixelLine < fd_pixCharHeight; iPixelLine++) { // for all of the letter's pixels
        for (INDEX iPixelColumn = 0; iPixelColumn < fd_pixCharWidth;
             iPixelColumn++) { // calculate current pixel's adress in font's texture
          ...
          if (ByteSwap(*puwPixel) & colPixMask) {
            ...
            if (iPixelColumn < pixCurrentStart)
              pixCurrentStart = iPixelColumn;
            // if examined pixel is wider in letter than last opaque pixel found, remember it as right-most pixel
            if (iPixelColumn > pixCurrentEnd)
              pixCurrentEnd = iPixelColumn;
          }
        }
      }
