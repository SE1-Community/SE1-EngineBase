...
static void UnpackFilter_OGL(INDEX iFilter, GLenum &eMagFilter, GLenum &eMinFilter) {
  switch (iFilter) {
    ...
    case 21:
      eMagFilter = GL_LINEAR;
      eMinFilter = GL_LINEAR_MIPMAP_NEAREST;
      break;
    case 222:
    case 22:
      eMagFilter = GL_LINEAR;
...
extern void MimicTexParams_OGL(CTexParams &tpLocal) {
  ...
  if (tpLocal.tp_iFilter != _tpGlobal[0].tp_iFilter) { // update OpenGL texture filters
  ...
  if (tpLocal.tp_eWrapU != _tpGlobal[GFX_iActiveTexUnit].tp_eWrapU
      || tpLocal.tp_eWrapV != _tpGlobal[GFX_iActiveTexUnit].tp_eWrapV) { // prepare temp vars
    ...
    if (_pGfx->gl_ulFlags & GLF_EXT_EDGECLAMP) {
      if (eWrapU == GL_CLAMP)
        eWrapU = GL_CLAMP_TO_EDGE;
      if (eWrapV == GL_CLAMP)
        eWrapV = GL_CLAMP_TO_EDGE;
    }
...
extern void UploadTexture_OGL(ULONG *pulTexture, PIX pixSizeU, PIX pixSizeV, GLenum eInternalFormat, BOOL bUseSubImage) {
  ...
  while (pixSizeU > 0 && pixSizeV > 0) {
    ...
    if (bUseSubImage) {
      pglTexSubImage2D(GL_TEXTURE_2D, iMip, 0, 0, pixSizeU, pixSizeV, GL_RGBA, GL_UNSIGNED_BYTE, pulTexture + pixOffset);
    } else {
      pglTexImage2D(GL_TEXTURE_2D, iMip, eInternalFormat, pixSizeU, pixSizeV, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                    pulTexture + pixOffset);
    }
    OGL_CHECKERROR;
  ...
  if (!_tpCurrent->tp_bSingleMipmap && pixSizeU != pixSizeV) { // prepare variables
    PIX pixSize = Max(pixSizeU, pixSizeV);
    ASSERT(pixSize <= 2048);
    ULONG *pulSrc = pulTexture + pixOffset - pixSize * 2;
    ULONG *pulDst = _aulLastMipmaps;
    // loop thru mipmaps
    while (pixSizeU > 0 || pixSizeV > 0) { // make next mipmap
      if (pixSizeU == 0)
        pixSizeU = 1;
      if (pixSizeV == 0)
        pixSizeV = 1;
...
extern INDEX AdjustDitheringType_OGL(GLenum eFormat, INDEX iDitheringType) {
  switch (eFormat) {
    ...
    case GL_RGBA8:
    case GL_LUMINANCE8:
    case GL_LUMINANCE8_ALPHA8: return NONE;
