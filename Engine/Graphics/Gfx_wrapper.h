...
extern void (*gfxSetOrtho)(const FLOAT fLeft, const FLOAT fRight, const FLOAT fTop, const FLOAT fBottom,
                           const FLOAT fNear, const FLOAT fFar, const BOOL bSubPixelAdjust);

// Set frustrum matrix
extern void (*gfxSetFrustum)(const FLOAT fLeft, const FLOAT fRight, const FLOAT fTop, const FLOAT fBottom, const FLOAT fNear, const FLOAT fFar);
...
class CTexParams {
  public:
    ...
    // Constructor
    inline CTexParams(void) {
      Clear();
      tp_bSingleMipmap = FALSE;
    };

    inline void Clear(void) {
      tp_iFilter = 00;
      tp_iAnisotropy = 0;
      tp_eWrapU = tp_eWrapV = (GfxWrap)NONE;
    };

    inline BOOL IsEqual(CTexParams tp) {
      return tp_iFilter == tp.tp_iFilter && tp_iAnisotropy == tp.tp_iAnisotropy && tp_eWrapU == tp.tp_eWrapU
...
// Return size of uploaded texture
extern SLONG gfxGetTextureSize(ULONG ulTexObject, BOOL bHasMipmaps = TRUE);

// Return bytes/pixels ratio for uploaded texture or texture format
