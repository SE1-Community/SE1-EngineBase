...
static void ogl_EnableClipping(void) {
  // only if supported
  if (!(_pGfx->gl_ulFlags & GLF_EXT_CLIPHINT))
    return;
...
static void ogl_DisableClipping(void) {
  ...
  if (gap_iOptimizeClipping < 2 || !(_pGfx->gl_ulFlags & GLF_EXT_CLIPHINT))
    return;
...
static void ogl_SetOrtho(const FLOAT fLeft, const FLOAT fRight, const FLOAT fTop, const FLOAT fBottom, const FLOAT fNear,
...
// set frustrum matrix
static void ogl_SetFrustum(const FLOAT fLeft, const FLOAT fRight, const FLOAT fTop, const FLOAT fBottom, const FLOAT fNear,
                           const FLOAT fFar) {
  // check API
  ASSERT(_pGfx->gl_eCurrentAPI == GAT_OGL);
