...
extern INDEX shd_iForceFlats
  = 0; // force all shadowmaps to be flat (internal!) - 0=don't, 1=w/o overbrighting, 2=w/ overbrighting
...
extern INDEX d3d_bAlternateDepthReads
  = FALSE; // should check delayed depth reads at the end of current frame (FALSE) or at begining of the next (TRUE)
...
static void GAPInfo(void) {
  ...
  const BOOL bNoDriverOGL = (_pGfx->go_hglRC == NULL);
  #ifdef SE1_D3D
  const BOOL bNoDriverD3D = (_pGfx->gl_pd3dDevice == NULL);
  #else
  const BOOL bNoDriverD3D = TRUE;
  #endif // SE1_D3D

  // in case of driver hasn't been initialized yet
  if ((bNoDriverOGL && bNoDriverD3D) || eAPI == GAT_NONE) {
    // be brief, be quick
  ...
  if (_pGfx->gl_ctRealTextureUnits < 2)
    CPrintF("not supported\n");
  else {
    if (gap_iUseTextureUnits > 1)
      CPrintF("enabled (using %d texture units)\n", gap_iUseTextureUnits);
    else
      CPrintF("disabled\n");
  ...
  if (_pGfx->gl_iMaxTextureAnisotropy >= 2) {
    ...
    CPrintF("- Anisotropic texture filtering: not supported\n");
  ...
  if (fMaxLODBias > 0) {
    ...
    CPrintF("- Texture LOD biasing: not supported\n");
  ...
  if (eAPI == GAT_OGL) {
    ...
    if (_pGfx->gl_iMaxTessellationLevel > 0) {
      if (_pGfx->gl_iTessellationLevel > 0) {
        ...
        if (gap_bForceTruform)
          CPrintF("(for all models)\n");
        else
          CPrintF("(only for Truform-ready models)\n");
    ...
    if (_pGfx->gl_ulFlags & GLF_EXT_COMPILEDVERTEXARRAY) {
      ...
      if (ogl_bUseCompiledVertexArrays) {
        ...
        if (CVA_bModels) {
          CPrintF("%smodels", strSep);
          strSep = "/";
        }
        if (CVA_b2D) {
          CPrintF("%sparticles", strSep);
        }
        CPrintF(")\n");
      } else
        CPrintF("disabled\n");
    ...
    if (!(_pGfx->gl_ulFlags & GLF_TEXTURECOMPRESSION))
      ...
      if (_pGfx->gl_ulFlags & GLF_EXTC_S3TC) {
        CPrintF("%sS3TC", strSep);
        strSep = ", ";
      }
      if (_pGfx->gl_ulFlags & GLF_EXTC_FXT1) {
        CPrintF("%sFTX1", strSep);
        strSep = ", ";
      }
      if (_pGfx->gl_ulFlags & GLF_EXTC_LEGACY) {
        CPrintF("%sold S3TC", strSep);
      }
    ...
    if (_pGfx->go_strWinExtensions != "")
      CPrintF("%s", ReformatExtensionsString(_pGfx->go_strWinExtensions));
  ...
  if (eAPI == GAT_D3D) {
    ...
    if (_pGfx->gl_ulFlags & GLF_D3D_HASHWTNL) {
      if (_pGfx->gl_ctMaxStreams < GFX_MINSTREAMS)
        CPrintF("cannot be used\n");
      else if (_pGfx->gl_ulFlags & GLF_D3D_USINGHWTNL)
        CPrintF("enabled (%d streams)\n", _pGfx->gl_ctMaxStreams);
      else
        CPrintF("disabled\n");
    } else
      CPrintF("not present\n");
    ...
    if (_pGfx->gl_iMaxTessellationLevel > 0) {
      if (!(_pGfx->gl_ulFlags & GLF_D3D_USINGHWTNL))
        CPrintF("not possible with SW T&L\n");
    ...
    if (_pGfx->gl_ulFlags & GLF_D3D_ZBIAS)
      CPrintF("supported\n");
    else
      CPrintF("not supported\n");
...
extern void UpdateGfxSysCVars(void) {
  ...
  if (_pGfx->gl_iMaxTextureAnisotropy > 1) {
    sys_bHasTextureAnisotropy = 1;
  }
  if (_pGfx->gl_fMaxTextureLODBias > 0) {
    sys_bHasTextureLODBias = 1;
  }
  if (_pGfx->gl_ctTextureUnits > 1) {
    sys_bHasMultitexturing = 1;
  }
  if (_pGfx->gl_iMaxTessellationLevel > 0) {
    sys_bHasTruform = 1;
  }
  ...
  if (_pGfx->gl_ulFlags & GLF_ADJUSTABLEGAMMA) {
    sys_bHasAdjustableGamma = 1;
  }
  if (_pGfx->gl_ulFlags & GLF_32BITTEXTURES) {
    sys_bHas32bitTextures = 1;
  }
  if (_pGfx->gl_ulFlags & GLF_VSYNC) {
    sys_bHasSwapInterval = 1;
  }
  if (_pGfx->gl_eCurrentAPI == GAT_OGL && !(_pGfx->gl_ulFlags & GLF_EXT_COMPILEDVERTEXARRAY)) {
    sys_bHasCVAs = 0;
  }
  #ifdef SE1_D3D
  if (_pGfx->gl_eCurrentAPI == GAT_D3D && !(_pGfx->gl_ulFlags & GLF_D3D_HASHWTNL)) {
...
extern BOOL ProbeMode(CTimerValue tvLast) {
  ...
  if (gfx_tmProbeDecay > 999)
    gfx_tmProbeDecay = 999;
  CTimerValue tvNow = _pTimer->GetHighPrecisionTimer();
  const TIME tmDelta = (tvNow - tvLast).GetSeconds();
  if (tmDelta > gfx_tmProbeDecay)
    return TRUE;
  return FALSE;
}
...
static void RecacheShadows(void) {
  ...
  if (shd_bCacheAll)
    ...
    CPrintF(TRANS("All shadows uncached.\n"));
}
...
static void PrepareTables(void) {
  ...
  for (i = -256 * 2; i < 256 * 4; i++)
    aubClipByte[i + 256 * 2] = (UBYTE)Clamp(i, 0L, 255L);
  // prepare fast sqrt tables
  for (i = 0; i < SQRTTABLESIZE; i++)
    aubSqrt[i] = (UBYTE)(sqrt((FLOAT)(i * 65536 / SQRTTABLESIZE)));
  for (i = 1; i < SQRTTABLESIZE; i++)
    auw1oSqrt[i] = (UWORD)(sqrt((FLOAT)(SQRTTABLESIZE - 1) / i) * 255.0f);
  auw1oSqrt[0] = MAX_UWORD;
  // prepare fast sin/cos table
  for (i = -256; i < 256 + 64; i++)
    afSinTable[i + 256] = Sin((i - 128) / 256.0f * 360.0f);
...
BOOL CGfxLibrary::SetDisplayMode(enum GfxAPIType eAPI, INDEX iAdapter, PIX pixSizeI, PIX pixSizeJ,
  ...
  if (eNewAPI == GAT_CURRENT)
    eNewAPI = gl_eCurrentAPI;
...
BOOL CGfxLibrary::StartDisplayMode(enum GfxAPIType eAPI, INDEX iAdapter, PIX pixSizeI, PIX pixSizeJ,
  ...
  // DirectX driver ?
    ...
    if (!bSuccess) {
      return FALSE; // what, didn't make it?
    }
...
void CGfxLibrary::StopDisplayMode(void) {
  ...
  // shutdown API
  if (gl_eCurrentAPI == GAT_OGL) { // OpenGL
    EndDriver_OGL();
    MonitorsOn(); // re-enable multimonitor support if disabled
    CDS_ResetMode();
  }
  #ifdef SE1_D3D
  else if (gl_eCurrentAPI == GAT_D3D) { // Direct3D
    EndDriver_D3D();
    MonitorsOn();
  }
  #endif // SE1_D3D
...
BOOL CGfxLibrary::SetCurrentViewport(CViewPort *pvp) {
  if (gl_eCurrentAPI == GAT_OGL) {
    return SetCurrentViewport_OGL(pvp);
  }
  #ifdef SE1_D3D
  if (gl_eCurrentAPI == GAT_D3D) {
    return SetCurrentViewport_D3D(pvp);
  }
  #endif // SE1_D3D
  if (gl_eCurrentAPI == GAT_NONE) {
    return TRUE;
  }
...
BOOL CGfxLibrary::LockDrawPort(CDrawPort *pdpToLock) {
  ...
  if (pdpToLock->dp_Width < 1 || pdpToLock->dp_Height < 1)
    return FALSE;
...
void CGfxLibrary::CreateWorkCanvas(PIX pixWidth, PIX pixHeight, CDrawPort **ppdpNew) {
  ...
  // create a window
  HWND hWnd = ::CreateWindowExA(0, WorkCanvasCLASS,
                                "",                                                                      // title
                                WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_POPUP, 0, 0, pixWidth, pixHeight, // window size
                                NULL, NULL,
                                NULL, // hInstance,
                                NULL);
  ASSERT(hWnd != NULL);
...
void CGfxLibrary::ReduceShadows(void) {
  ...
  if (_bShadowsUpdated) {
    ...
    slCachedShadowMemory = 0;
    slDynamicShadowMemory = 0;
    ctCachedShadows = 0;
    ctFlatShadows = 0;
    ctDynamicShadows = 0;
...
void CGfxLibrary::SwapBuffers(CViewPort *pvp) {
  ...
  if (gl_eCurrentAPI == GAT_OGL) {
    ...
    if (!(gl_ulFlags & GLF_EXT_COMPILEDVERTEXARRAY))
  ...
  // Direct3D
    ...
    if (_iLastVertexBufferSize != d3d_iVertexBuffersSize || (gl_iTessellationLevel < 1 && gap_iTruformLevel > 0)
  ...
  ogl_bUseCompiledVertexArrays = 0;
  if (CVA_b2D)
    ogl_bUseCompiledVertexArrays += 100;
  if (CVA_bWorld)
    ogl_bUseCompiledVertexArrays += 10;
  if (CVA_bModels)
  ...
  if ((gl_ulFlags & GLF_EXT_TBUFFER) && go_ctSampleBuffers > 1) {
    ...
    if (go_iCurrentWriteBuffer < 0)
      go_iCurrentWriteBuffer = go_ctSampleBuffers - 1;
...
static BOOL GenerateGammaTable(void) {
  // only if needed
  if (_fLastBrightness == gfx_fBrightness && _fLastContrast == gfx_fContrast && _fLastGamma == gfx_fGamma
      && _iLastLevels == gfx_iLevels && _fLastBiasR == gfx_fBiasR && _fLastBiasG == gfx_fBiasG && _fLastBiasB == gfx_fBiasB)
    return FALSE;
