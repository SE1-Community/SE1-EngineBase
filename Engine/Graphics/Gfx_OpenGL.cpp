...
int aiAttribList[] = {WGL_DRAW_TO_WINDOW_EXT,
                      TRUE,
...
void WIN_CheckError(BOOL bRes, const char *strDescription) {
  ...
  if (dwWindowsErrorCode == ERROR_SUCCESS)
    return; // ignore stupid 'successful' error
...
static INDEX ChoosePixelFormatTB(HDC hdc, const PIXELFORMATDESCRIPTOR *ppfd, PIX pixResWidth, PIX pixResHeight) {
  _TBCapability = FALSE;
  ...
  dummyhwnd = CreateWindowExA(WS_EX_TOPMOST, classname, "Dummy OGL window", WS_POPUP | WS_VISIBLE, 0, 0, pixResWidth,
                              pixResHeight, NULL, NULL, hInstance, NULL);
  // didn't make it?
  if (dummyhwnd == NULL) {
    UnregisterClassA(classname, hInstance);
    return 0;
  ...
  if (strstr((const char *)extensions, "WGL_EXT_extensions_string ") != NULL) { // windows extension string supported
  ...
  if ((strstr((const char *)wglextensions, "WGL_ARB_pixel_format ") != NULL)
    ...
    pwglChoosePixelFormatARB
      = (BOOL(__stdcall *)(HDC, const int *, const FLOAT *, UINT, int *, UINT *))pwglGetProcAddress("wglChoosePixelFormatARB");
    ...
    FreeMemory(piFormats);
  } else { // wglChoosePixelFormatARB extension does not exist :(
    iPixelFormat = 0;
  }
  BACKOFF
  return iPixelFormat;
...
BOOL CGfxLibrary::SetupPixelFormat_OGL(HDC hdc, BOOL bReport /*=FALSE*/) {
  ...
  if (ogl_iTBufferEffect > 0 && pixResWidth > 0 && pixResHeight > 0) { // lets T-buffer ... :)
    // CPrintF( "TBuffer init...\n");
    ogl_iTBufferSamples = (1L) << FastLog2(ogl_iTBufferSamples);
    if (ogl_iTBufferSamples < 2)
    ...
    if (_TBCapability)
      pglGetIntegerv(GL_SAMPLES_3DFX, (GLint *)&go_ctSampleBuffers);
    if (!iPixelFormat) {
      ogl_iTBufferEffect = 0;
      CPrintF(TRANS("TBuffer initialization failed.\n"));
    } else
      CPrintF(TRANS("TBuffer initialization passed (%d buffers in use).\n"), go_ctSampleBuffers);
  }
  ...
  // test acceleration
  memset(&pfd, 0, sizeof(pfd));
  if (!pwglDescribePixelFormat(hdc, iPixelFormat, sizeof(pfd), &pfd))
    return FALSE;
  ...
  if (pfd.iPixelType == PFD_TYPE_RGBA)
    strPixelType = "TYPE_RGBA";
  else if (pfd.iPixelType & PFD_TYPE_COLORINDEX)
    strPixelType = "TYPE_COLORINDEX";
  else
    strPixelType = "unknown";
  // prepare flags description
  CTString strFlags = "";
  if (pfd.dwFlags & PFD_DRAW_TO_WINDOW)
    strFlags += "DRAW_TO_WINDOW ";
  if (pfd.dwFlags & PFD_DRAW_TO_BITMAP)
    strFlags += "DRAW_TO_BITMAP ";
  if (pfd.dwFlags & PFD_SUPPORT_GDI)
    strFlags += "SUPPORT_GDI ";
  if (pfd.dwFlags & PFD_SUPPORT_OPENGL)
    strFlags += "SUPPORT_OPENGL ";
  if (pfd.dwFlags & PFD_GENERIC_ACCELERATED)
    strFlags += "GENERIC_ACCELERATED ";
  if (pfd.dwFlags & PFD_GENERIC_FORMAT)
    strFlags += "GENERIC_FORMAT ";
  if (pfd.dwFlags & PFD_NEED_PALETTE)
    strFlags += "NEED_PALETTE ";
  if (pfd.dwFlags & PFD_NEED_SYSTEM_PALETTE)
    strFlags += "NEED_SYSTEM_PALETTE ";
  if (pfd.dwFlags & PFD_DOUBLEBUFFER)
    strFlags += "DOUBLEBUFFER ";
  if (pfd.dwFlags & PFD_STEREO)
    strFlags += "STEREO ";
  if (pfd.dwFlags & PFD_SWAP_LAYER_BUFFERS)
    strFlags += "SWAP_LAYER_BUFFERS ";
  if (pfd.dwFlags & PFD_DEPTH_DONTCARE)
    strFlags += "DEPTH_DONTCARE ";
  if (pfd.dwFlags & PFD_DOUBLEBUFFER_DONTCARE)
    strFlags += "DOUBLEBUFFER_DONTCARE ";
  if (pfd.dwFlags & PFD_STEREO_DONTCARE)
    strFlags += "STEREO_DONTCARE ";
  if (pfd.dwFlags & PFD_SWAP_COPY)
    strFlags += "SWAP_COPY ";
  if (pfd.dwFlags & PFD_SWAP_EXCHANGE)
    strFlags += "SWAP_EXCHANGE ";
  if (strFlags == "")
    strFlags = "none";
...
void CGfxLibrary::TestExtension_OGL(ULONG ulFlag, const char *strName) {
  if (HasExtension(go_strExtensions, strName))
    AddExtension_OGL(ulFlag, strName);
}

// creates OpenGL drawing context
BOOL CGfxLibrary::CreateContext_OGL(HDC hdc) {
  if (!SetupPixelFormat_OGL(hdc, TRUE))
    return FALSE;
...
void CGfxLibrary::InitContext_OGL(void) {
  ...
  if ((gl_ulFlags & GLF_EXTC_ARB) || (gl_ulFlags & GLF_EXTC_FXT1) || (gl_ulFlags & GLF_EXTC_S3TC)
      || (gl_ulFlags & GLF_EXTC_LEGACY)) {
    gl_ulFlags |= GLF_TEXTURECOMPRESSION;
  }
  ...
  if (HasExtension(go_strExtensions, "GL_EXT_texture_lod_bias")) {
    ...
    if (gleError || glfRet < 0.1f || glfRet > 4.0f)
      glfRet = 4.0f;
  ...
  */
  ...
  if (go_strSupportedExtensions == "")
    go_strSupportedExtensions = "none";
...
BOOL CGfxLibrary::InitDriver_OGL(BOOL b3Dfx /*=FALSE*/) {
  ...
  } catch (char *strError) { // didn't make it :(
    if (gl_hiDriver != NONE)
...
BOOL CGfxLibrary::SetCurrentViewport_OGL(CViewPort *pvp) {
  ...
  if (pvp->vp_ctDisplayChanges < gl_ctDriverChanges) {
    ...
    // set it
    CTempDC tdc(pvp->vp_hWnd);
    if (!SetupPixelFormat_OGL(tdc.hdc))
      return FALSE;
    pvp->vp_ctDisplayChanges = gl_ctDriverChanges;
  }
  ...
  if (gl_pvpActive != NULL) {
    ...
    if (bOneWindow && gl_pvpActive->vp_hWnd != NULL && gl_pvpActive->vp_hWnd != pvp->vp_hWnd)
      return FALSE;
    // no need to set context if it is the same window as last time
    if (gl_pvpActive->vp_hWnd == pvp->vp_hWnd)
      return TRUE;
  }
  ...
  if (!pwglMakeCurrent(tdc.hdc, go_hglRC))
    return FALSE;
...
extern void SetTBufferEffect(BOOL bEnable) {
  ...
  if (ogl_iTBufferSamples < 2)
    ogl_iTBufferSamples = 4;
  // if supported
  if (_pGfx->gl_ulFlags & GLF_EXT_TBUFFER) { // disable multisampling if not required
    ASSERT(pglTBufferMask3DFX != NULL);
    if (ogl_iTBufferEffect == 0 || _pGfx->go_ctSampleBuffers < 2 || !bEnable)
      pglDisable(GL_MULTISAMPLE_3DFX);
    else {
      pglEnable(GL_MULTISAMPLE_3DFX);
      UINT uiMask = 0xFFFFFFFF;
      // set one buffer in case of motion-blur
      if (ogl_iTBufferEffect == 2)
        uiMask = (1UL) << _pGfx->go_iCurrentWriteBuffer;
