...
void CTextureData::CreateEffectTexture(PIX pixWidth, PIX pixHeight, MEX mexWidth, CTextureData *ptdBaseTexture,
...
static ULONG DetermineInternalFormat(CTextureData *pTD) {
  ...
  if (bAlphaChannel) {
    ...
    switch (iQuality) {
      case 3:
      case 2: ulInternalFormat = TS.ts_tfRGBA8; break; // uploaded as 32 bit or compressed
  ...
  // choose internal texture format for opaque textures
    ...
    switch (iQuality) {
      case 3:
      case 2: ulInternalFormat = TS.ts_tfRGB8; break; // uploaded as 32 bit or compressed
      case 1: break;                                  // uploaded as 16 bit (default)
      case 0:
        if (pTD->td_ulFlags & TEX_32BIT)
...
static void Convert(CTextureData *pTD) {
  ...
  UBYTE r, g, b, a;
  // for each frame
  for (INDEX iFr = 0; iFr < pTD->td_ctFrames; iFr++) { // get addresses of current frames (new and old)
    PIX pixFrameOffset = iFr * pixFrameSize;
    // for each pixel
    for (INDEX iPix = 0; iPix < pixMipSize; iPix++) { // read 16-bit pixel
  ...
  *inFile >> iVersion;
  ...
  do {
    ...
    // if this is chunk containing raw frames
      ...
      if (!(bHasContext || td_ulFlags & TEX_STATIC)) { // determine frames' size
      ...
      // if current version
        ...
        for (INDEX iFr = 0; iFr < td_ctFrames; iFr++) { // loop thru frames
    ...
    else if (idChunk == CChunkID("FXBF")) { // skip chunk in old versions
    ...
    } else if (idChunk == CChunkID("FXB2")) { // read effect buffers
      ASSERT(td_pixBufferWidth > 0 && td_pixBufferHeight > 0);
      ULONG ulSize = AllocEffectBuffers(this);
    ...
    else if (idChunk == CChunkID("FXDT")) { // read effect class
      ULONG ulGlobalEffect;
    ...
    else if (idChunk == CChunkID("DTLT")) { // skip chunk (this is here only for compatibility reasons)
      CTFileName fnTmp;
      *inFile >> fnTmp;
    } else {
  ...
  if (_bExport || (td_ulFlags & TEX_CONSTANT))
  ...
  if (td_ctFrames < 2 && (!gap_bAllowSingleMipmap || td_ctFineMipLevels > 1)) { // get last mipmap pointer
  ...
  if (!_bExport && bHasContext && !(td_ulFlags & TEX_STATIC))
    SetAsCurrent();
}
...
void CTextureData::Write_t(CTStream *outFile) // throw char *
{
  ...
  *outFile << td_ctFrames;
  ...
  if (td_ptegEffect == NULL) { // write chunk containing raw frames
    ...
    for (INDEX iFr = 0; iFr < td_ctFrames; iFr++) { // determine write params
      ULONG *pulCurrentFrame = td_pulFrames + (iFr * td_slFrameSize / BYTES_PER_TEXEL);
  ...
  // if exists global effect struct in texture
  else { // write chunk containing effect data
    ...
    *outFile << td_ptegEffect->teg_atesEffectSources.Count();
    ...
    FOREACHINDYNAMICARRAY(td_ptegEffect->teg_atesEffectSources, CTextureEffectSource,
                          itEffectSource) { // write type of effect source
    ...
    if (td_pubBuffer1 != NULL && td_pubBuffer2 != NULL) { // write chunk containing effect buffers
  ...
  if (td_ptdBaseTexture != NULL) { // write chunk containing base texture file name
...
void CTextureData::SetAsCurrent(INDEX iFrameNo /*=0*/, BOOL bForceUpload /*=FALSE*/) {
  ...
  if (td_ptegEffect != NULL) {
    ...
    INDEX iWantedMipLevel
      = td_iFirstMipLevel + ClampTextureSize(pixClampAreaSize, _pGfx->gl_pixMaxTextureDimension, pixWidth, pixHeight);
  ...
  if (bNeedUpload) {
    ...
    if (td_ctFrames > 1) {
      // animation textures
      for (INDEX iFr = 0; iFr < td_ctFrames; iFr++) { // determine frame offset and upload texture frame
        ULONG *pulCurrentFrame = td_pulFrames + (iFr * td_slFrameSize / BYTES_PER_TEXEL);
  ...
  if (td_ctFrames > 1)
    ulTexObject = ((ULONG *)td_ulObject)[iFrameNo]; // animation
  if (bUseProbe) {
...
void ProcessScript_t(const CTFileName &inFileName) // throw char *
{
  ...
  FOREVER {
    ...
    // if none of known key-words isn't recognised, throw error
      ...
      sprintf(err_str, TRANS("Unidentified key-word found (line: \"%s\") or unexpected end of file reached."), ld_line);
      throw(err_str);
    }
  }
  if (NoOfDataFound != 2)
...
SLONG CTextureData::GetUsedMemory(void) {
  ...
  if (td_pubBuffer1 != NULL)
    slUsed += 2 * GetEffectBufferSize(this); // two buffers
