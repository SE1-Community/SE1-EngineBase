...
typedef BOOL EnumDisplayDevices_t(PVOID Unused, // not used; must be NULL
                                  DWORD iDevNum, // specifies display device
                                  PDISPLAY_DEVICE lpDisplayDevice, // pointer to structure to receive display device information
                                  DWORD dwFlags); // flags to condition function behavior

typedef LONG ChangeDisplaySettingsEx_t(LPCSTR lpszDeviceName, LPDEVMODE lpDevMode, HWND hwnd, DWORD dwflags, LPVOID lParam);
...
void Mon_DisableEnable9x_t(BOOL bDisable) {
  ...
  HKEY key;
  LONG ret = RegOpenKeyEx(HKEY_CURRENT_CONFIG, _T("Display\\Settings"), 0, KEY_ENUMERATE_SUB_KEYS | KEY_SET_VALUE, &key);
  ...
  // there is at least one subkey for each secondary monitor, maybe more
  DWORD index = 0;
  ...
  while (ret == ERROR_SUCCESS) {
    ...
    if (ret == ERROR_SUCCESS) {
      ...
      if (ret == ERROR_SUCCESS) {
        ret = RegSetValueEx(subKey, _T("AttachToDesktop"), 0, REG_SZ, reinterpret_cast<const BYTE *>(attach), sizeof(TCHAR) * 2);
        RegCloseKey(subKey);
