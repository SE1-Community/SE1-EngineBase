...
static void UpdateClipPlane_D3D(void) {
  ...
  if (pulLast[0] == pulThis[0] && pulLast[1] == pulThis[1] && pulLast[2] == pulThis[2] && pulLast[3] == pulThis[3])
    return;
  // update (if supported!)
...
static void d3d_BlendFunc(GfxBlend eSrc, GfxBlend eDst) {
  ...
  D3D_CHECKERROR(hr);
  hr = _pGfx->gl_pd3dDevice->GetRenderState(D3DRS_DESTBLEND, (DWORD *)&d3dDst);
  D3D_CHECKERROR(hr);
...
static void d3d_SetColorMask(ULONG ulColorMask) {
  ...
  ASSERT(_pGfx->gl_eCurrentAPI == GAT_D3D);
  if (!(_pGfx->gl_ulFlags & GLF_D3D_COLORWRITES)) { // emulate disabling of all channels
    if (ulColorMask == 0) {
      d3d_EnableBlend();
  ...
  if ((ulColorMask & CT_RMASK) == CT_RMASK)
    ulBitMask |= D3DCOLORWRITEENABLE_RED;
  if ((ulColorMask & CT_GMASK) == CT_GMASK)
    ulBitMask |= D3DCOLORWRITEENABLE_GREEN;
  if ((ulColorMask & CT_BMASK) == CT_BMASK)
    ulBitMask |= D3DCOLORWRITEENABLE_BLUE;
  if ((ulColorMask & CT_AMASK) == CT_AMASK)
    ulBitMask |= D3DCOLORWRITEENABLE_ALPHA;
...
static void d3d_CullFace(GfxFace eFace) {
  ...
  if (gap_bOptimizeStateChanges) {
    if (GFX_eCullFace == D3DCULL_NONE && eFace == GFX_NONE)
      return;
    if (GFX_eCullFace == D3DCULL_CCW && eFace == eFrontFace)
      return;
    if (GFX_eCullFace == D3DCULL_CW && eFace == eBackFace)
      return;
  }
  ...
  if (eFace == eFrontFace)
    hr = _pGfx->gl_pd3dDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW);
  else if (eFace == eBackFace)
    hr = _pGfx->gl_pd3dDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_CW);
  else
    hr = _pGfx->gl_pd3dDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);
  D3D_CHECKERROR(hr);
  GFX_eCullFace = eFace;
...
static void d3d_FrontFace(GfxFace eFace) {
  ...
  if (GFX_eCullFace != GFX_NONE)
    d3d_CullFace(GFX_eCullFace);
}
...
static void d3d_SetViewMatrix(const FLOAT *pfMatrix /*=NULL*/) {
  ...
  if (pfMatrix == NULL && GFX_bViewMatrix == NONE && gap_bOptimizeStateChanges)
    return;
...
static void d3d_SetOrtho(const FLOAT fLeft, const FLOAT fRight, const FLOAT fTop, const FLOAT fBottom, const FLOAT fNear,
                         const FLOAT fFar, const BOOL bSubPixelAdjust /*=FALSE*/) {
  // check API and matrix type
  ASSERT(_pGfx->gl_eCurrentAPI == GAT_D3D);

  // cached?
  if (GFX_fLastL == fLeft && GFX_fLastT == fTop && GFX_fLastN == fNear && GFX_fLastR == fRight && GFX_fLastB == fBottom
      && GFX_fLastF == fFar && gap_bOptimizeStateChanges)
    return;
  GFX_fLastL = fLeft;
  GFX_fLastT = fTop;
  GFX_fLastN = fNear;
  ...
  const FLOAT fRpL = fRight + fLeft + fAdj;
  const FLOAT fRmL = fRight - fLeft;
  const FLOAT fFpN = fFar + fNear;
  const FLOAT fTpB = fTop + fBottom + fAdj;
  const FLOAT fTmB = fTop - fBottom;
  const FLOAT fFmN = fFar - fNear;
  const FLOAT afMatrix[16]
    = {2 / fRmL, 0, 0, 0, 0, 2 / fTmB, 0, 0, 0, 0, -1 / fFmN, 0, -fRpL / fRmL, -fTpB / fTmB, -fNear / fFmN, 1};
...
static void d3d_SetFrustum(const FLOAT fLeft, const FLOAT fRight, const FLOAT fTop, const FLOAT fBottom, const FLOAT fNear,
                           const FLOAT fFar) {
  // check API
  ASSERT(_pGfx->gl_eCurrentAPI == GAT_D3D);
  ...
  const FLOAT fRpL = fRight + fLeft;
  const FLOAT fTpB = fTop + fBottom;
  const FLOAT fFpN = fFar + fNear;
  const FLOAT fRmL = fRight - fLeft;
  const FLOAT fTmB = fTop - fBottom;
  const FLOAT fFmN = fFar - fNear;
  const FLOAT afMatrix[4][4] = {{2 * fNear / fRmL, 0, 0, 0},
                                {0, 2 * fNear / fTmB, 0, 0},
                                {fRpL / fRmL, fTpB / fTmB, -fFar / fFmN, -1},
                                {0, 0, -fFar * fNear / fFmN, 0}}; // set it
...
static void d3d_SetTextureWrapping(enum GfxWrap eWrapU, enum GfxWrap eWrapV) {
  ...
  D3D_CHECKERROR(hr);
  hr = _pGfx->gl_pd3dDevice->GetTextureStageState(GFX_iActiveTexUnit, D3DTSS_ADDRESSV, (ULONG *)&d3dtaV);
  D3D_CHECKERROR(hr);
...
static void d3d_GenerateTexture(ULONG &ulTexObject) {
  ...
  HRESULT hr = _pGfx->gl_pd3dDevice->CreateTexture(1, 1, 0, 0, (D3DFORMAT)TS.ts_tfRGBA8, D3DPOOL_MANAGED,
                                                   (LPDIRECT3DTEXTURE8 *)&ulTexObject);
  D3D_CHECKERROR(hr);
...
extern void d3dSetVertexShader(DWORD dwHandle) {
  ASSERT(_pGfx->gl_eCurrentAPI == GAT_D3D);
  if (_pGfx->gl_dwVertexShader == dwHandle)
    return;
