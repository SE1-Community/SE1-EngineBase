...
void FlipBitmap(UBYTE *pubSrc, UBYTE *pubDst, PIX pixWidth, PIX pixHeight, INDEX iFlipType, BOOL bAlphaChannel) {
  ...
  // flip horizontal
  if (iFlipType == 2 || iFlipType == 3) {            // for each row
    for (INDEX iRow = 0; iRow < pixHeight; iRow++) { // find row pointer
      PIX pixRowOffset = iRow * pixWidth;
      // for each pixel in row
      for (INDEX iPix = 0; iPix < pixHalfWidth; iPix++) { // transfer pixels
  ...
  if (iFlipType == 1 || iFlipType == 3) {                // for each row
    for (INDEX iRow = 0; iRow < pixHalfHeight; iRow++) { // find row pointers
      PIX pixBegOffset = iRow * pixWidth;
      PIX pixEndOffset = (pixHeight - 1 - iRow) * pixWidth;
      // for each pixel in row
      for (INDEX iPix = 0; iPix < pixWidth; iPix++) { // transfer pixels
...
pixLoopN:           
  ...
  } else { // NEAREST-NEIGHBOUR but with border preserving
...
void MakeMipmaps(INDEX ctFineMips, ULONG *pulMipmaps, PIX pixWidth, PIX pixHeight, INDEX iFilter /*=NONE*/) {
  ...
  while (pixCurrWidth > 1 && pixCurrHeight > 1) { // determine mip size
...
void ColorizeMipmaps(INDEX i1stMipmapToColorize, ULONG *pulMipmaps, PIX pixWidth, PIX pixHeight) {
  ...
  // loop thru mip-map levels
  while (pixCurrWidth > 1 && pixCurrHeight > 1) { // prepare current mip-level
...
// dither tables
static ULONG ulDither4[4][4] = {{0x0F0F0F0F, 0x07070707, 0x0D0D0D0D, 0x05050505},
...
void DitherBitmap(INDEX iDitherType, ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight, PIX pixCanvasWidth,
                  PIX pixCanvasHeight) {
  ...
  switch (iDitherType) { // low dithers
    case 1:
    ...
    case 2:
      ...
      goto ditherOrder;
    case 3: mmErrDiffMask = 0x0003000300030003; goto ditherError;
    // medium dithers
    case 4:
      pulDitherTable = &ulDither2[0][0];
      mmShift = 0;
    ...
    case 6:
      ...
      mmMask = 0x7F7F7F7F7F7F7F7F;
      goto ditherOrder;
    case 7: mmErrDiffMask = 0x0007000700070007; goto ditherError;
    // high dithers
    case 8:
      pulDitherTable = &ulDither3[0][0];
      mmShift = 0;
    ...
    case 9:
      ...
      mmMask = 0xFFFFFFFFFFFFFFFF;
      goto ditherOrder;
    case 10: mmErrDiffMask = 0x000F000F000F000F; goto ditherError;
    default:
...
void DitherMipmaps(INDEX iDitherType, ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight) {
  ...
  // loop thru mipmaps
  PIX pixMipSize;
  while (pixWidth > 0 && pixHeight > 0) { // dither one mipmap
...
static INDEX aiFilters[6][3] = {{0, 1, 16}, // minimum
                                {0, 2, 8},  // low
                                {1, 2, 7},  // medium
                                {1, 2, 3},  // high
                                {3, 4, 5},  // maximum
...
static void GenerateConvolutionMatrix(INDEX iFilter) {
  ...
  mm = iMc & 0xFFFF;
  mmMc = (mm << 48) | (mm << 32) | (mm << 16) | mm;
  mm = iMe & 0xFFFF;
  mmMe = (mm << 48) | (mm << 32) | (mm << 16) | mm;
  mm = iMm & 0xFFFF;
  mmMm = (mm << 48) | (mm << 32) | (mm << 16) | mm;
  mm = iEch & 0xFFFF;
  mmEch = (mm << 48) | (mm << 32) | (mm << 16) | mm;
  mm = iEm & 0xFFFF;
  mmEm = (mm << 48) | (mm << 32) | (mm << 16) | mm;
  mm = iCm & 0xFFFF;
  mmCm = (mm << 48) | (mm << 32) | (mm << 16) | mm;
}

// applies filter to bitmap
void FilterBitmap(INDEX iFilter, ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight, PIX pixCanvasWidth,
                  PIX pixCanvasHeight) {
  ...
  if (pixWidth < 4 || pixHeight < 4) { // don't blur it at all, but eventually only copy
    if (pulDst != pulSrc)
      memcpy(pulDst, pulSrc, pixCanvasWidth * pixCanvasHeight * BYTES_PER_TEXEL);
...
void AdjustBitmapColor(ULONG *pulSrc, ULONG *pulDst, PIX pixWidth, PIX pixHeight, SLONG const slHueShift,
...
void DrawTriangle_Mask(UBYTE *pubMaskPlane, SLONG slMaskWidth, SLONG slMaskHeight, struct PolyVertex2D *ppv2Vtx1,
                       struct PolyVertex2D *ppv2Vtx2, struct PolyVertex2D *ppv2Vtx3, BOOL bTransparency) {
