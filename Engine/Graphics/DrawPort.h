...
class ENGINE_API CDrawPort {
  ...
  public:
    ...
    // Set rendering in overlay mode
    inline void SetOverlappedRendering(BOOL bOverlay) {
      dp_bRenderingOverlay = bOverlay;
    };

    // Get rendering in overlay mode
    inline BOOL IsOverlappedRendering(void) const {
      return dp_bRenderingOverlay;
    };

    // Return unique drawport number
    ULONG GetID(void);

    // Get dimensions and location of drawport
    inline PIX GetWidth(void) const {
      return dp_Width;
    };

    inline PIX GetHeight(void) const {
      return dp_Height;
    };
    ...
    inline void SetTextCharSpacing(PIX pixSpacing) {
      dp_pixTextCharSpacing = pixSpacing;
    };

    inline void SetTextLineSpacing(PIX pixSpacing) {
      dp_pixTextLineSpacing = pixSpacing;
    };

    inline void SetTextScaling(FLOAT fScalingFactor) {
      dp_fTextScaling = fScalingFactor;
    };

    inline void SetTextAspect(FLOAT fAspectRatio) {
      dp_fTextAspect = fAspectRatio;
    };
    ...
    // Return width of entire text string (with scale included)
    ULONG GetTextWidth(const CTString &strText) const;

    // Write text string on drawport (left-aligned)
    ...
    void PutTexture(class CTextureObject *pTO, const PIXaabbox2D &boxScreen, const COLOR colBlend = 0xFFFFFFFF) const;
    void PutTexture(class CTextureObject *pTO, const PIXaabbox2D &boxScreen, const COLOR colUL, const COLOR colUR, const COLOR colDL, const COLOR colDR) const;
    void PutTexture(class CTextureObject *pTO, const PIXaabbox2D &boxScreen, const MEXaabbox2D &boxTexture, const COLOR colBlend = 0xFFFFFFFF) const;
    ...
    void AddTexture(const FLOAT fI0, const FLOAT fJ0, const FLOAT fI1, const FLOAT fJ1, const FLOAT fU0, const FLOAT fV0,
                    const FLOAT fU1, const FLOAT fV1, const COLOR col) const;

    // Add one textured quad to rendering queue (up-left start, counter-clockwise)
    ...
    // Add one flat triangle rendering queue (up-left start, counter-clockwise)
    void AddTriangle(const FLOAT fI0, const FLOAT fJ0, const FLOAT fI1, const FLOAT fJ1, const FLOAT fI2, const FLOAT fJ2,
    ...
    // Lock and unlock raster through drawport functions
    BOOL Lock(void);
    void Unlock(void);
    ...
    void Fill(PIX pixI, PIX pixJ, PIX pixWidth, PIX pixHeight, COLOR colUL, COLOR colUR, COLOR colDL, COLOR colDR) const;
    ...
    void RenderLensFlare(CTextureObject *pto, FLOAT fI, FLOAT fJ, FLOAT fSizeI, FLOAT fSizeJ, ANGLE aRotation, COLOR colLight) const;
...
void ENGINE_API Particle_RenderQuad3D(const FLOAT3D &vPos0, const FLOAT3D &vPos1, const FLOAT3D &vPos2, const FLOAT3D &vPos3, COLOR col);
