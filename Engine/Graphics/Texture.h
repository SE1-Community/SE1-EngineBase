...
// td_ulFlags bits (on disk and in memory)
...
class ENGINE_API CTextureData : public CAnimData {
  public:
    ...
    // Convert global mip level to the corresponding one of texture
    INDEX ClampMipLevel(FLOAT fMipFactor) const;

    // Get size in meters
    inline MEX GetWidth(void) const {
      return td_mexWidth;
    };

    inline MEX GetHeight(void) const {
      return td_mexHeight;
    };
    
    // Get size in pixels
    inline PIX GetPixWidth(void) const {
      return td_mexWidth >> td_iFirstMipLevel;
    };

    inline PIX GetPixHeight(void) const {
    ...
    // Set new texture mex width and change height remaining texture's aspect ratio
...
ENGINE_API extern void CreateTexture_t(const CTFileName &inFileName, MEX inMex, INDEX inMipmaps, BOOL bForce32bit);
ENGINE_API extern void CreateTexture_t(const CTFileName &inFileName, const CTFileName &outFileName, MEX inMex, INDEX inMipmaps, BOOL bForce32bit);
