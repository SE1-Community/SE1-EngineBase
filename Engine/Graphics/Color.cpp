...
void ColorToHSV(COLOR const colSrc, UBYTE &ubH, UBYTE &ubS, UBYTE &ubV) {
  ...
  if (ubV > 1) {
    ...
    ubS = (slD * 255) / ubV;
    if (ubR == ubV)
      ubH = 0 + (((SLONG)ubG - ubB) * 85) / (slD * 2);
    else if (ubG == ubV)
      ubH = 85 + (((SLONG)ubB - ubR) * 85) / (slD * 2);
    else
      ubH = 170 + (((SLONG)ubR - ubG) * 85) / (slD * 2);
  }
}
...
BOOL IsBlack(COLOR const col) {
  UBYTE ubR, ubG, ubB;
  ColorToRGB(col, ubR, ubG, ubB);
  if (ubR < GRAY_TRESHOLD && ubG < GRAY_TRESHOLD && ubB < GRAY_TRESHOLD)
    return TRUE;
  return FALSE;
}
...
BOOL IsWhite(COLOR const col) {
  UBYTE ubR, ubG, ubB;
  ColorToRGB(col, ubR, ubG, ubB);
  if (ubR > WHITE_TRESHOLD && ubG > WHITE_TRESHOLD && ubB > WHITE_TRESHOLD)
    return TRUE;
  return FALSE;
}
...
BOOL CompareChroma(COLOR col1, COLOR col2) {
  ...
  // trivial?
  if (slMax1 < GRAY_TRESHOLD || slMax2 < GRAY_TRESHOLD)
    return TRUE;
  ...
  if (Abs(slR - slR2) > (GRAY_TRESHOLD / 2))
    return FALSE;
  if (Abs(slG - slG2) > (GRAY_TRESHOLD / 2))
    return FALSE;
  if (Abs(slB - slB2) > (GRAY_TRESHOLD / 2))
    return FALSE;
  return TRUE;
}
...
COLOR AdjustColor(COLOR const col, SLONG const slHueShift, SLONG const slSaturation) {
  // nothing?
  if (slHueShift == 0 && slSaturation == 256)
    return col;
  // saturation?
  COLOR colRes = col;
  UBYTE ubA = (col & CT_AMASK) >> CT_ASHIFT;
  if (slSaturation != 256) { // calculate gray factor
    UBYTE ubR, ubG, ubB;
    ColorToRGB(col, ubR, ubG, ubB);
...
COLOR AddColors(COLOR col1, COLOR col2) {
  ...
  if (col2 == 0)
    return col1;
  if (col1 == 0xFFFFFFFF || col2 == 0xFFFFFFFF)
    return 0xFFFFFFFF;
  COLOR colRet;
  __asm {
    xor     ebx,ebx
