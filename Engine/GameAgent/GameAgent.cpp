...
#define CHK_BUFFSTRLEN \
  if ((iLen < 0) || (iLen > BUFFSZSTR)) { \
    CPrintF("\n" \
...
#define CLEANMSSRUFF1 \
  closesocket(_sock); \
  WSACleanup();

#define CLEANMSSRUFF2 \
  if (cResponse) \
    free(cResponse); \
  closesocket(_sock); \
  WSACleanup();
...
void _sendHeartbeat(INDEX iChallenge) {
  CTString strPacket;
  if (!ga_bMSLegacy) {
    strPacket.PrintF("0;challenge;%d;players;%d;maxplayers;%d;level;%s;gametype;%s;version;%s;product;%s", iChallenge,
                     _pNetwork->ga_srvServer.GetPlayersCount(), _pNetwork->ga_sesSessionState.ses_ctMaxPlayers,
                     _pNetwork->ga_World.wo_strName, _getGameModeName(_getSP()->sp_gmGameMode), _SE_VER_STRING,
  ...
  if (iLen > 0) {
    if (!ga_bMSLegacy) {
      ...
      switch (_szBuffer[0]) {
        ...
        case 2: // server status request
        {
          ...
          strPacket.PrintF("0;players;%d;maxplayers;%d;level;%s;gametype;%s;version;%s;gamename;%s;sessionname;%s",
                           _pNetwork->ga_srvServer.GetPlayersCount(), _pNetwork->ga_sesSessionState.ses_ctMaxPlayers,
                           _pNetwork->ga_World.wo_strName, _getGameModeName(_getSP()->sp_gmGameMode), _SE_VER_STRING,
                           _pShell->GetString("sam_strGameName"), _pShell->GetString("gam_strSessionName"));
        ...
        case 4: // ping
        {
          // just send back 1 byte and the amount of players in the server (this could be useful in some cases for external
          // scripts)
          CTString strPacket;
      ...
      if (sPch1) {
        ...
        strPacket.PrintF(PCKQUERY, _pShell->GetString("sam_strGameName"), _SE_VER_STRING,
                         //_pShell->GetString("net_strLocalHost"),
                         strLocation, _pShell->GetString("gam_strSessionName"), _pShell->GetINDEX("net_iPort"),
                         _pNetwork->ga_World.wo_strName, _getGameModeName(_getSP()->sp_gmGameMode),
                         _pNetwork->ga_srvServer.GetPlayersCount(), _pNetwork->ga_sesSessionState.ses_ctMaxPlayers,
                         _pShell->GetINDEX("gam_bFriendlyFire"), _pShell->GetINDEX("gam_bWeaponsStay"),
                         _pShell->GetINDEX("gam_bAmmoStays"), _pShell->GetINDEX("gam_bHealthArmorStays"),
                         _pShell->GetINDEX("gam_bAllowHealth"), _pShell->GetINDEX("gam_bAllowArmor"),
                         _pShell->GetINDEX("gam_bInfiniteAmmo"), _pShell->GetINDEX("gam_bRespawnInPlace"));
        ...
        CTString strPacket;
        strPacket.PrintF(PCKINFO, _pShell->GetString("gam_strSessionName"), _pShell->GetINDEX("net_iPort"),
                         _pNetwork->ga_World.wo_strName, _getGameModeName(_getSP()->sp_gmGameMode),
                         _pNetwork->ga_srvServer.GetPlayersCount(), _pNetwork->ga_sesSessionState.ses_ctMaxPlayers);
        _sendPacketTo(strPacket, &_sinFrom);
        ...
        strPacket.PrintF(PCKBASIC, _pShell->GetString("sam_strGameName"), _SE_VER_STRING,
                         //_pShell->GetString("net_strLocalHost"));
                         strLocation);
        _sendPacketTo(strPacket, &_sinFrom);
...
extern void GameAgent_EnumTrigger(BOOL bInternet) {
  ...
  if (!bInternet && ga_bMSLegacy) {
    ...
    if (!ga_bMSLegacy) {
      ...
      SOCKET _sock = NULL;
      u_int uiMSIP;
      int iErr, iLen, iDynsz, iEnctype = 0;
      u_short usMSport = MSPORT;

      u_char ucGamekey[] = {SERIOUSSAMKEY}, ucGamestr[] = {SERIOUSSAMSTR}, *ucSec = NULL, *ucKey = NULL;

      char *cFilter = "", *cWhere = "", cMS[128] = {0}, *cResponse = NULL, *cMsstring = NULL, *cSec = NULL;
      ...
      iLen = _snprintf(cMsstring, BUFFSZSTR, PCK, ucGamestr, iEnctype, ucKey, ucGamestr, cWhere, cFilter);
  ...
  if (!ga_bMSLegacy) {
    ...
    if (iLen != -1) {
      ...
      switch (_szBuffer[0]) {
        ...
        default: CPrintF("Unknown enum packet ID %x!\n", _szBuffer[0]); break;
      }
    }
  } else {
    // MSLegacy
