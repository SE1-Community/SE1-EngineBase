...
functions:
  ...
  export INDEX GetMyPlayerIndex(void) {
    ...
    // if not found, still return a relatively logical value
    return 15;
  }
  ...
  export void DumpSync_t(CTStream &strm, INDEX iExtensiveSyncCheck) {
    ...
    for (INDEX iGUID = 0; iGUID < sizeof(en_pcCharacter.pc_aubGUID); iGUID++) {
      strm.FPrintF_t("%02X", en_pcCharacter.pc_aubGUID[iGUID]);
    }

    strm.FPrintF_t("\n");
    strm.FPrintF_t("appearance: ");

    for (INDEX iAppearance = 0; iAppearance < MAX_PLAYERAPPEARANCE; iAppearance++) {
      strm.FPrintF_t("%02X", en_pcCharacter.pc_aubAppearance[iAppearance]);
    }
