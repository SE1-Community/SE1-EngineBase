...
// [Cecil] NOTE: Could be put back in place since it's not a VC6 compiler anymore
...
static inline void AddAccelerationOnPlane(FLOAT3D &vCurrentVelocity, const FLOAT3D &vDesiredVelocity, 
                                          FLOAT fAcceleration, FLOAT fDecceleration, const FLOAT3D &vPlaneNormal)
{
  FLOAT3D vCurrentParallel, vCurrentOrthogonal;
...
properties:
 ...
 41 FLOAT en_fCollisionDamageFactor = 20.0f, // collision damage amount multiplier (adjustable)
...
functions:
  ...
  // Used for recursive adding
  export void AddToMoversDuringMoving(void) {
    // if already added
    if (en_lnInMovers.IsLinked()) {
      // do nothing
      return;
  ...
  export const ANGLE3D &GetDesiredRotation(void) const {
  ...
  export const FLOAT3D &GetDesiredTranslation(void) const {
  ...
  export void GiveImpulseTranslationRelative(const FLOAT3D &vImpulseSpeedRelative) {
    ...
    plImpulseSpeedAbsolute.RelativeToAbsolute(CPlacement3D(FLOAT3D(0.0f, 0.0f, 0.0f), en_plPlacement.pl_OrientationAngle));
  ...
  export void LaunchAsPropelledProjectile(const FLOAT3D &vImpulseSpeedRelative, CMovableEntity *penLauncher) {
  ...
  export void LaunchAsFreeProjectile(const FLOAT3D &vImpulseSpeedRelative, CMovableEntity *penLauncher) {
    en_vCurrentTranslationAbsolute += vImpulseSpeedRelative*en_mRotation;
    //en_vCurrentTranslationAbsolute += penLauncher->en_vCurrentTranslationAbsolute;
    ...
    FLOAT fParallelMultiplier, FLOAT fNormalMultiplier, FLOAT fMaxExitSpeed, TIME tmControl)
  {
    ...
    en_vCurrentTranslationAbsolute = vCurrentParallel * fParallelMultiplier
                                   + vCurrentNormal * fNormalMultiplier + vDirection*fStrength;
  ...
  export ANGLE GetRelativePitch(const FLOAT3D &vDirection) {
    ...
    FLOAT fUp = +vDirection(1) * en_mRotation(1, 2)
  ...
  void UpdateOneSectorForce(CBrushSector &bsc, FLOAT fRatio) {
    ...
    for (INDEX iSphere = 0; iSphere < absSpheres.Count(); iSphere++) {
      ...
      if (bsc.bsc_bspBSPTree.TestSphere(FLOATtoDOUBLE(ms.ms_vRelativeCenter0), ms.ms_fR) >= 0) {
        bAnyIn = TRUE;
        break;
      }
    }
    ...
    for (INDEX iForce = 0; iForce < _aefForces.Count(); iForce++) {
      if (penEntity == _aefForces[iForce].ef_penEntity && iForceType == _aefForces[iForce].ef_iForceType) {
  ...
  void TestFields(INDEX &iUpContent, INDEX &iDnContent, FLOAT &fImmersionFactor) {
    ...
    {for (INDEX iForce = 0; iForce < _aefForces.Count(); iForce++) {
      ...
      _aefForces[iForce].ef_penEntity->GetForce(_aefForces[iForce].ef_iForceType,
                                                en_plPlacement.pl_PositionVector,  fsGravity, fsField);
  ...
  void TestBreathing(CContentType &ctUp) {
    ...
    BOOL bCanBreathe = (ctUp.ct_ulFlags & CTF_BREATHABLE_LUNGS) && (en_ulPhysicsFlags & EPF_HASLUNGS)
    ...
    if (bCanBreathe) {
      ...
      if (llBreathDelay > 2/*_pTimer->TickQuantum*2*/) {
  ...
  BOOL IsStandingOnPolygon(CBrushPolygon *pbpo) {
    ...
    if (en_pciCollisionInfo == NULL || !(en_pciCollisionInfo->ci_ulFlags & CIF_CANSTANDONHANDLE)) {
  ...
  BOOL IsPolygonBelowPoint(CBrushPolygon *pbpo, const FLOAT3D &vPoint, FLOAT fMaxDist) {
    ...
    if ((pbpo->bpo_ulFlags & BPOF_PASSABLE) || !AllowForGroundPolygon(pbpo)) {
      // it cannot be below
      _pfPhysicsProfile.StopTimer(CPhysicsProfile::PTI_ISSTANDINGONPOLYGON);
      return FALSE;
    }
    ...
    if (fCos >= -stReference.st_fClimbSlopeCos && fCos < 0 || stReference.st_ulFlags & STF_SLIDEDOWNSLOPE) {
      // it cannot be below
      _pfPhysicsProfile.StopTimer(CPhysicsProfile::PTI_ISSTANDINGONPOLYGON);
      return FALSE;
    }
  ...
  BOOL WouldFallInNextPosition(void) {
    ...
    if (en_pbpoStandOn != NULL && IsPolygonBelowPoint(en_pbpoStandOn, en_vNextPosition, en_fStepDnHeight)) {
      // it won't fall
      return FALSE;
    }
    ...
    if (en_penReference != NULL && en_penReference->en_RenderType == RT_BRUSH
     && !(en_penReference->en_ulFlags & ENF_ZONING) && en_penReference->en_pbrBrush != NULL)
    {
  ...
  BOOL TryToGoUpstairs(const FLOAT3D &vTranslationAbsolute, const CSurfaceType &stHit, BOOL bHitStairsOrg) {
    ...
    if ((ctDn.ct_ulFlags & CTF_SWIMABLE) && !(ctUp.ct_ulFlags & CTF_SWIMABLE) && en_fImmersionFactor > 0.3f) {
      // add immersion height to up step
      if (en_pciCollisionInfo != NULL) {
        fStairsHeight = fStairsHeight * 2 + en_fImmersionFactor * (en_pciCollisionInfo->ci_fMaxHeight - en_pciCollisionInfo->ci_fMinHeight);
    ...
    for (INDEX iStep = 0; iStep < 3; iStep++) {
      ...
      if (cm.cm_fMovementFraction < 1.0f) {
        ...
        BOOL bEarlyClipAllowed = 
          ...
          iStep == 2 && (vHitPlane % en_vGravityDir < -stHit.st_fClimbSlopeCos || bHitStairsNow);

        // if early clip is allowed
        if (bEarlyClipAllowed || bSlidingAllowed) {
          // try to go to where it is clipped (little bit before)
          en_vNextPosition = en_plPlacement.pl_PositionVector + avTranslation[iStep] * (cm.cm_fMovementFraction * 0.98f);

          if (bSlidingAllowed && iStep != 2) {
            FLOAT3D vSliding = cm.cm_plClippedPlane.ProjectDirection(avTranslation[iStep] * (1.0f - cm.cm_fMovementFraction))
  ...
  BOOL TryToMove(CMovableEntity * penPusher, BOOL bTranslate, BOOL bRotate) {
    ...
    BOOL bIgnoreRotation = !bRotate || ((ulCIFlags & CIF_IGNOREROTATION) || ((ulCIFlags & CIF_IGNOREHEADING)
    ...
    if (cmMove.cm_fMovementFraction > 1.0f) {
      ...
      if (bTranslate && en_penReference != NULL && (en_ulPhysicsFlags & EPF_TRANSLATEDBYGRAVITY)
    ...
    // if the move is clipped
      ...
      if ((en_ulPhysicsFlags & EPF_TRANSLATEDBYGRAVITY) && !(en_ulPhysicsFlags & EPF_FLOATING)
       && (((en_vGravityDir % (FLOAT3D &)cmMove.cm_plClippedPlane) < (en_vGravityDir % en_vReferencePlane)))) {
      ...
      if (((en_ulPhysicsFlags & EPF_ONBLOCK_MASK) == EPF_ONBLOCK_BOUNCE) && bTranslate) {
        ...
        GetParallelAndNormalComponents(en_vMoveTranslation, cmMove.cm_plClippedPlane, vNormal, vParallel);
      ...
      if (penPusher != NULL && (cmMove.cm_penHit->en_ulPhysicsFlags & EPF_PUSHABLE)) {
        ...
        FLOAT3D vRadius = cmMove.cm_penHit->en_plPlacement.pl_PositionVector - penPusher->en_plPlacement.pl_PositionVector;
      ...
      // if entity slides if blocked
      } else if (((en_ulPhysicsFlags & EPF_ONBLOCK_MASK) == EPF_ONBLOCK_SLIDE)
        ...
        if (bTranslate) {
          ...
          if (_ctSliding == 0) {
            ...
            vSliding = cmMove.cm_plClippedPlane.ProjectDirection(en_vMoveTranslation * (1.0f - cmMove.cm_fMovementFraction));
            _ctSliding++;
          ...
          // if second plane
            ...
            // get sliding velocity
            GetParallelComponent(en_vMoveTranslation * (1.0f - cmMove.cm_fMovementFraction), _vSlideDir, vSliding);
          ...
          if (cmMove.cm_pbpoHit != NULL) {
            CSurfaceType &stHit = en_pwoWorld->wo_astSurfaceTypes[cmMove.cm_pbpoHit->bpo_bppProperties.bpp_ubSurfaceType];

            // if it is not beeing pushed, and it can climb stairs
            if (penPusher == NULL && (en_ulPhysicsFlags & EPF_ONBLOCK_MASK) == EPF_ONBLOCK_CLIMBORSLIDE) {
              ...
              if ((vHitPlane % en_vGravityDir > -stHit.st_fClimbSlopeCos) || bHitStairs) {
                ...
                // if can go upstairs
                if ((2 * fSlidingVertical2 <= fSliding2) && TryToGoUpstairs(en_vMoveTranslation, stHit, bHitStairs)) {
                  // movement is ok
                  _pfPhysicsProfile.StopTimer(CPhysicsProfile::PTI_TRYTOTRANSLATE);
                  return FALSE;
                }
          ...
          vSliding += _vSlideOffDir * (ClampUp(en_vMoveTranslation.Length(), 0.5f) / 100.0f);
  ...
  export void PreMoving(void) {
    ...
    // [Cecil] 2021-02-20: Removed PreMovingOld() since old demos are gonna be obsolete either way
    ...
    {FOREACHINLIST(CEntity, en_lnInParent, en_lhChildren, itenChild) {
      ...
      if ((itenChild->en_ulPhysicsFlags & EPF_MOVABLE) && !((CMovableEntity *)&*itenChild)->en_lnInMovers.IsLinked()) {
    ...
    // used for normalizing from SI units to game ticks
    ...
    FLOAT fBouyancy = (1 - (ctDn.ct_fDensity / en_fDensity) * en_fImmersionFactor
                         - (ctUp.ct_fDensity / en_fDensity) * (1 - en_fImmersionFactor));

    FLOAT fSpeedModifier = ctDn.ct_fSpeedMultiplier * en_fImmersionFactor + ctUp.ct_fSpeedMultiplier * (1 - en_fImmersionFactor);
    FLOAT fFluidFriction = ctDn.ct_fFluidFriction * en_fImmersionFactor + ctUp.ct_fFluidFriction * (1 - en_fImmersionFactor);
    FLOAT fControlMultiplier = ctDn.ct_fControlMultiplier * en_fImmersionFactor + ctUp.ct_fControlMultiplier * (1 - en_fImmersionFactor);
    ...
    // if the entity is affected by gravity
      ...
      if (en_penReference != NULL) {
        ...
        // if not on a steep slope
          ...
          AddAccelerationOnPlane(vTranslationAbsolute, vDesiredTranslationAbsolute,
                                 ACC * fPlaneYAbs * fPlaneYAbs * fFriction * fControlMultiplier,
                                 DEC * fPlaneYAbs * fPlaneYAbs * fFriction * fControlMultiplier, en_vReferencePlane);
        }
      ...
      // if it doesn't have a reference entity
        ...
        if (_pTimer->GetGameTick() - en_llJumped < CTimer::InTicks(en_tmMaxJumpControl)) {
          ...
          AddAccelerationOnPlane(vTranslationAbsolute, vDesiredTranslationAbsolute,
                                 ...
                                 DEC * fControlMultiplier * en_fJumpControlMultiplier, FLOATplane3D(en_vGravityDir, 0));
        }

        // if wants to jump and can jump
        if (fJump < -0.01f && _pTimer->GetGameTick() > en_llLastVerticalMove + CTimer::InTicks(0.25f)) {
          // jump
          vTranslationAbsolute += en_vGravityDir * fJump;
          en_llJumped = _pTimer->GetGameTick();
          en_pbpoStandOn = NULL;
    ...
    if (en_pbpoStandOn == NULL && (vTranslationAbsolute.ManhattanNorm() > 1E-5f || en_vReferencePlane % en_vGravityDir < 0.0f)) {
      en_penReference = NULL;
      en_vReferencePlane = FLOAT3D(0.0f, 0.0f, 0.0f);
      en_iReferenceSurface = 0;
    }
  ...
  export void PostMoving(void) {
     ...
     && en_aDesiredRotationRelative.ManhattanNorm() == 0)
    {
      ...
      // if there is no reference
        ...
        // !!!! test for forces also when implemented
        if ((!(en_ulPhysicsFlags & (EPF_TRANSLATEDBYGRAVITY | EPF_ORIENTEDBYGRAVITY)) || en_fGravityA == 0.0f)) {
          // mark for removing from list of movers
          en_ulFlags |= ENF_INRENDERING;
        }
      }
    ...
    if (_bPredictionActive && (IsPredictable() || IsPredictor())) {
      ...
      if (IsPredictor()) {
        ...
        if (penTail->en_llLastPredictionHead == llTickNow || penTail->en_llLastPredictionHead < 0) {
          ...
          penTail->en_vPredError += en_plPlacement.pl_PositionVector - penTail->en_vLastHead;
