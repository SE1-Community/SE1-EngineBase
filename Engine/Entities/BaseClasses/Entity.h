...
#define DUMPVECTOR2(strDes, v) strm.FPrintF_t("%s:  %g,%g,%g\n", strDes, (v)(1), (v)(2), (v)(3))
...
class ENGINE_API CEntity {
  ...
  public:
    ...
    void CopyOneProperty(CEntityProperty &epPropertySrc, CEntityProperty &epPropertyDest, CEntity &enOther, ULONG ulFlags);
    ...
    virtual void SetPlacement_internal(const CPlacement3D &plNew, const FLOATmatrix3D &mRotation, BOOL bNear);
    ...
    // Make predictor (complete raw copy with all states/variables and making predictor/predicted links)
    #define COPY_PREDICTOR (1UL << 2)

    virtual void Copy(CEntity &enOther, ULONG ulFlags);

    virtual CEntity &operator=(CEntity &enOther) {
      ASSERT(FALSE);
      return *this;
    };
    ...
    void FindEntitiesInRange(const FLOATaabbox3D &boxRange, CDynamicContainer<CEntity> &cen, BOOL bCollidingOnly);
  ...
  public:
    ...
    // Get sound length of a sound component
    double GetSoundLength(SLONG idSoundComponent);

    // Get main model blend color
    COLOR GetModelColor(void) const;

    // Set main model blend color
  ...
  public:
    ...
    inline const CPlacement3D &GetPlacement(void) const {
      return en_plPlacement;
    };
    
    // Get entity rotation matrix
    inline const FLOATmatrix3D &GetRotationMatrix(void) const {
      return en_mRotation;
    };

    // This one is used in rendering - returns lerped placement between ticks
    ...
    inline ULONG GetFlags(void) const {
      return en_ulFlags;
    };

    // Spawn flags
    inline void SetSpawnFlags(ULONG ulFlags) {
      en_ulSpawnFlags = ulFlags;
    };

    inline ULONG GetSpawnFlags(void) const {
      return en_ulSpawnFlags;
    };

    // Physics flags
    void SetPhysicsFlags(ULONG ulFlags);

    inline ULONG GetPhysicsFlags(void) const {
      return en_ulPhysicsFlags;
    };

    // Collision flags
    void SetCollisionFlags(ULONG ulFlags);

    inline ULONG GetCollisionFlags(void) const {
      return en_ulCollisionFlags;
    };

    // Prediction checks
    inline BOOL IsPredictor(void) const {
      return en_ulFlags & ENF_PREDICTOR;
    };

    inline BOOL IsPredicted(void) const {
      return en_ulFlags & ENF_PREDICTED;
    };

    inline BOOL IsPredictable(void) const {
      return en_ulFlags & ENF_PREDICTABLE;
    };
    ...
    inline enum RenderType GetRenderType(void) {
      return en_RenderType;
    };

    // Get entity's class
    inline CEntityClass *GetClass(void) {
      return en_pecClass;
    };
    ...
    // Get brush data
    inline CBrush3D *GetBrush(void) {
      return en_pbrBrush;
    };

    // Get model data
    inline CModelObject *GetModelObject(void) {
      return en_pmoModelObject;
    };

    // Get SKA model data
    inline CModelInstance *GetModelInstance(void) {
      return en_pmiModelInstance;
    };

    // Get terrain data
    inline CTerrain *GetTerrain(void) {
      return en_ptrTerrain;
    };

    // Get parent entity
    inline CEntity *GetParent(void) {
      return en_penParent;
    };
    ...
    // Get world polygon from an index
    CBrushPolygon *GetWorldPolygonPointer(INDEX ibpo);

    // Get index of a world polygon
    ...
    // Get and set the pointer to the predictor/predicted (auto-generated by ECC feature)
