...
CEntity::~CEntity(void) {
  ...
  // Models are always destructed on End(), but brushes and terrains are not,
  // so if the pointer is not NULL, then it must be a brush or terrain.
...
// Check if the entity is an empty brush
...
const CTString &CEntity::GetDescription(void) const {
  // name abd some more verbose data
...
void CEntity::SetPredictionTime(TICK llAdvance) {
  NOTHING; // don't use time prediction by default
}

// Called by engine to get the upper time limit
TICK CEntity::GetPredictionTime(void) {
  return -1; // don't use time prediction by default
}

// Get maximum allowed range for predicting this entity
FLOAT CEntity::GetPredictionRange(void) {
  return UpperLimit(0.0f); // cli_fPredictEntitiesRange is the limit by default
}
...
BOOL CEntity::IsPredictionHead(void) {
  ...
  if ((en_ulFlags & ENF_PREDICTOR) && _pTimer->GetGameTick() <= _pNetwork->ga_sesSessionState.ses_llPredictionHeadTick) {
    // it cannot be head of the chain
    return FALSE;
  }
...
void CEntity::End(void) {
  ...
  // NOTE: Must not remove from thinker/mover list here, or CServer::ProcessGameTick() might crash!
  End_internal();
}
...
void CEntity::End_internal(void) {
  ...
  switch (en_RenderType) {
    ...
    default: ASSERTALWAYS("Unsupported entity type");
  }
...
void CEntity::Teleport(const CPlacement3D &plNew, BOOL bTelefrag) {
  ASSERT(GetFPUPrecision() == FPT_24BIT);
       ...
       || en_RenderType == RT_SKAMODEL || en_RenderType == RT_SKAEDITORMODEL) {
        ...
        if (pmme->CheckForCollisionNow(pmme->en_iCollisionBox, &ppenObstacleDummy)) {
          CPrintF("Entity '%s' was teleported inside a wall at (%g,%g,%g)!\n", GetName(),
                  en_plPlacement.pl_PositionVector(1), en_plPlacement.pl_PositionVector(2), en_plPlacement.pl_PositionVector(3));
        }
      }
    }
  }
...
// Set placement of this entity (use this only in WED)
...
// Fall down to floor (use this only in WED)
...
void CEntity::SetPlacement_internal(const CPlacement3D &plNew, const FLOATmatrix3D &mRotation, BOOL bNear) {
  ...
  if (en_RenderType == RT_BRUSH || en_RenderType == RT_FIELDBRUSH) {
    ...
    if (bHasShadows) {
      ...
      en_pwoWorld->FindShadowLayers(en_pbrBrush->GetFirstMip()->bm_boxBoundingBox, FALSE, FALSE /* no directional */);
      _penLightUpdating = NULL;
    }
...
static void CheckTerrainForShadingInfo(CTerrain *ptrTerrain) {
  ...
  FLOAT fDistance = TestRayCastHit(ptrTerrain, pen->en_mRotation, pen->en_plPlacement.pl_PositionVector, _vHandle,
                                   _vHandle + vTerrainNormal, _fNearDistance, FALSE, plHitPlane, vHitPoint);
...
void CEntity::FindShadingInfo(void) {
  ...
  if (_pbpoNear != NULL) {
    ...
    MEX2D vmexShadow;
    _pbpoNear->bpo_mdShadow.GetTextureCoordinates(_pbpoNear->bpo_pbplPlane->bpl_pwplWorking->wpl_mvRelative, _vNearPoint, vmexShadow);
...
void CCollisionInfo::FromModel(CEntity *penModel, INDEX iBox) {
  ...
  if (ctSpheres == 1 && ci_absSpheres[0].ms_vCenter(1) == 0 && ci_absSpheres[0].ms_vCenter(2) == 0 && ci_absSpheres[0].ms_vCenter(3) == 0) {
    ci_ulFlags |= CIF_IGNOREROTATION;
  }

  if (iBoxType == LENGTH_EQ_WIDTH && ci_absSpheres[0].ms_vCenter(1) == 0 && ci_absSpheres[0].ms_vCenter(3) == 0) {
...
void CEntity::FindSectorsAroundEntity(void) {
  ...
  FLOATobbox3D boxEntity = FLOATobbox3D(en_boxSpatialClassification, en_plPlacement.pl_PositionVector, en_mRotation);
         ...
         && boxEntity.HasContactWith(FLOATobbox3D(itbsc->bsc_boxBoundingBox))) {
          ...
          if (itbsc->bsc_bspBSPTree.TestSphere(FLOATtoDOUBLE(vSphereCenter), FLOATtoDOUBLE(fSphereRadius)) >= 0) {
            ...
            if (itbsc->bsc_bspBSPTree.TestBox(boxdEntity) >= 0) {
              ...
              if (en_RenderType == RT_BRUSH || en_RenderType == RT_FIELDBRUSH || en_RenderType == RT_TERRAIN) { // brushes first
...
void CEntity::FindSectorsAroundEntityNear(void) {
  ...
  // make oriented bounding box of the entity
  FLOATobbox3D oboxEntity = FLOATobbox3D(en_boxSpatialClassification, en_plPlacement.pl_PositionVector, en_mRotation);
  ...
  FOREACHINLIST(CBrushSector, bsc_lnInActiveSectors, lhActive, itbsc) {
    ...
    BOOL bIn = (pbsc->bsc_boxBoundingBox.HasContactWith(boxEntity))
...
BOOL CEntity::HandleEvent(const CEntityEvent &ee) {
  ...
  // It's possible to override this in some class if some other way of event handling is desired.
  return FALSE;
}
...
void CEntity::FindEntitiesInRange(const FLOATaabbox3D &boxRange, CDynamicContainer<CEntity> &cen, BOOL bCollidingOnly) {
  ASSERT(GetFPUPrecision() == FPT_24BIT);
...
void CEntity::SetModel(SLONG idModelComponent) {
  ...
  CEntityComponent *pecModel = en_pecClass->ComponentForTypeAndID(ECT_MODEL, idModelComponent);
...
void CEntity::SetModelMainTexture(SLONG idTextureComponent) {
  ASSERT(en_RenderType == RT_MODEL || en_RenderType == RT_EDITORMODEL);
  CEntityComponent *pecTexture = en_pecClass->ComponentForTypeAndID(ECT_TEXTURE, idTextureComponent);
...
void CEntity::PrecacheClass(SLONG slID, INDEX iUser) {
...
const CTFileName &CEntity::FileNameForComponent(SLONG slType, SLONG slID) {
  // find the component
  CEntityComponent *pec = en_pecClass->ComponentForTypeAndID((EntityComponentType)slType, slID);
...
void CEntity::GetEntityPointRatio(const FLOAT3D &vRatio, FLOAT3D &vAbsPoint, BOOL bLerped) {
  ...
  if (en_RenderType != RT_MODEL && en_RenderType != RT_EDITORMODEL && en_RenderType != RT_BRUSH
   && en_RenderType != RT_SKAMODEL && en_RenderType != RT_SKAEDITORMODEL) {
    ASSERT(FALSE);
...
void CEntity::InflictDirectDamage(CEntity *penToDamage, CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmount,
                                  const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) {
  ASSERT(GetFPUPrecision() == FPT_24BIT);

  // if any of the entities are not allowed to execute now
  if (!IsAllowedForPrediction() || !penToDamage->IsAllowedForPrediction() || !penInflictor->IsAllowedForPrediction()) {
    // do nothing
    return;
  }
...
void CEntity::InflictRangeDamage(CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmount,
  ...
  FOREACHINDYNAMICCONTAINER(cenInRange, CEntity, iten) {
     ...
     || (en.en_RenderType == RT_BRUSH) && CheckBrushRangeDamage(en, vCenter, fMinD, vHitPos))
    {
      ...
      if (fAmount > 0) {
        ...
        en.ReceiveDamage(penInflictor, dmtType, fAmount, vHitPos, (vHitPos - vCenter).Normalize());
      }
    }
  }
}
...
void CEntity::InflictBoxDamage(CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmount, const FLOATaabbox3D &box) {
  ASSERT(GetFPUPrecision() == FPT_24BIT);
  ...
  FOREACHINDYNAMICCONTAINER(cenInRange, CEntity, iten) {
    ...
    if (fDamageAmount > 0) {
      // inflict damage to it
      en.ReceiveDamage(penInflictor, dmtType, fDamageAmount, box.Center(),
  ...
  if (_pNetwork->ga_ulDemoMinorVersion <= 2) {
    ...
    FOREACHINDYNAMICCONTAINER(en_pwoWorld->wo_cenEntities, CEntity, iten) {
      ...
      if (pen->en_ulPhysicsFlags & EPF_MOVABLE) {
        ...
        // add to movers if the gravity has changed
        pmen->AddToMovers();
      }
    }
...
void CEntity::ReceiveDamage(CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmount,
                            const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) {
...
void CEntity::Read_t(CTStream *istr) {
  ...
  if (istr->PeekID_t() == CChunkID("ENT4")) { // entity v4
    ...
    (*istr) >> (ULONG &)en_RenderType >> en_ulPhysicsFlags >> en_ulCollisionFlags >> en_ulSpawnFlags >> en_ulFlags;
    ...
    (*istr) >> (ULONG &)en_RenderType >> en_ulPhysicsFlags >> en_ulCollisionFlags >> en_ulFlags;
  }
...
void CEntity::Write_t(CTStream *ostr) {
  ...
  (*ostr) << (ULONG &)en_RenderType << en_ulPhysicsFlags << en_ulCollisionFlags << en_ulSpawnFlags << en_ulFlags;
...
void CEntity::ChecksumForSync(ULONG &ulCRC, INDEX iExtensiveSyncCheck) {
  if (iExtensiveSyncCheck > 0) {
    CRC_AddLONG(ulCRC, en_ulFlags & ~(ENF_SELECTED | ENF_INRENDERING | ENF_VALIDSHADINGINFO | ENF_FOUNDINGRIDSEARCH
...
void CEntity::DumpSync_t(CTStream &strm, INDEX iExtensiveSyncCheck) {
  ...
  if (iExtensiveSyncCheck > 0) {
    strm.FPrintF_t("en_fSpatialClassificationRadius: %g(%08x)\n", en_fSpatialClassificationRadius,
                   (ULONG &)en_fSpatialClassificationRadius);
  }

  strm.FPrintF_t("placement: %g,%g,%g : %g,%g,%g\n",
                 en_plPlacement.pl_PositionVector(1), en_plPlacement.pl_PositionVector(2), en_plPlacement.pl_PositionVector(3),
                 en_plPlacement.pl_OrientationAngle(1), en_plPlacement.pl_OrientationAngle(2), en_plPlacement.pl_OrientationAngle(3));

  if (iExtensiveSyncCheck > 0) {
    strm.FPrintF_t("placement raw:\n %08X %08X %08X\n %08X %08X %08X\n", (ULONG &)en_plPlacement.pl_PositionVector(1),
                   (ULONG &)en_plPlacement.pl_PositionVector(2), (ULONG &)en_plPlacement.pl_PositionVector(3),
                   (ULONG &)en_plPlacement.pl_OrientationAngle(1), (ULONG &)en_plPlacement.pl_OrientationAngle(2),
                   (ULONG &)en_plPlacement.pl_OrientationAngle(3));

    strm.FPrintF_t("matrix:\n %g %g %g\n %g %g %g\n %g %g %g\n", en_mRotation(1, 1), en_mRotation(1, 2), en_mRotation(1, 3),
                   en_mRotation(2, 1), en_mRotation(2, 2), en_mRotation(2, 3), en_mRotation(3, 1), en_mRotation(3, 2),
                   en_mRotation(3, 3));

    strm.FPrintF_t("matrix raw:\n %08X %08X %08X\n %08X %08X %08X\n %08X %08X %08X\n", (ULONG &)en_mRotation(1, 1),
                   (ULONG &)en_mRotation(1, 2), (ULONG &)en_mRotation(1, 3), (ULONG &)en_mRotation(2, 1),
                   (ULONG &)en_mRotation(2, 2), (ULONG &)en_mRotation(2, 3), (ULONG &)en_mRotation(3, 1),
                   (ULONG &)en_mRotation(3, 2), (ULONG &)en_mRotation(3, 3));
...
// Return amount of memory used by entity
...
CEntity *CEntity::CreateEntity(const CPlacement3D &plPlacement, SLONG idClass) {
  CEntityComponent *pecClassComponent = en_pecClass->ComponentForTypeAndID(ECT_CLASS, idClass);
