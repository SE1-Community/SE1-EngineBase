...
BOOL IsDerivedFromClass(CEntity *pen, const char *pstrClassName) {
  ...
  for (CDLLEntityClass *pdecDLLClass = pen->GetClass()->ec_pdecDLLClass; pdecDLLClass != NULL;
...
void CEntity::End_internal(void) {
  ...
  switch (en_RenderType) {
    // if it is brush
    case RT_BRUSH: DiscardCollisionInfo(); break;
    // if it is field brush
    case RT_FIELDBRUSH: DiscardCollisionInfo(); break;
    ...
    case RT_SKAEDITORMODEL:
      ...
      en_psiShadingInfo = NULL;
      break;
    case RT_TERRAIN: DiscardCollisionInfo(); break;
...
void CCollisionInfo::FromModel(CEntity *penModel, INDEX iBox) {
  ...
  if (ctSpheres == 1 && ci_absSpheres[0].ms_vCenter(1) == 0 && ci_absSpheres[0].ms_vCenter(2) == 0
...
void CEntity::GetModelVerticesAbsolute(CStaticStackArray<FLOAT3D> &avVertices, FLOAT fNormalOffset, FLOAT fMipFactor) {
  ASSERT(en_RenderType == RT_MODEL || en_RenderType == RT_EDITORMODEL || en_RenderType == RT_SKAMODEL
         || en_RenderType == RT_SKAEDITORMODEL);
  // get placement
 ...
 */
void CEntity::InflictDirectDamage(CEntity *penToDamage, CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmmount,
...
static inline FLOAT IntensityAtDistance(FLOAT fDamageAmmount, FLOAT fHotSpotRange, FLOAT fFallOffRange, FLOAT fDistance) {
...
void CEntity::InflictRangeDamage(CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmmount, const FLOAT3D &vCenter,
  ...
  FOREACHINDYNAMICCONTAINER(cenInRange, CEntity, iten) {
    ...
    FLOAT3D vHitPos;
    FLOAT fMinD;
    if ((en.en_RenderType == RT_MODEL || en.en_RenderType == RT_EDITORMODEL || en.en_RenderType == RT_SKAMODEL
         || en.en_RenderType == RT_SKAEDITORMODEL)
...
void CEntity::InflictBoxDamage(CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmmount, const FLOATaabbox3D &box) {
  ASSERT(GetFPUPrecision() == FPT_24BIT);
  ...
  FOREACHINDYNAMICCONTAINER(cenInRange, CEntity, iten) {
    ...
    if (fDamageAmmount > 0) {
      // inflict damage to it
      en.ReceiveDamage(penInflictor, dmtType, fDamageAmmount, box.Center(),
...
void CEntity::ReceiveDamage(CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamageAmmount, const FLOAT3D &vHitPoint,
...
void CEntity::ChecksumForSync(ULONG &ulCRC, INDEX iExtensiveSyncCheck) {
  if (iExtensiveSyncCheck > 0) {
    CRC_AddLONG(ulCRC, en_ulFlags
                         & ~(ENF_SELECTED | ENF_INRENDERING | ENF_VALIDSHADINGINFO | ENF_FOUNDINGRIDSEARCH | ENF_WILLBEPREDICTED
                             | ENF_PREDICTABLE));
...
void CEntity::DumpSync_t(CTStream &strm, INDEX iExtensiveSyncCheck) // throw char *
{
  ...
  if (iExtensiveSyncCheck > 0) {
    strm.FPrintF_t("en_ulFlags:          0x%08X\n", en_ulFlags
                                                      & ~(ENF_SELECTED | ENF_INRENDERING | ENF_VALIDSHADINGINFO
  ...
  strm.FPrintF_t("placement: %g,%g,%g : %g,%g,%g\n", en_plPlacement.pl_PositionVector(1), en_plPlacement.pl_PositionVector(2),
                 en_plPlacement.pl_PositionVector(3), en_plPlacement.pl_OrientationAngle(1),
                 en_plPlacement.pl_OrientationAngle(2), en_plPlacement.pl_OrientationAngle(3));
  if (iExtensiveSyncCheck > 0) {
...
SLONG CEntity::GetUsedMemory(void) {
  ...
  switch (en_RenderType) {
    ...
    case CEntity::RT_EDITORMODEL: slUsedMemory += en_pmoModelObject->GetUsedMemory(); break;
    case CEntity::RT_SKAMODEL:
    case CEntity::RT_SKAEDITORMODEL: slUsedMemory += en_pmiModelInstance->GetUsedMemory();
    default: break;
  }
...
// apply some damage to the entity (see event EDamage for more info)
void CLiveEntity::ReceiveDamage(CEntity *penInflictor, enum DamageType dmtType, FLOAT fDamage, const FLOAT3D &vHitPoint,
                                const FLOAT3D &vDirection) {
  CEntityPointer penThis = this; // keep this entity alive during this function
