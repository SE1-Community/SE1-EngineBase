  ...
  for (INDEX iProperty = 0; iProperty < ctProperties; iProperty++) {
    ...
    if (pepProperty == NULL) {
      ...
      switch (eptType) {
        ...
        case CEntityProperty::EPT_MODELINSTANCE: SkipModelInstance_t(istrm); break;
        // if it is ANIMOBJECT
        ...
        case CEntityProperty::EPT_FILENAME:
          ...
          for (;;) {
            if (!FileExists(PROPERTY(pepProperty->ep_slOffset, CTFileName))) {
              ...
              if (GetReplacingFile(PROPERTY(pepProperty->ep_slOffset, CTFileName), fnReplacingFile, FILTER_ALL FILTER_END)) {
