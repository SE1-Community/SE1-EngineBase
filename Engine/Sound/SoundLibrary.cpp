...
static BOOL StartUp_dsound(CSoundLibrary &sl, BOOL bReport = TRUE) {
  ...
  if (bReport)
    CPrintF(TRANS("Direct Sound initialization ...\n"));
  ...
  pDirectSoundCreate
    = (HRESULT(WINAPI *)(GUID FAR *, LPDIRECTSOUND FAR *, IUnknown FAR *))GetProcAddress(_hInstDS, "DirectSoundCreate");
  ...
  SLONG slBufferSize = (SLONG)(ceil(snd_tmMixAhead * sl.sl_SwfeFormat.nSamplesPerSec) * sl.sl_SwfeFormat.wBitsPerSample / 8
                               * sl.sl_SwfeFormat.nChannels);
  ...
  if (snd_iInterface == 2) {
    ...
    hResult
      = sl.sl_pKSProperty->QuerySupport(DSPROPSETID_EAX_ListenerProperties, DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE, &ulSupport);
    if (hResult != DS_OK || !(ulSupport & KSPROPERTY_SUPPORT_SET))
      return DSFail(sl, TRANS("  ! EAX error: Cannot query property support.\n"));
    // made it - EAX's on!
  ...
  sl.sl_slDecodeBufferSize
    = sl.sl_slMixerBufferSize * ((44100 + sl.sl_SwfeFormat.nSamplesPerSec - 1) / sl.sl_SwfeFormat.nSamplesPerSec);
  ...
  sl.sl_pswDecodeBuffer
    = (SWORD *)AllocMemory(sl.sl_slDecodeBufferSize + 4); // (+4 because of linear interpolation of last samples)
  ...
  if (bReport) {
    ...
    CPrintF(TRANS("  %dHz, %dbit, %s, mix-ahead: %gs\n"), sl.sl_SwfeFormat.nSamplesPerSec, sl.sl_SwfeFormat.wBitsPerSample,
            strDevice, snd_tmMixAhead);
...
static BOOL StartUp_waveout(CSoundLibrary &sl, BOOL bReport = TRUE) {
  ...
  sl.sl_bUsingEAX = FALSE;
  if (bReport)
    CPrintF(TRANS("WaveOut initialization ...\n"));
  ...
  sl.sl_slMixerBufferSize = (SLONG)(ceil(snd_tmMixAhead * sl.sl_SwfeFormat.nSamplesPerSec) * sl.sl_SwfeFormat.wBitsPerSample / 8
                                    * sl.sl_SwfeFormat.nChannels);
  ...
  if (bReport) {
    CPrintF(TRANS("  parameters: %d Hz, %d bit, stereo, mix-ahead: %gs\n"), sl.sl_SwfeFormat.nSamplesPerSec,
            sl.sl_SwfeFormat.wBitsPerSample, snd_tmMixAhead);
...
static void SetFormat_internal(CSoundLibrary &sl, CSoundLibrary::SoundFormat EsfNew, BOOL bReport) {
  ...
  if (bReport)
    CPrintF(TRANS("Setting sound format ...\n"));
  if (sl.sl_EsfFormat == CSoundLibrary::SF_NONE) {
    if (bReport)
      CPrintF(TRANS("  (no sound)\n"));
    return;
  }
...
void CSoundLibrary::ClearLibrary(void) {
  ...
  if (sl_hwoWaveOut != NULL) { // reset wave out play buffers (stop playing)
    MMRESULT res;
...
BOOL CSoundLibrary::SetEnvironment(INDEX iEnvNo, FLOAT fEnvSize /*=0*/) {
  ...
  if (fEnvSize < 1 || fEnvSize > 99)
    fEnvSize = 8;
  HRESULT hResult;
  hResult = sl_pKSProperty->Set(DSPROPSETID_EAX_ListenerProperties, DSPROPERTY_EAXLISTENER_ENVIRONMENT, NULL, 0, &iEnvNo,
                                sizeof(DWORD));
  if (hResult != DS_OK)
    return DSFail(*this, TRANS("  ! EAX error: Cannot set environment.\n"));
  hResult = sl_pKSProperty->Set(DSPROPSETID_EAX_ListenerProperties, DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE, NULL, 0, &fEnvSize,
                                sizeof(FLOAT));
  if (hResult != DS_OK)
...
void CSoundLibrary::UpdateSounds(void) {
  ...
  if (sl_bUsingEAX && _fLastPanning != snd_fEAXPanning) { // determine new panning
    _fLastPanning = snd_fEAXPanning;
    FLOAT fPanLeft = -1.0f;
    FLOAT fPanRight = +1.0f;
    if (snd_fEAXPanning < 0)
      fPanRight = MINPAN + Abs(snd_fEAXPanning) * MAXPAN; // pan left
    if (snd_fEAXPanning > 0)
      fPanLeft = MINPAN + Abs(snd_fEAXPanning) * MAXPAN; // pan right
    // set and apply
    HRESULT hr1, hr2, hr3;
...
static void CopyMixerBuffer_waveout(CSoundLibrary &sl) {
  MMRESULT res;
  SLONG slOffset = 0;
  for (INDEX iBuffer = 0; iBuffer < sl.sl_awhWOBuffers.Count(); iBuffer++) { // skip prepared buffer
    WAVEHDR &wh = sl.sl_awhWOBuffers[iBuffer];
...
static SLONG PrepareSoundBuffer_dsound(CSoundLibrary &sl) {
  ...
  // done
  // CPrintF( "LP/LW: %5d / %5d,   RP/RW: %5d / %5d,    MIX: %5d\n", dwCurrentCursor, _iWriteOffset, dwCurrentCursor2,
  // _iWriteOffset2, slDataToMix); // grgr
  return slDataToMix;
}
...
static SLONG PrepareSoundBuffer_waveout(CSoundLibrary &sl) {
  ...
  SLONG slDataToMix = 0;
  for (INDEX iBuffer = 0; iBuffer < sl.sl_awhWOBuffers.Count(); iBuffer++) { // if done playing
    WAVEHDR &wh = sl.sl_awhWOBuffers[iBuffer];
