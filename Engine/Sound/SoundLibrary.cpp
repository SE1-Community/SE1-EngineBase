...
static void DSPlayBuffers(CSoundLibrary &sl) {
  ...
  if (sl.sl_bUsingEAX) {
    ...
    for (INDEX i = 0; i < 10; i++) {
      // shoud be enough to screw interrupts
...
static BOOL StartUp_dsound(CSoundLibrary &sl, BOOL bReport = TRUE) {
  ...
  sl.sl_slDecodeBufferSize = sl.sl_slMixerBufferSize * ((44100 + sl.sl_SwfeFormat.nSamplesPerSec - 1) / sl.sl_SwfeFormat.nSamplesPerSec);
  ...
  if (bReport) {
    ...
    CPrintF(TRANS("  %dHz, %dbit, %s, mix-ahead: %gs\n"), sl.sl_SwfeFormat.nSamplesPerSec,
            sl.sl_SwfeFormat.wBitsPerSample, strDevice, snd_tmMixAhead);
...
BOOL CSoundLibrary::SetEnvironment(INDEX iEnvNo, FLOAT fEnvSize /*=0*/) {
  ...
  HRESULT hResult;
  hResult = sl_pKSProperty->Set(DSPROPSETID_EAX_ListenerProperties, DSPROPERTY_EAXLISTENER_ENVIRONMENT,
                                NULL, 0, &iEnvNo, sizeof(DWORD));
  if (hResult != DS_OK) {
    return DSFail(*this, TRANS("  ! EAX error: Cannot set environment.\n"));
  }

  hResult = sl_pKSProperty->Set(DSPROPSETID_EAX_ListenerProperties, DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE,
                                NULL, 0, &fEnvSize, sizeof(FLOAT));
...
static SLONG PrepareSoundBuffer_dsound(CSoundLibrary &sl) {
  ...
  /*CPrintF("LP/LW: %5d / %5d,   RP/RW: %5d / %5d,    MIX: %5d\n", dwCurrentCursor,
            _iWriteOffset, dwCurrentCursor2, _iWriteOffset2, slDataToMix);*/ // grgr
...
void CSoundLibrary::MixSounds(void) {
  ...
  /*if (!_bOpened) {
    _filMixerBuffer = fopen( "d:\\MixerBufferDump.raw", "wb");
  }
