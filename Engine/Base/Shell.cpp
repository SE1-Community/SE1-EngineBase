...
void LoadCommands(void) {
  ...
  for (INDEX i = 0; i < afnmCmds.Count(); i++) {
    ...
    // declare it
    extern void Declaration(ULONG ulQualifiers, INDEX istType, CShellSymbol & ssNew, INDEX(*pPreFunc)(INDEX), void (*pPostFunc)(INDEX));
...
CTString CShell::GetValue(const CTString &strName) {
  ...
  switch (stt) {
    case STT_STRING: strValue = *(CTString *)pss->ss_pvValue; break;
    case STT_INDEX: strValue.PrintF("%d", *(INDEX *)pss->ss_pvValue); break;
    case STT_FLOAT: strValue.PrintF("%g", *(FLOAT *)pss->ss_pvValue); break;
...
void CShell::SetValue(const CTString &strName, const CTString &strValue) {
  ...
  switch (stt) {
    case STT_STRING: *(CTString *)pss->ss_pvValue = strValue; break;
    case STT_INDEX: ((CTString &)strValue).ScanF("%d", (INDEX *)pss->ss_pvValue); break;
  ...
  try {
    ...
    FOREACHINDYNAMICARRAY(sh_assSymbols, CShellSymbol, itss) {
      ...
      // if string
        ...
        // dump as index
        fScript.FPrintF_t("persistent extern %sCTString %s=\"%s\";\n", strUser, ss.ss_strName,
                          (const char *)ScriptEsc(*(CTString *)ss.ss_pvValue));
