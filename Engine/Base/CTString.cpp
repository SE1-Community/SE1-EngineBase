...
void CTString::ReadFromText_t(CTStream &strmStream, const CTString &strKeyword = "") {
  ASSERT(IsValid());
...
void CTString::ReadUntilEOF_t(CTStream &strmFile) {
  // get the file size
...
void CTString::Load_t(const class CTFileName &fnmFile) {
  ASSERT(IsValid());
...
void CTString::LoadKeepCRLF_t(const class CTFileName &fnmFile) {
  ASSERT(IsValid());
...
void CTString::Save_t(const class CTFileName &fnmFile) {
...
void CTString::SaveKeepCRLF_t(const class CTFileName &fnmFile) {
  // open the file for writing
  CTFileStream strmFile;
  strmFile.Create_t(fnmFile);
    ...
    if (*pcSrc != '^') { // advance to next char
      *pcDst = *pcSrc;
      ...
      switch (*pcSrc) {
        ...
        case 'f': pcSrc += FindZero((UBYTE *)pcSrc, 3); continue;
