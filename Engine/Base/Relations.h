...
#define FOREACHSRCLINK(head, iter) for (LISTITER(CRelationLnk, rl_lnSrc) iter(head); !iter.IsPastEnd(); iter.MoveToNext())
// make 'for' construct for walking a list in codomain member
#define FOREACHDSTLINK(head, iter) for (LISTITER(CRelationLnk, rl_lnDst) iter(head); !iter.IsPastEnd(); iter.MoveToNext())

// get a domain member related to a codomain member through a link
#define DST(plink, dstclass, dstmember) ((dstclass *)(((UBYTE *)(&(plink->GetDst()))) - offsetof(dstclass, dstmember)))
// get a codomain member that a domain member is related to through a link
#define SRC(plink, srcclass, srcmember) ((srcclass *)(((UBYTE *)(&(plink->GetSrc()))) - offsetof(srcclass, srcmember)))
