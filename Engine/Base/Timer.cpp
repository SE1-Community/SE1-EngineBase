...
// [Cecil] TODO: Add a tick constant for different uses instead of dividing by the following constant
...
CTimer::CTimer(BOOL bInterrupt) {
  ...
  if (tm_bInterrupt) {
    // period value [ms], resolution (0 == max. possible), callback, user, event type
    tm_TimerID = timeSetEvent(ULONG(TickQuantum * 1000.0f), 0, &CTimer_TimerFunc, 0, TIME_PERIODIC);
...
void CTimer::SetLerp(FLOAT fFactor) {
  ASSERT(this != NULL);
  tm_fLerpFactor = fFactor;
  tm_fLerpFactor2 = fFactor;
}

void CTimer::SetLerp2(FLOAT fFactor) {
  ASSERT(this != NULL);
  tm_fLerpFactor2 = fFactor;
}
...
CTString TimeToString(FLOAT fTime) {
  ...
  CTString strTime(0, "%02d:%02d:%02d", iHou, iMin, iSec);
  return strTime;
}
