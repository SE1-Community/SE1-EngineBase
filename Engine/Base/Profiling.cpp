...
void CProfileForm::CalibrateProfilingTimers(void) {
  enum Epsilons { ETI_TOTAL, ETI_STARTSTOP, ETI_START, ETI_STOP, ETI_DUMMY, ETI_COUNT };
...
#define REPEATCOUNT 10000
  ...
  __int64 llMinStartStopTime(0x7fffffffffffffff);
  {
    for (INDEX i = 0; i < REPEATCOUNT; i++) {
      ...
      __int64 llThisStartStopTime = pfCalibration.pf_aptTimers[ETI_STARTSTOP].pt_tvElapsed.tv_llValue;
  ...
  __int64 llMinStartTime(0x7fffffffffffffff);
  {
    for (INDEX i = 0; i < REPEATCOUNT; i++) {
      ...
      __int64 llThisStartTime = pfCalibration.pf_aptTimers[ETI_START].pt_tvElapsed.tv_llValue;
  ...
  __int64 llMinStopTime(0x7fffffffffffffff);
  {
    for (INDEX i = 0; i < REPEATCOUNT; i++) {
      ...
      pfCalibration.StopTimer(ETI_TOTAL);
      __int64 llThisStopTime = pfCalibration.pf_aptTimers[ETI_STOP].pt_tvElapsed.tv_llValue;
 ...
 */
CProfileForm::CProfileForm(const CTString &strTitle, const CTString &strAveragingUnits, INDEX ctCounters, INDEX ctTimers) {
...
void CProfileForm::StopTimer_internal(INDEX iTimer) {
  ...
  // ASSERT(pt.pt_tvStarted.tv_llValue>0);
  CTimerValue tvNow = CTimerValue(ReadTSC_profile()) - _tvCurrentProfilingEpsilon;
  pt.pt_tvElapsed += tvNow - pf_aptTimers[iTimer].pt_tvStarted - _tvStartStopEpsilon + _tvStartEpsilon;
  pf_ctRunningTimers--;
...
double CProfileForm::GetTimerPercentageOfModule(INDEX iTimer) {
  // must not report while some timers are active!
  ASSERT(pf_ctRunningTimers == 0);
  return pf_aptTimers[iTimer].pt_tvElapsed.GetSeconds() / pf_tvOverAllElapsed.GetSeconds() * 100;
}
...
void CProfileTimer::Report(char *&strBuffer, INDEX ctAveragingCount, CTimerValue tvAppElapsed, CTimerValue tvModElapsed) {
  if (ctAveragingCount == 0) {
    ctAveragingCount = 1;
  }
  ...
  if (pt_strAveragingName == "") {
    strBuffer += sprintf(
      strBuffer, "%-45s: %6.2f%% %6.2f%% %6.2f ms\n", pt_strName, pt_tvElapsed.GetSeconds() / tvAppElapsed.GetSeconds() * 100,
      pt_tvElapsed.GetSeconds() / tvModElapsed.GetSeconds() * 100, pt_tvElapsed.GetSeconds() / ctAveragingCount * 1000);
  } else {
    ...
    strBuffer
      += sprintf(strBuffer, "%-45s: %6.2f%% %6.2f%% %6.2f ms (%4.0fc/%s x%d)\n", pt_strName,
                 pt_tvElapsed.GetSeconds() / tvAppElapsed.GetSeconds() * 100,
                 pt_tvElapsed.GetSeconds() / tvModElapsed.GetSeconds() * 100, pt_tvElapsed.GetSeconds() / ctAveragingCount * 1000,
                 pt_tvElapsed.GetSeconds() / ctLocalAveraging * _pTimer->tm_llCPUSpeedHZ, pt_strAveragingName,
...
void CProfileForm::Report(CTString &strReport) {
  ...
  if (pf_ctRunningTimers != 0) {
    strBuffer += sprintf(strBuffer, "WARNING: Some timers are still active - the results are wrong!\n");
  }
  // ASSERT(pf_ctRunningTimers == 0);
  ...
  strBuffer += sprintf(strBuffer, "Module time: %6.2f%% of application time. Average time: %6.2fms\n",
