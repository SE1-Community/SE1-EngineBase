...
int CTStream::ExceptionFilter(DWORD dwCode, _EXCEPTION_POINTERS *pExceptionInfoPtrs) {
  // exit if the exception is not a page fault
...
void CTStream::Throw_t(char *strFormat, ...) {
...
void CTStream::GetLine_t(char *strBuffer, SLONG slBufferSize, char cDelimiter) {
  ...
  FOREVER {
    ...
    // test if maximum buffer length reached
...
void CTStream::PutLine_t(const char *strBuffer) {
  // check parameters
...
void CTStream::PutString_t(const char *strString) {
  // check parameters
...
void CTStream::FPrintF_t(const char *strFormat, ...) {
...
CChunkID CTStream::GetID_t(void) {
  CChunkID cidToReturn;
  Read_t(&cidToReturn.cid_ID[0], CID_LENGTH);
  return (cidToReturn);
}

CChunkID CTStream::PeekID_t(void) {
  // read the chunk id
  CChunkID cidToReturn;
  Read_t(&cidToReturn.cid_ID[0], CID_LENGTH);
...
void CTStream::ExpectID_t(const CChunkID &cidExpected) {
...
void CTStream::ExpectKeyword_t(const CTString &strKeyword) {
...
void CTStream::ReadRawChunk_t(void *pvBuffer, SLONG slSize) {
  Read_t((char *)pvBuffer, slSize);
}

void CTStream::ReadChunk_t(void *pvBuffer, SLONG slExpectedSize) {
...
void CTStream::ReadFullChunk_t(const CChunkID &cidExpected, void *pvBuffer, SLONG slExpectedSize) {
  ExpectID_t(cidExpected);
  ReadChunk_t(pvBuffer, slExpectedSize);
};

void *CTStream::ReadChunkAlloc_t(SLONG slSize) {
  UBYTE *buffer;
  SLONG chunkSize;
...
void CTStream::ReadStream_t(CTStream &strmOther) {
  // implement this !!!! @@@@
}

void CTStream::WriteID_t(const CChunkID &cidSave) {
  Write_t(&cidSave.cid_ID[0], CID_LENGTH);
}

void CTStream::WriteSize_t(SLONG slSize) {
  Write_t((char *)&slSize, sizeof(SLONG));
}

void CTStream::WriteRawChunk_t(void *pvBuffer, SLONG slSize) {
  Write_t((char *)pvBuffer, slSize);
}

void CTStream::WriteChunk_t(void *pvBuffer, SLONG slSize) {
  WriteSize_t(slSize);
  WriteRawChunk_t(pvBuffer, slSize);
}

void CTStream::WriteFullChunk_t(const CChunkID &cidSave, void *pvBuffer, SLONG slSize) {
  WriteID_t(cidSave); // throws char *
  WriteChunk_t(pvBuffer, slSize); // throws char *
}

void CTStream::WriteStream_t(CTStream &strmOther) {
  // implement this !!!! @@@@
}
...
void CTFileStream::Open_t(const CTFileName &fnFileName, CTStream::OpenMode om) {
  // if current thread has not enabled stream handling
  if (!_bThreadCanHandleStreams) {
    // error
    ::ThrowF_t(TRANS("Cannot open file `%s', stream handling is not enabled for this thread"), (CTString &)fnFileName);
  }
  ...
  if (fstrm_pFile == NULL && fstrm_iZipHandle == -1) {
    // throw exception
    Throw_t(TRANS("Cannot open file `%s' (%s)"), (CTString &)fnmFullFileName, strerror(errno));
  }
...
void CTFileStream::Create_t(const CTFileName &fnFileName, enum CTStream::CreateMode cm) {
  (void)cm; // OBSOLETE!
  ...
  if (!_bThreadCanHandleStreams) {
    // error
    ::ThrowF_t(TRANS("Cannot create file `%s', stream handling is not enabled for this thread"), (CTString &)fnFileNameAbsolute);
  }
...
CTMemoryStream::CTMemoryStream(void *pvBuffer, SLONG slSize, CTStream::OpenMode om /*= CTStream::OM_READ*/) {
  // if current thread has not enabled stream handling
  if (!_bThreadCanHandleStreams) {
    // error
    ::FatalError(TRANS("Can create memory stream, stream handling is not enabled for this thread"));
...
// Test if a file exists for writing (this can be different from normal FileExists() if a mod uses BaseWriteExclude.lst)
...
// Get CRC32 of a file
ULONG GetFileCRC32_t(const CTFileName &fnmFile) {
  // open the file
  CTFileStream fstrm;
