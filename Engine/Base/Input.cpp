...
// Procedure called when message is retreived (hook code, message identifier, mouse coordinates)
...
// Procedure called when message is sent  (hook code, message identifier, mouse coordinates)
...
static void Poll2ndMouse(void) {
  ...
  while (iRetries > 0 && !ReadFile(_h2ndMouse, aubMouseBuffer, dwLength, &dwLength, NULL)) {
    iRetries--;
  }

  if (iRetries <= 0) {
    return; // what, didn't make it?
  }

  // parse the mouse packets
  for (INDEX i = 0; i < dwLength; i++) {
    // prepare
    if (aubMouseBuffer[i] & 64) {
      _iByteNum = 0;
    }
...
static void Startup2ndMouse(INDEX iPort) {
  ...
  if (_h2ndMouse == NONE) {
    _h2ndMouse = CreateFileA(str2ndMousePort, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
// Add axis and buttons for given joystick
void CInput::AddJoystickAbbilities(INDEX iJoy) {
  ...
  for (UINT iAxis = 0; iAxis < 6; iAxis++) {
    ...
    switch (iAxis) {
      case 0:
        cai.cai_strAxisName = strJoystickName + " Axis X";
        strTran = strJoystickNameTra + TRANS(" Axis X");
        break;

      case 1:
        cai.cai_strAxisName = strJoystickName + " Axis Y";
        strTran = strJoystickNameTra + TRANS(" Axis Y");
        break;

      case 2:
        cai.cai_strAxisName = strJoystickName + " Axis Z";
        strTran = strJoystickNameTra + TRANS(" Axis Z");
        break;

      case 3:
        cai.cai_strAxisName = strJoystickName + " Axis R";
        strTran = strJoystickNameTra + TRANS(" Axis R");
        break;

      case 4:
        cai.cai_strAxisName = strJoystickName + " Axis U";
        strTran = strJoystickNameTra + TRANS(" Axis U");
        break;

      case 5:
        cai.cai_strAxisName = strJoystickName + " Axis V";
        strTran = strJoystickNameTra + TRANS(" Axis V");
        break;
    }
  }
...
// Initialize all available devices and enumerates available controls
...
void CInput::GetInput(BOOL bPreScan) {
  ...
  if (GetCursorPos(&pntMouse)) {
    ...
    if (inp_bMousePrecision) {
      ...
      if (_tmTime > inp_fMousePrecisionTimeout) {
        fSensitivity /= inp_fMousePrecisionFactor;
      }
    }
  ...
  if (_h2ndMouse != NONE) {
    ...
    if (_i2ndMouseButtons & 2) {
      inp_ubButtonsBuffer[KID_2MOUSE1] = 0xFF;
    }

    if (_i2ndMouseButtons & 1) {
      inp_ubButtonsBuffer[KID_2MOUSE2] = 0xFF;
    }

    if (_i2ndMouseButtons & 4) {
      inp_ubButtonsBuffer[KID_2MOUSE3] = 0xFF;
    }
...
// Scan axis and buttons for given joystick
