...
void CAnimData::CreateAnimations(INDEX ctAnimations, CTString strName /*="None"*/, INDEX iDefaultFrame /*=0*/,
                                 TIME tmSpeed /*=0.02f*/) {
...
void CAnimObject::PauseAnim(void) {
  ...
  ao_ulFlags |= AOF_PAUSED;
  ao_llAnimStart = _pTimer->GetGameTick() - ao_llAnimStart; // set difference from current time as start time,
...
void CAnimObject::ContinueAnim(void) {
  ...
  INDEX iStoppedFrame
    = (pCOA->oa_NumberOfFrames + (SLONG)(CTimer::InSeconds(ao_llAnimStart) / pCOA->oa_SecsPerFrame) % pCOA->oa_NumberOfFrames)
      % pCOA->oa_NumberOfFrames;
