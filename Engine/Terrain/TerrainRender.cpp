...
void PrepareScene(CAnyProjection3D &apr, CDrawPort *pdp, CTerrain *ptrTerrain) {
  ...
  _vViewerAbs
    = (_aprProjection->ViewerPlacementR().pl_PositionVector - pen->en_plPlacement.pl_PositionVector) * !pen->en_mRotation;
...
__forceinline void Lerp(GFXVertex &vResult, const GFXVertex &vOriginal, const GFXVertex &v1, const GFXVertex &v2,
                        const FLOAT &fFactor) {
  ...
  */
  ...
  if (iTopNeigbour >= 0) {
    ...
    if (tt.tt_iLod == ttTop.tt_iLod && fLerpFactor <= tt.tt_fLodLerpFactor) {
      ...
      // Get destination vertex pointer to copy vertices from top neighbour (first vertex in top left corner of this tile - first
      // vertex in array) Get destination vertex pointer to lerp vertices from top neighbour (first vertex added as additional top
      // border vertex)
  ...
  if (iBottomNeigbour >= 0) {
    ...
    if (tt.tt_iLod == ttBottom.tt_iLod && fLerpFactor < tt.tt_fLodLerpFactor) {
      ...
      // Get destination vertex pointer to lerp vertices from bottom neighbour (first vertex added as additional bottom border
      // vertex)
  ...
  if (iLeftNeigbour >= 0) {
    ...
    if (tt.tt_iLod == ttLeft.tt_iLod && fLerpFactor <= tt.tt_fLodLerpFactor) {
      ...
      // Get destination vertex pointer to copy vertices from left neighbour (first vertex in top left corner of this tile - first
      // vertex in array) Get destination vertex pointer to lerp vertices from left neighbour (first vertex added as additional
      // left border vertex)
      const INDEX iDstCopyVtx = 0;
  ...
  if (iRightNeigbour >= 0) {
    ...
    if (tt.tt_iLod == ttRight.tt_iLod && fLerpFactor < tt.tt_fLodLerpFactor) {
      ...
      // Get destination vertex pointer to lerp vertices from right neighbour (first vertex added as additional right border
      // vertex)
...
static void DrawQuadTreeNode(INDEX iqtn) {
  ...
  FLOATobbox3D obbox
    = FLOATobbox3D(qtn.qtn_aabbox, (pen->en_plPlacement.pl_PositionVector - _aprProjection->pr_vViewerPosition) * mAbsToView,
                   mAbsToView * pen->en_mRotation);
