  ...
  for (INDEX itl = 0; itl < ctLayers; itl++) {
    ...
    for (UINT ir = 0; ir < ptdDest->GetPixHeight(); ir++) {
      ...
      for (UINT ic = 0; ic < ptdDest->GetPixWidth(); ic++) {
        ...
        SLONG xStrength
          = (xStrengthX1 << 1)
            + (SLONG)((((xStrengthX2 >> 0) - (xStrengthX1 >> 0)) * xFactV) >> 15); // Lerp(fiStrengthX1,fiStrengthX2,fiFactV);
...
COLOR CTerrain::GetShadeColor(CShadingInfo *psi) {
  ...
  ULONG ulPixel = ((slRed << 24) & 0xFF000000) | ((slGreen << 16) & 0x00FF0000) | ((slBlue << 8) & 0x0000FF00) | 0xFF;
...
FLOATplane3D CTerrain::GetPlaneFromPoint(FLOAT3D &vAbsPoint) {
  ...
  FLOAT3D vx2
    = FLOAT3D((pixX + 0) * tr_vStretch(1), tr_auwHeightMap[iPix + pixWidth] * tr_vStretch(2), (pixZ + 1) * tr_vStretch(3));
  FLOAT3D vx3
    = FLOAT3D((pixX + 1) * tr_vStretch(1), tr_auwHeightMap[iPix + pixWidth + 1] * tr_vStretch(2), (pixZ + 1) * tr_vStretch(3));
...
void CTerrain::BuildTerrainData() {
  ...
  for (INDEX iy = 0; iy < tr_ctTilesY; iy++) {
    ...
    for (INDEX ix = 0; ix < tr_ctTilesX; ix++) {
      ...
      // Set tile neighbours
      if (iy > 0)
        tt.tt_aiNeighbours[NB_TOP] = iTileIndex - tr_ctTilesX;
      if (ix > 0)
        tt.tt_aiNeighbours[NB_LEFT] = iTileIndex - 1;
      if (iy < tr_ctTilesX - 1)
        tt.tt_aiNeighbours[NB_BOTTOM] = iTileIndex + tr_ctTilesX;
      if (ix < tr_ctTilesY - 1)
  ...
  for (INDEX itl=0;itl<ctLayers;itl++) {
    ...
    for (UINT ir=0;ir<ptdDest->GetHeight();ir++) {
      ...
      for (UINT ic=0;ic<ptdDest->GetWidth();ic++) {
        ...
        SLONG xStrengthX1 = (x1 << 7) + (SLONG)(((x2-x1)*xFactH) >> 9); //Lerp(fi1,fi2,fiFactH);
        SLONG xStrengthX2 = (x3 << 7) + (SLONG)(((x4-x3)*xFactH) >> 9); //Lerp(fi3,fi4,fiFactH);
        SLONG xStrength   = (xStrengthX1 << 1) + (SLONG)((((xStrengthX2 >> 0)-(xStrengthX1 >> 0))*xFactV) >> 15);
//Lerp(fiStrengthX1,fiStrengthX2,fiFactV);
...
static void ShowTerrainInfo(CAnyProjection3D &apr, CDrawPort *pdp, CTerrain *ptrTerrain) {
  ...
  strInfo.PrintF("Tris = %d\nNodes = %d\nDelayed nodes = %d\nTop maps = %d\nTexgens = %d, %d\nShadowmap updates = %d\n", _ctTris,
                 _ctNodesVis, _ctDelayedNodes, ctTopMaps, ctGeneratedTopMaps, ctGlobalTopMaps, _ctShadowMapUpdates);
  ...
  SLONG slTotal = slHeightMap + slEdgeMap + slShadowMap + slQTNodes + slQTLevels + slTiles + slArrayHoldes + slLayers + slTopMaps
                  + slGlobalTopMap + slTileBatchingSize + slVertexSmoothing;
  CTString strMemoryUsed;
  strMemoryUsed.PrintF(
    "Heightmap = %d KB\nEdgemap   = %d KB\nShadowMap = %d KB\nQuadTree  = %d KB\nTiles     = %d KB\nArrays    = %d KB\nLayers    "
    "= %d KB\nTopMaps   = %d KB\nGlobal TM = %d KB\nShared TM = %d KB\nVtx lerp  = %d KB\nBatching  = %d KB\nTotal     = %d KB\n",
    slHeightMap / 1024, slEdgeMap / 1024, slShadowMap / 1024, (slQTNodes + slQTLevels) / 1024, slTiles / 1024,
    slArrayHoldes / 1024, slLayers / 1024, slTopMaps / 1024, slGlobalTopMap / 1024, _slSharedTopMapSize / 1024,
    slVertexSmoothing / 1024, slTileBatchingSize / 1024, slTotal / 1024);

  strInfo += strMemoryUsed;

  extern FLOAT3D _vDirection;
  strInfo += CTString(0, "Shadow map size = %d,%d [%d]\nShading map size= %d,%d [%d]\n", ptrTerrain->GetShadowMapWidth(),
                      ptrTerrain->GetShadowMapHeight(), ptrTerrain->tr_iShadowMapSizeAspect, ptrTerrain->GetShadingMapWidth(),
...
void CTerrain::Read_t(CTStream *istrFile) {
  ...
  if (iSavedVersion > 4) {
    ...
    ThrowF_t(TRANS("The terrain version on disk is %d.\n"
             ...
             iSavedVersion, _iTerrainVersion);
  }
