...
static FLOATaabbox3D CalculateAABBoxFromRect(CTerrain *ptrTerrain, Rect rcExtract) {
  ...
  bboxExtract.minvect
    = FLOAT3D(rcExtract.rc_iLeft * ptrTerrain->tr_vStretch(1), fMinY, rcExtract.rc_iTop * ptrTerrain->tr_vStretch(3));
  bboxExtract.maxvect
    = FLOAT3D(rcExtract.rc_iRight * ptrTerrain->tr_vStretch(1), fMaxY, rcExtract.rc_iBottom * ptrTerrain->tr_vStretch(3));
  return bboxExtract;
}
...
//
void ShowSelectionInternal(CTerrain *ptrTerrain, Rect &rcExtract, CTextureData *ptdBrush, GFXColor colSelection, FLOAT fStrenght,
                           SelectionFill sfFill) {
  ASSERT(ptrTerrain != NULL);
...
void ShowSelectionInternal(CTerrain *ptrTerrain) {
  ...
  // Show selection
  ShowSelectionInternal(ptrTerrain, _rcSelectionExtract, _ptdSelectionBrush, _colSelection, _fSelectionStrenght,
                        _sfSelectionFill);
}

void ShowSelection(CTerrain *ptrTerrain, Rect &rcExtract, CTextureData *ptdBrush, COLOR colSelection, FLOAT fStrenght,
                   SelectionFill sfFill /*=SF_POLYGON*/) {
...
void SetBufferForEditing(CTerrain *ptrTerrain, UWORD *puwEditedBuffer, Rect &rcExtract, BufferType btBufferType,
                         INDEX iBufferData /*=-1*/) {
  ASSERT(ptrTerrain != NULL);
  ASSERT(rcExtract.Width() > 0);
  ASSERT(rcExtract.Height() > 0);
