...
void CBrushPolygon::InitializeShadowMap(void) {
  ...
  FOREACHINSTATICARRAY(bpo_abpePolygonEdges, CBrushPolygonEdge, itbpe) {
    ...
    MEX2D vTexture;
    bpo_mdShadow.GetTextureCoordinates(bpo_pbplPlane->bpl_pwplWorking->wpl_mvRelative, v0, vTexture);
...
FLOAT CBrushShadowLayer::GetLightStrength(PIX pixU, PIX pixV, FLOAT fLRRatio, FLOAT fUDRatio) {
  ...
  if (bsl_pubLayer == NULL || (bsl_ulFlags & BSLF_ALLLIGHT) || (pixU < bsl_pixMinU) || (pixV < bsl_pixMinV)
  ...
  PIX pixU0 = pixU - bsl_pixMinU;
  PIX pixU1 = Min(pixU0 + 1, bsl_pixSizeU - 1);
  PIX pixV0 = pixV - bsl_pixMinV;
  PIX pixV1 = Min(pixV0 + 1, bsl_pixSizeV - 1);
  ...
  if (bsl_pubLayer[ulOffsetUL / 8] & (1 << (ulOffsetUL % 8))) {
    fUL = 1.0f;
  };
  if (bsl_pubLayer[ulOffsetUR / 8] & (1 << (ulOffsetUR % 8))) {
    fUR = 1.0f;
  };
  if (bsl_pubLayer[ulOffsetDL / 8] & (1 << (ulOffsetDL % 8))) {
    fDL = 1.0f;
  };
  if (bsl_pubLayer[ulOffsetDR / 8] & (1 << (ulOffsetDR % 8))) {
    fDR = 1.0f;
  };
...
void CBrushShadowMap::ReadLayers_t(CTStream *pstrm) // throw char *
{
    ...
    *pstrm >> ctLayers;
    ...
    for (INDEX iLayer = 0; iLayer < ctLayers; iLayer++) {
      ...
      *pstrm >> pbsl->bsl_pixSizeV;
      ...
      if ((pbsl->bsl_ulFlags & BSLF_CALCULATED) && (pbsl->bsl_pubLayer == NULL) && !(pbsl->bsl_ulFlags & BSLF_ALLLIGHT)
  ...
  if (bUncalculated) {
    ...
    _pwoCurrentLoading->wo_baBrushes.ba_lhUncalculatedShadowMaps.AddTail(bsm_lnInUncalculatedShadowMaps);
  }
}
...
void CBrushShadowMap::FindLightRectangle(CLightSource &ls, class CLightRectangle &lr) {
  ...
  // if the light is point
    ...
    bpoPolygon.bpo_mdShadow.GetTextureCoordinates(bpoPolygon.bpo_pbplPlane->bpl_pwplWorking->wpl_mvRelative, vHotPoint,
                                                  vmexHotPoint);
    ...
    MEX mexFallOff = MEX(sqrt(ls.ls_rFallOff * ls.ls_rFallOff - lr.lr_fLightPlaneDistance * lr.lr_fLightPlaneDistance) * 1024.0f);
...
BOOL CBrushShadowMap::HasDynamicLayers(void) {
  // for each layer
  FOREACHINLIST(CBrushShadowLayer, bsl_lnInShadowMap, bsm_lhLayers, itbsl) { // light source must be valid
...
BOOL CBrushShadowMap::IsShadowFlat(COLOR &colFlat) {
  ...
  if (!shd_bAllowFlats && shd_iForceFlats < 1)
    return FALSE;
  ...
  FOREACHINLIST(CBrushShadowLayer, bsl_lnInShadowMap, bsm_lhLayers, itbsl) {
    ...
    if (ls.ls_ulFlags & LSF_DIRECTIONAL) {
      ...
      if ((bsl.bsl_ulFlags & BSLF_ALLDARK) || !(pbpo->bpo_ulFlags & BPOF_HASDIRECTIONALLIGHT))
        continue;
