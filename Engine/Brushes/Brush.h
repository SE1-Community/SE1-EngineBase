...
class ENGINE_API CBrushShadowLayer {
  public:
    ...
    // Rectangle where the light influences the polygon
    PIX bsl_pixMinU;
    PIX bsl_pixMinV;
    PIX bsl_pixSizeU;
    PIX bsl_pixSizeV;
...
class ENGINE_API CBrushShadowMap : public CShadowMap {
  public:
    CListNode bsm_lnInUncalculatedShadowMaps; // for linking in list of all shadow maps that need calculation
    ...
    // Read and write layers from/to stream
    ...
    INDEX GetShadowLayersCount(void) {
      return bsm_lhLayers.Count();
    };
...
class ENGINE_API CBrushPolygonTexture {
  public:
    ...
    CBrushPolygonTexture &CopyTextureProperties(CBrushPolygonTexture &bptOther, BOOL bCopyMapping) {
      ...
      if (bCopyMapping)
        bpt_mdMapping = bptOther.bpt_mdMapping;
      return *this;
    };
    ...
    // Get amount of memory used by this object
    inline SLONG GetUsedMemory(void) {
      return sizeof(CBrushPolygonTexture);
    };
};
...
#define BPOF_MASK_FOR_COPYING \
  (BPOF_PASSABLE | BPOF_DOESNOTCASTSHADOW | BPOF_FULLBRIGHT | BPOF_TRANSLUCENT | BPOF_TRANSPARENT | BPOF_HASDIRECTIONALLIGHT \
 | BPOF_INVISIBLE | BPOF_NOPLANEDIFFUSION | BPOF_DETAILPOLYGON | BPOF_PORTAL | BPOF_ACCURATESHADOWS | BPOF_OCCLUDER \
 | BPOF_HASDIRECTIONALAMBIENT | BPOF_DYNAMICLIGHTSONLY | BPOF_DOESNOTRECEIVESHADOW | BPOF_NODYNAMICLIGHTS | BPOF_DARKCORNERS)
 ...
 *   if BSCF2_VISIBILITYINCLUDE is off, entity is visible from sector only if
 *   !(EntityVisTweaks&SectorVisFlags&VISM_INCLUDEEXCLUDE)
...
class ENGINE_API CBrush3D : public CBrushBase {
  public:
    ...
    // This is a brush, not a terrain
    INDEX GetBrushType() {
      return CBrushBase::BT_BRUSH3D;
    }
