...
#define FOREACHINLIST_R(baseclass, member, head, iter) \
  for (LISTITER(baseclass, member) iter(head.IterationTail()); !iter->member.IsHeadMarker(); iter.MoveToPrev())
...
void CBrush3D::OptimizeObject3D(CObject3D &ob) {
  ...
  _pfWorldEditingProfile.IncrementCounter(CWorldEditingProfile::PCI_SECTORSOPTIMIZED, ob.ob_aoscSectors.Count());
  ob.Optimize();
 ...
 */
void CBrush3D::PrepareRelativeToAbsoluteProjection(CSimpleProjection3D_DOUBLE &prRelativeToAbsolute) {
