...
class CTriangularizer {
  public:
    ...
    inline void EdgeToPlane(const DOUBLE3D &vVertex0, const DOUBLE3D &vVertex1, DOUBLEplane3D &plPlane) const;
 ...
 * Create a splitter plane for an edge.
 */
inline void CTriangularizer::EdgeToPlane(const DOUBLE3D &vVertex0, const DOUBLE3D &vVertex1, DOUBLEplane3D &plPlane) const {
 ...
 * Clip edge to a plane and check if something remains behind.
 */
BOOL CTriangularizer::ClipEdge(DOUBLE3D &vVertex0, DOUBLE3D &vVertex1, const DOUBLEplane3D &plPlane) const {
...
BOOL CTriangularizer::EdgeInsideTriangle(const CBrushEdge *pbed) const {
  ...
  return (ClipEdge(vVertex0, vVertex1, tr_plLeft) && ClipEdge(vVertex0, vVertex1, tr_plRight)
...
DOUBLE CTriangularizer::TriangleQuality(void) const {
  DOUBLE3D vEdgeBottom = GetVertex(tr_pbedBottom->bed_pbvxVertex1) - GetVertex(tr_pbedBottom->bed_pbvxVertex0);

  DOUBLE3D vEdgeLeft = GetVertex(tr_pbedBottom->bed_pbvxVertex0) - GetVertex(tr_pbvxTopVertex);

  DOUBLE3D vEdgeRight = GetVertex(tr_pbvxTopVertex) - GetVertex(tr_pbedBottom->bed_pbvxVertex1);
...
void CTriangularizer::FindExistingTriangleEdges(void) {
  ...
  FOREACHINDYNAMICARRAY(tr_abedEdges, CBrushEdge, itbed) {
    ...
    if (tr_pbedBottom == itbed) {
      ...
      if (itbed->bed_pbvxVertex1 == tr_pbedBottom->bed_pbvxVertex0 && itbed->bed_pbvxVertex0 == tr_pbvxTopVertex) {
        // remember it as the left edge
        ASSERT(tr_pbedLeft == NULL);
        tr_pbedLeft = itbed;

        // if it is the right edge of the triangle
      } else if (itbed->bed_pbvxVertex0 == tr_pbedBottom->bed_pbvxVertex1 && itbed->bed_pbvxVertex1 == tr_pbvxTopVertex) {
...
void EdgeDir(const DOUBLE3D &vPoint0, const DOUBLE3D &vPoint1, DOUBLE3D &vDirection, DOUBLE3D &vReferencePoint) {
  ...
  /* calculate the reference point on the line from any point on line (p) and direction (d):
    r = p - ((p.d)/(d.d))*d
  */
  vReferencePoint = vPoint0 - vDirection * ((vPoint0 % vDirection)) / (vDirection % vDirection);
}
...
void CTriangularizer::DumpEdges(void) {
  DPrintF("%d\n", tr_abedEdges.Count());
  FOREACHINDYNAMICARRAY(tr_abedEdges, CBrushEdge, itbed) {
    DPrintF("(%f, %f) ->", GetVertex(itbed->bed_pbvxVertex0)(1), GetVertex(itbed->bed_pbvxVertex0)(2));
    DPrintF(" (%f, %f) ", GetVertex(itbed->bed_pbvxVertex1)(1), GetVertex(itbed->bed_pbvxVertex1)(2));

    DOUBLE3D vDirection, vReferencePoint;
    EdgeDir(GetVertex(itbed->bed_pbvxVertex0), GetVertex(itbed->bed_pbvxVertex1), vDirection, vReferencePoint);

    DPrintF("[(%f, %f), ", vDirection(1), vDirection(2));
    DPrintF("(%f, %f)]\n", vReferencePoint(1), vReferencePoint(2));
  }
}
...
void CTriangularizer::FindBestTriangle(void) {
  ...
  for (INDEX ibedBottom = 0; ibedBottom < tr_abedEdges.Count(); ibedBottom++) {
    ...
    EdgeToPlane(GetVertex(tr_pbedBottom->bed_pbvxVertex0), GetVertex(tr_pbedBottom->bed_pbvxVertex1), tr_plEdgeConsidered);
    ...
    for (INDEX ibedTop = 0; ibedTop < ctEdges; ibedTop++) {
      ...
      if (tr_pbvxTopVertex == tr_pbedBottom->bed_pbvxVertex0 || tr_pbvxTopVertex == tr_pbedBottom->bed_pbvxVertex1) {
        // skip this triangle
        continue;
      }
      ...
      EdgeToPlane(GetVertex(tr_pbvxTopVertex), GetVertex(tr_pbedBottom->bed_pbvxVertex0), tr_plLeft);
      // create right splitter
      EdgeToPlane(GetVertex(tr_pbedBottom->bed_pbvxVertex1), GetVertex(tr_pbvxTopVertex), tr_plRight);
  ...
  switch (iMaxNormal) {
    ...
    case 2:
      if (iMaxSign == -1) {
        tr_iAxis0 = 1;
        tr_iAxis1 = 3;
        tr_vPolygonNormal = DOUBLE3D(0, -1, 0);
      } else {
        tr_iAxis0 = 3;
        tr_iAxis1 = 1;
        tr_vPolygonNormal = DOUBLE3D(0, 1, 0);
