...
CBrushSector::CBrushSector(const CBrushSector &c) : bsc_bspBSPTree(*new DOUBLEbsptree3D) {
  ASSERT(FALSE);
};
...
void CBrushSector::CalculateBoundingBoxes(CSimpleProjection3D_DOUBLE &prRelativeToAbsolute) {
  ...
  for (INDEX ivx = 0; ivx < bsc_abvxVertices.Count(); ivx++) {
    ...
    bsc_awvxVertices[ivx].wvx_vRelative = bsc_abvxVertices[ivx].bvx_vRelative
      = DOUBLEtoFLOAT(bsc_abvxVertices[ivx].bvx_vdPreciseRelative);
  ...
  for (INDEX ipl = 0; ipl < bsc_abplPlanes.Count(); ipl++) {
    ...
    bsc_awplPlanes[ipl].wpl_plRelative = bsc_abplPlanes[ipl].bpl_plRelative
      = DOUBLEtoFLOAT(bsc_abplPlanes[ipl].bpl_pldPreciseRelative);
    ...
    GetMajorAxesForPlane(bsc_abplPlanes[ipl].bpl_plAbsolute, bsc_abplPlanes[ipl].bpl_iPlaneMajorAxis1,
  ...
  if (!(bsc_ulTempFlags & BSCTF_PRELOADEDBSP)) {
    ...
    if (pen != NULL && ((pen->en_ulFlags & ENF_ZONING) || pen->en_RenderType == CEntity::RT_FIELDBRUSH)) {
...
void GetSharedEdge(CBrushPolygon *pbpo0, CBrushPolygon *pbpo1, CBrushEdge *&pse) {
  for (INDEX iFirst = 0; iFirst < 3; iFirst++) {
    for (INDEX iSecond = 0; iSecond < 3; iSecond++) {
      if (pbpo0->bpo_abpePolygonEdges[iFirst].bpe_pbedEdge == pbpo1->bpo_abpePolygonEdges[iSecond].bpe_pbedEdge) {
...
BOOL CBrushSector::IsReTripleAvailable(CBrushPolygonSelection &selPolygon) {
  ...
  if (pbpoOld0->bpo_pbscSector != pbpoOld1->bpo_pbscSector)
    return FALSE;

  // both of them must be triangles
  if (pbpoOld0->bpo_aiTriangleElements.Count() != 3)
    return FALSE;
  if (pbpoOld1->bpo_aiTriangleElements.Count() != 3)
    return FALSE;
...
void GetNonSharedEdgesContainingVtx(CBrushPolygon *pbpo0, CBrushPolygon *pbpo1, CBrushEdge *pse, CBrushVertex *pvtx,
