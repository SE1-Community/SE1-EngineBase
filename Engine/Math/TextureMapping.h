...
// Description of texture mapping on a polygon relative to default mapping
// for its plane, version used to communicating information to user
...
class ENGINE_API CMappingDefinition {
  public:
    ...
    // Constructor
    inline CMappingDefinition(void) : md_fUoS(1), md_fUoT(0), md_fVoS(0), md_fVoT(1), md_fUOffset(0), md_fVOffset(0) {};
    ...
    void ProjectMapping(const FLOATplane3D &plOriginal, const CMappingDefinition &mdOriginal, const FLOATplane3D &pl);
    ...
    void Transform(const FLOATplane3D &plPlane, const class CPlacement3D &plSource, const class CPlacement3D &plTarget);
    ...
    void GetSpaceCoordinates(const CMappingVectors &mvDefault, const MEX2D &vTexture, FLOAT3D &vSpace) const;
};
