...
class CObjectSector {
  ...
  public:
    ...
    ENGINE_API CObjectPolygon *CreatePolygon(INDEX ctVertices, INDEX aivVertices[], CObjectMaterial &omaMaterial, ULONG ulFlags,
                                             BOOL bReverse);
    // Create a new polygon in given sector.
    ENGINE_API CObjectPolygon *CreatePolygon(INDEX ctVertices, DOUBLE3D avVertices[], CObjectMaterial &omaMaterial, ULONG ulFlags,
                                             BOOL bReverse);
...
class ENGINE_API CObject3D {
  ...
  public:
    ...
    void LoadAny3DFormat_t(const CTFileName &FileName, const FLOATmatrix3D &mTransform,
                           enum LoadType ltLoadType = LT_NORMAL); // throw (char *)
