...
class CObjectVertex : public DOUBLE3D {
  public:
    ...
    inline CObjectVertex(const DOUBLE3D &v) : DOUBLE3D(v) {
      ovx_ulFlags = 0;
    };
...
class CObjectEdge {
  public:
    ...
    union {
      ...
      // Used in splitting collinear edges
      struct {
...
class CObjectMaterial {
  public:
    ...
    // Color assignment
    inline void SetColor(const COLOR &clrColor) {
      omt_Color = clrColor;
    };
};
...
class ENGINE_API CObjectPolygonEdge {
  public:
    ...
    inline CObjectPolygonEdge(CObjectEdge *poed, BOOL bReverse) : ope_Edge(poed), ope_Backward(bReverse) {};
...
class CObjectSector {
  public:
    ...
    inline CObjectEdge &CreateEdge(INDEX ivx0, INDEX ivx1) {
      ...
      // create edge
      CObjectEdge &oeResult = *osc_aoedEdges.New(1) = CObjectEdge(osc_aovxVertices[ivx0], osc_aovxVertices[ivx1]);
    ...
    inline void CreateEdgeInPolygon(CObjectPolygon &opoPolygon, const DOUBLE3D &vVertex0, const DOUBLE3D &vVertex1) {
      *opoPolygon.opo_PolygonEdges.New(1) = CObjectPolygonEdge(&CreateEdge(vVertex0, vVertex1));
    };
    ...
    void CreateEdgeLines(CStaticArray<CEdgeEx> &aedxEdgeLines, CStaticArray<CEdgeEx *> &apedxSortedEdgeLines, INDEX &ctEdges);
    ...
    void SplitCollinearEdgesRun(CStaticArray<CEdgeEx *> &apedxSortedEdgeLines, INDEX iFirstInRun, INDEX iLastInRun);

    // Split an edge with a run of vertices
    inline void SplitEdgeWithVertices(CObjectEdge &oedOriginal, CStaticArray<CObjectVertex *> &apvxVertices);
  ...
  public:
    ...
    ENGINE_API CObjectPolygon *CreatePolygon(INDEX ctVertices, INDEX aivVertices[], CObjectMaterial &omaMaterial,
                                             ULONG ulFlags, BOOL bReverse);

    // Create a new polygon in given sector
    ENGINE_API CObjectPolygon *CreatePolygon(INDEX ctVertices, DOUBLE3D avVertices[], CObjectMaterial &omaMaterial,
                                             ULONG ulFlags, BOOL bReverse);
...
class ENGINE_API CObject3D {
  ...
  public:
    ...
    void LoadAny3DFormat_t(const CTFileName &FileName, const FLOATmatrix3D &mTransform,
                           enum LoadType ltLoadType = LT_NORMAL); // throws char *
    ...
    // [Cecil] TODO: No definitions; remove or recreate the functions
    // Save in LightWave format
    void SaveLWO_t(const CTFileName &FileName); // throws char *
    // Save in 3DStudio format
    void SaveDXF_t(const CTFileName &FileName); // throws char *
    ...
    // Add material from object B to object A. (B should have only one open sector and no closed sectors)
    void CSGAddMaterial(CObject3D &obA, CObject3D &obB);
    void CSGAddMaterialReverse(CObject3D &obA, CObject3D &obB);

    // Remove material of object B from object A (B should have only one open sector and no closed sectors)
    void CSGRemoveMaterial(CObject3D &obA, CObject3D &obB);

    // Split sectors of object A using object B (B should have only one closed sector and no open sectors)
    void CSGSplitSectors(CObject3D &obA, CObject3D &obB);

    // Join sectors of object A with sectors of object B (both A and B should have only one sector)
    void CSGJoinSectors(CObject3D &obA, CObject3D &obB);

    // Split polygons of object A with sectors of object B (both A and B should have only one sector)
