...
void CParallelProjection3D::Prepare(void) {
  ...
  pr_vViewDirection = FLOAT3D(pr_vStepFactors(1) / pr_vZoomFactors(1), pr_vStepFactors(2) / pr_vZoomFactors(2), -1.0f);
 ...
 */
void CParallelProjection3D::PreClip(const FLOAT3D &v3dObjectPoint, FLOAT3D &v3dTransformedPoint) const {
 ...
 */
void CParallelProjection3D::PostClip(const FLOAT3D &v3dTransformedPoint, FLOAT3D &v3dViewPoint) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
  // multiply X and Y coordinates with zoom factor and add the center of screen
  v3dViewPoint(1)
    = pr_ScreenCenter(1) + v3dTransformedPoint(1) * pr_vZoomFactors(1) + v3dTransformedPoint(3) * pr_vStepFactors(1);
  v3dViewPoint(2)
    = pr_ScreenCenter(2) - v3dTransformedPoint(2) * pr_vZoomFactors(2) - v3dTransformedPoint(3) * pr_vStepFactors(2);
}

void CParallelProjection3D::PostClip(const FLOAT3D &v3dTransformedPoint, FLOAT fTransformedR, FLOAT3D &v3dViewPoint,
                                     FLOAT &fViewR) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
  // multiply X and Y coordinates with zoom factor and add the center of screen
 ...
 */
void CParallelProjection3D::ProjectCoordinate(const FLOAT3D &v3dObjectPoint, FLOAT3D &v3dViewPoint) const {
  // rotate and translate the point
 ...
 */
void CParallelProjection3D::ProjectDirection(const FLOAT3D &v3dObjectPoint, FLOAT3D &v3dViewPoint) const {
 ...
 */
void CParallelProjection3D::ProjectAABBox(const FLOATaabbox3D &boxObject, FLOATaabbox3D &boxView) const {
 ...
 */
void CParallelProjection3D::Project(const FLOATplane3D &p3dObjectPlane, FLOATplane3D &p3dTransformedPlane) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
 ...
 */
void CParallelProjection3D::RayThroughPoint(const FLOAT3D &v3dViewPoint, CPlacement3D &plRay) const {
