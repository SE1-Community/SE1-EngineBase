...
static inline void GetParallelComponent(const FLOAT3D &vFull, const FLOAT3D &vReference, FLOAT3D &vParallel) {
  vParallel = vReference * (vFull % vReference);
}

// Get component of a vector normal to given reference vector
static inline void GetNormalComponent(const FLOAT3D &vFull, const FLOAT3D &vReference, FLOAT3D &vNormal) {
  vNormal = vFull - vReference * (vFull % vReference);
}

// Get components of a vector parallel and normal to given reference vector
static inline void GetParallelAndNormalComponents(const FLOAT3D &vFull, const FLOAT3D &vReference, FLOAT3D &vParallel, FLOAT3D &vNormal) {
  vParallel = vReference * (vFull % vReference);
...
static inline FLOAT RotationMatrixValidity(const FLOATmatrix3D &m) {
  ...
  return Sqrt(vH(1) * vH(1) + vH(2) * vH(2) + vH(3) * vH(3) + vV(1) * vV(1) + vV(2) * vV(2) + vV(3) * vV(3));
}
...
inline void GetMajorAxesForPlane(const FLOATplane3D &plPlane, INDEX &iMajorAxis1, INDEX &iMajorAxis2) {
  ...
  ASSERT(Abs(plPlane(iMaxNormalAxis)) >= Abs(plPlane(iMajorAxis1)) && Abs(plPlane(iMaxNormalAxis)) >= Abs(plPlane(iMajorAxis2)));
}
