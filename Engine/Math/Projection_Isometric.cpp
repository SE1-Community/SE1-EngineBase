...
void CIsometricProjection3D::Prepare(void) {
  ...
  if (pr_bFaceForward) {
    ...
    if (pr_bHalfFaceForward) {
      ...
      t3dObjectRotation(1, 3) = vZ(1);
      t3dObjectRotation(2, 1) = vX(2);
      t3dObjectRotation(2, 2) = vY(2);
      t3dObjectRotation(2, 3) = vZ(2);
      t3dObjectRotation(3, 1) = vX(3);
      t3dObjectRotation(3, 2) = vY(3);
      t3dObjectRotation(3, 3) = vZ(3);
 ...
 */
void CIsometricProjection3D::PreClip(const FLOAT3D &v3dObjectPoint, FLOAT3D &v3dTransformedPoint) const {
 ...
 */
void CIsometricProjection3D::PostClip(const FLOAT3D &v3dTransformedPoint, FLOAT3D &v3dViewPoint) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
...
void CIsometricProjection3D::PostClip(const FLOAT3D &v3dTransformedPoint, FLOAT fTransformedR, FLOAT3D &v3dViewPoint,
                                      FLOAT &fViewR) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
  // multiply X and Y coordinates with zoom factor and add the center of screen
 ...
 */
void CIsometricProjection3D::ProjectCoordinate(const FLOAT3D &v3dObjectPoint, FLOAT3D &v3dViewPoint) const {
  // rotate and translate the point
 ...
 */
void CIsometricProjection3D::ProjectDirection(const FLOAT3D &v3dObjectPoint, FLOAT3D &v3dViewPoint) const {
 ...
 */
void CIsometricProjection3D::ProjectAABBox(const FLOATaabbox3D &boxObject, FLOATaabbox3D &boxView) const {
 ...
 */
void CIsometricProjection3D::Project(const FLOATplane3D &p3dObjectPlane, FLOATplane3D &p3dTransformedPlane) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
 ...
 */
void CIsometricProjection3D::RayThroughPoint(const FLOAT3D &v3dViewPoint, CPlacement3D &plRay) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
