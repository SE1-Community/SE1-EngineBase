...
template<class Type, int iDimensions> class AABBox {
  ...
  public:
    ...
    // Check if the bounding box is contained in another bounding box
    inline BOOL operator<=(const AABBox<Type, iDimensions> &b) const;
    ...
    inline BOOL TouchesSphere(const Vector<Type, iDimensions> &vSphereCenter, Type fSphereRadius) const;
...
// Clear to normalized empty bounding box
template<class Type, int iDimensions> inline void AABBox<Type, iDimensions>::SetToNormalizedEmpty(void) {
...
template<class Type, int iDimensions> inline AABBox<Type, iDimensions>::AABBox() {
  SetToNormalizedEmpty();
}

// Constructor for one-point bounding box
template<class Type, int iDimensions> inline AABBox<Type, iDimensions>::AABBox(const Vector<Type, iDimensions> &vPoint) {
  for (int i = 1; i <= iDimensions; i++) {
...
// Check if empty
template<class Type, int iDimensions> inline BOOL AABBox<Type, iDimensions>::IsEmpty(void) const {
...
// Get center vector (middle of box)
template<class Type, int iDimensions> inline const Vector<Type, iDimensions> AABBox<Type, iDimensions>::Center(void) const {
  // center is in the middle between min and max
  return (maxvect + minvect) / (Type)2;
}

// Get diagonal vector (size of box)
template<class Type, int iDimensions> inline const Vector<Type, iDimensions> AABBox<Type, iDimensions>::Size(void) const {
  // size is difference between min and max vectors
  return maxvect - minvect;
}

// Get minimal vector (lower left of box)
template<class Type, int iDimensions> inline const Vector<Type, iDimensions> &AABBox<Type, iDimensions>::Min(void) const {
  return minvect;
}

// Get maximal vector (upper right of box)
template<class Type, int iDimensions> inline const Vector<Type, iDimensions> &AABBox<Type, iDimensions>::Max(void) const {
  return maxvect;
}
...
inline BOOL AABBox<Type, iDimensions>::HasContactWith(const AABBox<Type, iDimensions> &b, Type tEpsilon) const {
  ...
  for (int i = 1; i <= iDimensions; i++) {
    // if spans in that dimension don't have contact
    if ((maxvect(i) + tEpsilon < b.minvect(i)) || (minvect(i) - tEpsilon > b.maxvect(i))) {
      // whole bounding boxes don't have contact
      return FALSE;
    }
  }
...
// Check if intersects or touches a sphere
template<class Type, int iDimensions>
inline BOOL AABBox<Type, iDimensions>::TouchesSphere(const Vector<Type, iDimensions> &vSphereCenter, Type fSphereRadius) const {
  // for all dimensions
  for (int i = 1; i <= iDimensions; i++) {
    // if spans in that dimension don't have contact
    if ((vSphereCenter(i) + fSphereRadius < minvect(i)) || (vSphereCenter(i) - fSphereRadius > maxvect(i))) {
      // no contact
      return FALSE;
    }
  }
...
// Expand the bounding box by given size
template<class Type, int iDimensions> inline void AABBox<Type, iDimensions>::Expand(Type tEpsilon) {
  // for all dimensions
  for (int i = 1; i <= iDimensions; i++) {
...
// Expand the bounding box by given factor of its size along each axis
template<class Type, int iDimensions> inline void AABBox<Type, iDimensions>::ExpandByFactor(Type tFactor) {
  // for all dimensions
  for (int i = 1; i <= iDimensions; i++) {
    // expand in that dimension
...
// Stretch the bounding box by a given sizing factor
template<class Type, int iDimensions> inline void AABBox<Type, iDimensions>::StretchByFactor(Type tSizing) {
  tSizing = Abs(tSizing);
  // for each dimension
  for (int i = 1; i <= iDimensions; i++) {
...
// Stretch the bounding box by a given sizing vector
template<class Type, int iDimensions> inline void AABBox<Type, iDimensions>::StretchByVector(Vector<Type, iDimensions> vSizing) {
  // for each dimension
  for (int i = 1; i <= iDimensions; i++) {
    // stretch in that dimension
