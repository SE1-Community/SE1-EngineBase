...
template<class Type> inline Type Lerp(const Type x0, const Type x1, const FLOAT fRatio) {
  if (fRatio == 0) {
    return x0;
  } else if (fRatio == 1) {
    return x1;
  } else {
    return x0 + (x1 - x0) * fRatio;
  }
}

template<class Type> inline Type Sgn(const Type x) {
  return (x) > Type(0) ? Type(1) : (x < 0 ? Type(-1) : Type(0));
}

template<class Type> inline Type SgnNZ(const Type x) {
...
template<class Type> inline Type ClampUp(const Type x, const Type uplimit) {
  return (x <= uplimit ? x : uplimit);
}

template<class Type> inline Type ClampDn(const Type x, const Type dnlimit) {
  return (x >= dnlimit ? x : dnlimit);
}

template<class Type> inline Type Clamp(const Type x, const Type dnlimit, const Type uplimit) {
  return (x >= dnlimit ? (x <= uplimit ? x : uplimit) : dnlimit);
}

// Fast implementations
inline DOUBLE Abs(const DOUBLE f) {
  return fabs(f);
}
...
inline SLONG Abs(const SLONG sl) {
  return labs(sl);
}
...
// Log base 2 of any float numner
...
inline SLONG FastLog2(SLONG x) {
  ...
  __asm__ __volatile__("bsrl  (%%ebx), %%eax     \n\t"
                       "movl   %%eax, (%%esi)    \n\t"
                       :
                       : "b"(&x), "S"(&slRet)
                       : "memory");
...
inline FLOAT Sqrt(FLOAT x) {
  return (FLOAT)sqrt(ClampDn(x, 0.0f));
}
...
inline ENGINE_API FLOAT SinFast(ANGLE a) {
  return (FLOAT)sin(a * (PI / ANGLE_180));
};

inline ENGINE_API FLOAT CosFast(ANGLE a) {
  return (FLOAT)cos(a * (PI / ANGLE_180));
};

inline ENGINE_API FLOAT TanFast(ANGLE a) {
  return (FLOAT)tan(a * (PI / ANGLE_180));
};
...
// (i.e. 0.2f means 20% fade in, 0.1f stands for 10% fade out)
