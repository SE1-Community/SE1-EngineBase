...
void CSimpleProjection3D::PostClip(const FLOAT3D &v3dTransformedPoint, FLOAT fTransformedR, FLOAT3D &v3dViewPoint, FLOAT &fViewR) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
  v3dViewPoint = v3dTransformedPoint;
...
FLOAT CSimpleProjection3D::GetDistance(const FLOAT3D &v3dObjectPoint) const {
  ...
  return v3dObjectPoint(1) * pr_RotationMatrix(3, 1)
...
// CSimpleProjection3D : Calculate plane gradient for a plane in 3D view space
