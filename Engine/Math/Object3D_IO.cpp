...
void CObject3D::BatchLoading_t(BOOL bOn) {
  ...
  if (bOn) {
    ...
    if (_hwnd == NULL) {
      ...
      _hwnd
        = CreateWindow(EXPLORATION_WINDOW, "Object Loader", 0, 100, 100, 100, 50, NULL, 0, (HINSTANCE)GetModuleHandle(NULL), 0);
...
void FillConversionArrays_t(const FLOATmatrix3D &mTransform) {
  ...
  FLOAT fDet = m(1, 1) * (m(2, 2) * m(3, 3) - m(2, 3) * m(3, 2)) + m(1, 2) * (m(2, 3) * m(3, 1) - m(2, 1) * m(3, 3))
