 ...
 */
void CSimpleProjection3D_DOUBLE::ProjectCoordinate(const DOUBLE3D &v3dObjectPoint, DOUBLE3D &v3dViewPoint) const {
 ...
 */
void CSimpleProjection3D_DOUBLE::ProjectDirection(const DOUBLE3D &v3dObjectPoint, DOUBLE3D &v3dViewPoint) const {
 ...
 */
void CSimpleProjection3D_DOUBLE::ProjectPlacement(const CPlacement3D &plObject, CPlacement3D &plView) const {
  // rotate and translate the position vector
  plView.pl_PositionVector = DOUBLEtoFLOAT(FLOATtoDOUBLE(plObject.pl_PositionVector) * pr_RotationMatrix + pr_TranslationVector);
 ...
 */
void CSimpleProjection3D_DOUBLE::ProjectMapping(const CMappingDefinition &mdObject, const DOUBLEplane3D &plObject,
 ...
 */
void CSimpleProjection3D_DOUBLE::ProjectAABBox(const DOUBLEaabbox3D &boxObject, DOUBLEaabbox3D &boxView) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
 ...
 */
void CSimpleProjection3D_DOUBLE::Project(const DOUBLEplane3D &p3dObjectPlane, DOUBLEplane3D &p3dTransformedPlane) const {
  // check that the projection object is prepared for projecting
  ASSERT(pr_Prepared);
  ...
  if (pr_ObjectStretch(1) != 1.0f || pr_ObjectStretch(2) != 1.0f || pr_ObjectStretch(3) != 1.0f) {
    ...
    p3dTransformedPlane = DOUBLEplane3D(((DOUBLE3D &)p3dTransformedPlane) / fLen, p3dTransformedPlane.Distance() * fLen);
  }
  // translate the plane
