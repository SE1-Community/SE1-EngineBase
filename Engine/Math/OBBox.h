...
template<class Type> class OBBox {
  ...
  public:
    ...
    inline OBBox(const Vector<Type, 3> &vO, const Vector<Type, 3> &vAxis0, const Vector<Type, 3> &vAxis1,
                 const Vector<Type, 3> &vAxis2, Type tSize0, Type tSize1, Type tSize2);

    // Constructor from axis aligned box and placement
    inline OBBox(const AABBox<Type, 3> &aabbox, const Vector<Type, 3> &vPos, const Matrix<Type, 3, 3> &mRot);
...
// Clear to normalized empty bounding box
template<class Type> inline void OBBox<Type>::SetToNormalizedEmpty(void) {
  for (int i = 0; i < 3; i++) {
...
template<class Type> inline OBBox<Type>::OBBox<Type>() {
  SetToNormalizedEmpty();
}

// Constructor from axis aligned box and placement
template<class Type>
inline OBBox<Type>::OBBox(const AABBox<Type, 3> &aabbox, const Vector<Type, 3> &vPos, const Matrix<Type, 3, 3> &mRot) {
  // translate and rotate the center
  box_vO = aabbox.Center() * mRot + vPos;

  // extracted orientation from the rotation matrix
  box_avAxis[0](1) = mRot(1, 1);
  box_avAxis[0](2) = mRot(2, 1);
  box_avAxis[0](3) = mRot(3, 1);
  box_avAxis[1](1) = mRot(1, 2);
  box_avAxis[1](2) = mRot(2, 2);
  box_avAxis[1](3) = mRot(3, 2);
  box_avAxis[2](1) = mRot(1, 3);
  box_avAxis[2](2) = mRot(2, 3);
  box_avAxis[2](3) = mRot(3, 3);
...
// Constructor from axis aligned box without placement
template<class Type> inline OBBox<Type>::OBBox(const AABBox<Type, 3> &aabbox) {
...
// Constructor from components
template<class Type> inline OBBox<Type>::OBBox(const Vector<Type, 3> &vO, const Vector<Type, 3> &vAxis0,
  const Vector<Type, 3> &vAxis1, const Vector<Type, 3> &vAxis2, Type tSize0, Type tSize1, Type tSize2) {
  box_vO = vO;
  box_avAxis[0] = vAxis0;
  box_avAxis[1] = vAxis1;
  box_avAxis[2] = vAxis2;
...
// Check if empty
template<class Type> inline BOOL OBBox<Type>::IsEmpty(void) const {
...
template<class Type> inline Type OBBox<Type>::TestAgainstPlane(const Plane<Type, 3> &pl) const {
...
// Check if two boxes intersect/touch using the separating axes theorem
template<class Type> inline BOOL OBBox<Type>::HasContactWith(const OBBox<Type> &boxB) const {
  ...
  Type vOffA[3] = {vOffAbs % boxA.box_avAxis[0], vOffAbs % boxA.box_avAxis[1], vOffAbs % boxA.box_avAxis[2]};
...
inline DOUBLEobbox3D FLOATtoDOUBLE(const FLOATobbox3D &boxf) {
  return DOUBLEobbox3D(FLOATtoDOUBLE(boxf.box_vO), FLOATtoDOUBLE(boxf.box_avAxis[0]), FLOATtoDOUBLE(boxf.box_avAxis[1]),
                       FLOATtoDOUBLE(boxf.box_avAxis[2]), FLOATtoDOUBLE(boxf.box_atSize[0]), FLOATtoDOUBLE(boxf.box_atSize[1]),
                       FLOATtoDOUBLE(boxf.box_atSize[2]));
}
inline FLOATobbox3D DOUBLEtoFLOAT(const DOUBLEobbox3D &boxd) {
  return FLOATobbox3D(DOUBLEtoFLOAT(boxd.box_vO), DOUBLEtoFLOAT(boxd.box_avAxis[0]), DOUBLEtoFLOAT(boxd.box_avAxis[1]),
                      DOUBLEtoFLOAT(boxd.box_avAxis[2]), DOUBLEtoFLOAT(boxd.box_atSize[0]), DOUBLEtoFLOAT(boxd.box_atSize[1]),
