...
ENGINE_API extern void AnglesToDirectionVector(const ANGLE3D &a3dAngles, FLOAT3D &vDirection);

// Create angles in 3D from direction vector (ignoring banking; direction must be normalized!)
ENGINE_API extern void DirectionVectorToAngles(const FLOAT3D &vDirection, ANGLE3D &a3dAngles);
ENGINE_API extern void DirectionVectorToAnglesNoSnap(const FLOAT3D &vDirection, ANGLE3D &a3dAngles);

// Create angles in 3D from up vector (ignoring objects relative heading; up vector must be normalized!)
ENGINE_API extern void UpVectorToAngles(const FLOAT3D &vUp, ANGLE3D &a3dAngles);
