  ...
  *pstrm << ls_lhLayers.Count();
  ...
  FOREACHINLIST(CBrushShadowLayer, bsl_lnInLightSource, ls_lhLayers, itbsl) {
    ...
    *pstrm << pbsc->bsc_abpoPolygons.Index(pbpo);
    ...
    FOREACHINLIST(CBrushShadowLayer, bsl_lnInShadowMap, itbsl->bsl_pbsmShadowMap->bsm_lhLayers, itbsl2) {
...
void CLightSource::FindShadowLayersPoint(BOOL bSelectedOnly) {
  ...
  if (ls_ulFlags & LSF_NONPERSISTENT) {
    ...
    if (((ULONG)shd_iAllowDynamic) > 2)
      shd_iAllowDynamic = 1L; // clamp fast
  ...
  if (ls_penEntity->en_ulPhysicsFlags & EPF_MOVABLE) {
    ...
    for (INDEX iPolygon = 0; iPolygon < apbpo.Count(); iPolygon++) {
      ...
      // if the polygon is not marked but it is influenced
      if (!(pbpo->bpo_ulFlags & BPOF_MARKEDLAYER) && IsPolygonInfluencedByPointLight(pbpo)) {
        // add a layer to the polygon
        AddLayer(*pbpo);
      }
    }
...
void CLightSource::SetLightSource(const CLightSource &lsOriginal) {
  ...
  BOOL bDiscardLayers = ls_rFallOff != lsOriginal.ls_rFallOff || ls_ubPolygonalMask != lsOriginal.ls_ubPolygonalMask
                        || ls_ulFlags != lsOriginal.ls_ulFlags || ls_fNearClipDistance != lsOriginal.ls_fNearClipDistance
                        || ls_fFarClipDistance != lsOriginal.ls_fFarClipDistance;
  // test if shadows should be uncached
  BOOL bUncacheShadows = bDiscardLayers || ls_rHotSpot != lsOriginal.ls_rHotSpot || ls_colColor != lsOriginal.ls_colColor
...
COLOR CLightSource::GetLightAmbient(void) const {
  if (ls_paoAmbientLightAnimation == NULL)
    return ls_colAmbient;
