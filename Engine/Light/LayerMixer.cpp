...
static inline void IncrementColorWithClip(UBYTE &ubR, UBYTE &ubG, UBYTE &ubB, SLONG slR, SLONG slG, SLONG slB) {
...
void CLayerMixer::CalculateData(CBrushShadowMap *pbsm, INDEX iMipmap) {
  ...
  PIX pixOffset = pbsm->sm_slMemoryUsed / BYTES_PER_TEXEL - GetMipmapOffset(15, lm_pixCanvasSizeU, lm_pixCanvasSizeV);
  ...
  for (PIX pixV = 0; pixV < _iRowCt; pixV++) {
    ...
    for (PIX pixU = 0; pixU < _iPixCt; pixU++) {
      ...
      if (*pubPoint & ubMask && (slL2Point < FTOX)) {
        ...
        slL = aubSqrt[slL];
        if (slL > _slHotSpot)
          slIntensity = ((255 - slL) * _slLightStep) >> 8;
        // add the intensity to the pixel
...
void CLayerMixer::AddDiffusionPoint(void) {
  ...
  if (_slLightStep != 0)
    slMax1oL = (256 << 8) / _slLightStep + 256;
  ...
  for (PIX pixV = 0; pixV < _iRowCt; pixV++) {
    ...
    for (PIX pixU = 0; pixU < _iPixCt; pixU++) {
      ...
      if (*pubMask & ubMask && (slL2Point < FTOX)) {
        ...
        SLONG slIntensity = _slLightMax;
        if (sl1oL < slMax1oL)
          slIntensity = ((sl1oL - 256) * _slLightStep) >> 16;
        // add the intensity to the pixel
        AddToCluster((UBYTE *)_pulLayer, slIntensity / 255.0f);
      }
      // advance to next pixel
...
BOOL CLayerMixer::PrepareOneLayerPoint(CBrushShadowLayer *pbsl, BOOL bNoMask) {
  ...
  if ((pixMinU + _iPixCt) > lm_pixPolygonSizeU && bNoMask)
    _iPixCt = lm_pixPolygonSizeU - pixMinU;
  if ((pixMinV + _iRowCt) > lm_pixPolygonSizeV)
    _iRowCt = lm_pixPolygonSizeV - pixMinV;
  ...
  if ((pbsl->bsl_pixSizeU >> lm_iMipShift) == 0 || (pbsl->bsl_pixSizeV >> lm_iMipShift) == 0 || _iPixCt <= 0 || _iRowCt <= 0)
    return FALSE;
  ...
  if (_ulLightFlags & LSF_SUBSTRACTSECTORAMBIENT) {
    ...
    if (_ulPolyFlags & BPOF_HASDIRECTIONALAMBIENT) { // find directional layers for each shadow layer
      FOREACHINLIST(CBrushShadowLayer, bsl_lnInShadowMap, lm_pbsmShadowMap->bsm_lhLayers, itbsl) { // loop thru layers
        CBrushShadowLayer &bsl = *itbsl;
        if (bsl.bsl_plsLightSource->ls_ulFlags & LSF_DIRECTIONAL) { // skip if no ambient color
          colAmbient = bsl.bsl_plsLightSource->ls_colAmbient & 0xFFFFFF00;
          if (IsBlack(colAmbient))
            continue;
          // substract ambient
          IncrementByteWithClip(((UBYTE *)&lm_colLight)[1], -((UBYTE *)&colAmbient)[1]);
          IncrementByteWithClip(((UBYTE *)&lm_colLight)[2], -((UBYTE *)&colAmbient)[2]);
...
rowDone:
  ...
  for (INDEX j = 0; j < lm_pixPolygonSizeV; j++) { // prepare row
    FLOAT fGrCol = fGrRow;
    SWORD fixRcol = fixRrow;
    SWORD fixGcol = fixGrow;
    SWORD fixBcol = fixBrow;
    for (INDEX i = 0; i < lm_pixPolygonSizeU; i++) { // loop pixels
...
void CLayerMixer::AddOneLayerDirectional(CBrushShadowLayer *pbsl, UBYTE *pubMask, UBYTE ubMask) {
  ...
  if (!(lm_pbpoPolygon->bpo_ulFlags & BPOF_HASDIRECTIONALLIGHT))
    return;
  ...
  // clamp influence to polygon size
  if (_iPixCt > lm_pixPolygonSizeU && pubMask == NULL)
    _iPixCt = lm_pixPolygonSizeU;
  if (_iRowCt > lm_pixPolygonSizeV)
    _iRowCt = lm_pixPolygonSizeV;
  _slModulo = (lm_pixCanvasSizeU - _iPixCt) * BYTES_PER_TEXEL;
  _pulLayer = lm_pulShadowMap;
  ...
  AnglesToDirectionVector(lm_plsLight->ls_penEntity->GetPlacement().pl_OrientationAngle, lm_vLightDirection);
  // calculate intensity
...
void CLayerMixer::MixOneMipmap(CBrushShadowMap *pbsm, INDEX iMipmap) {
  ...
  if (ulGradientType > 0) {
    ...
    if (pen != NULL)
      bHasGradient = pen->GetGradient(ulGradientType, gpGradient);
  }
  // add gradient if gradient is light
  if (bHasGradient && !gpGradient.gp_bDark)
    AddOneLayerGradient(gpGradient);
  ...
  if (shd_iFiltering > 0) {
    FilterBitmap(shd_iFiltering, lm_pulShadowMap, lm_pulShadowMap, lm_pixPolygonSizeU, lm_pixPolygonSizeV, lm_pixCanvasSizeU,
                 lm_pixCanvasSizeV);
  }
  // do eventual dithering of shadow layer
  const INDEX iDither = GetDither();
  if (!(_pGfx->gl_ulFlags & GLF_32BITTEXTURES))
    shd_bFineQuality = FALSE;
  if (iDither && !(shd_bFineQuality)) {
    DitherBitmap(iDither, lm_pulShadowMap, lm_pulShadowMap, lm_pixPolygonSizeU, lm_pixPolygonSizeV, lm_pixCanvasSizeU,
                 lm_pixCanvasSizeV);
  }
}
...
CLayerMixer::CLayerMixer(CBrushShadowMap *pbsm, INDEX iFirstMip, INDEX iLastMip, BOOL bDynamic) {
  ...
  if (bDynamic) {
    ...
    // need to mix in
    for (INDEX iMipmap = iFirstMip; iMipmap <= iLastMip; iMipmap++)
      MixOneMipmapDynamic(pbsm, iMipmap);
  }
  // mix static layers
  else {
    for (INDEX iMipmap = iFirstMip; iMipmap <= iLastMip; iMipmap++)
      MixOneMipmap(pbsm, iMipmap);
  }
}
