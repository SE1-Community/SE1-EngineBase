...
static void ConvertBytesToBits(UBYTE *pubBytesSrc, UBYTE *pubBitsDst, INDEX ctBytes) {
  ...
  for (INDEX i = 0; i < (ctBytes + 7) / 8; i++) {
    ...
    *pubBitsDst++ = (pubBytesSrc[0] & 0x01) | (pubBytesSrc[1] & 0x02) | (pubBytesSrc[2] & 0x04) | (pubBytesSrc[3] & 0x08)
                    | (pubBytesSrc[4] & 0x10) | (pubBytesSrc[5] & 0x20) | (pubBytesSrc[6] & 0x40) | (pubBytesSrc[7] & 0x80);
    pubBytesSrc += 8;
  }
}
...
void CLayerMaker::CalculateData(void) {
  ...
  lm_pbpoPolygon->bpo_mdShadow.GetSpaceCoordinates(lm_pbpoPolygon->bpo_pbplPlane->bpl_pwplWorking->wpl_mvRelative, vmex0, lm_vO);
  ...
  lm_pbpoPolygon->bpo_mdShadow.GetSpaceCoordinates(lm_pbpoPolygon->bpo_pbplPlane->bpl_pwplWorking->wpl_mvRelative, vmexU,
                                                   lm_vStepU);
  lm_vStepU = lm_vStepU * mPolygonRotation + vPolygonTranslation;
  lm_pbpoPolygon->bpo_mdShadow.GetSpaceCoordinates(lm_pbpoPolygon->bpo_pbplPlane->bpl_pwplWorking->wpl_mvRelative, vmexV,
                                                   lm_vStepV);
  ...
  mInvOrientation(1, 1) = -vX(1);
  mInvOrientation(1, 2) = vY(1);
  mInvOrientation(1, 3) = -vZ(1);
  mInvOrientation(2, 1) = -vX(2);
  mInvOrientation(2, 2) = vY(2);
  mInvOrientation(2, 3) = -vZ(2);
  mInvOrientation(3, 1) = -vX(3);
  mInvOrientation(3, 2) = vY(3);
  mInvOrientation(3, 3) = -vZ(3);
...
inline BOOL TestPointInPolygon(CBrushPolygon &bpo, const FLOAT3D &v) {
  ...
  if (v(iMajorAxis1) < bpo.bpo_boxBoundingBox.Min()(iMajorAxis1) || v(iMajorAxis1) > bpo.bpo_boxBoundingBox.Max()(iMajorAxis1)
...
#define ADDNEIGHBOUR(du, dv) \
  if ((pixLayerU + (du) >= 0) && (pixLayerU + (du) < pixLayerSizeU) && (pixLayerV + (dv) >= 0) \
      && (pixLayerV + (dv) < pixLayerSizeV) && (pubPolygonMask[slOffsetMap + (du) + ((dv) << pixSizeULog2)])) { \
    ctInPolygon++; \
...
void CLayerMaker::MakePolygonMask(void) {
  ...
  if (lm_pbsmShadowMap->bsm_pubPolygonMask != NULL) {
    ...
    ConvertBitsToBytes(lm_pbsmShadowMap->bsm_pubPolygonMask, lm_pubPolygonMask, lm_mmtPolygonMask.mmt_slTotalSize);
    ...
    ConvertBytesToBits(lm_pubPolygonMask, lm_pbsmShadowMap->bsm_pubPolygonMask, lm_mmtPolygonMask.mmt_slTotalSize);
  }
}
...
ULONG CLayerMaker::MakeShadowMask(CBrushShadowLayer *pbsl) {
  ...
  if (!(pbsl->bsl_plsLightSource->ls_ulFlags & LSF_CASTSHADOWS) || (lm_pbpoPolygon->bpo_ulFlags & BPOF_DOESNOTRECEIVESHADOW)) {
    // do nothing
  ...
  if (lm_pbpoPolygon->bpo_ulFlags & BPOF_ACCURATESHADOWS) {
    ...
    MakeMipmapsForMask(lm_pubLayer, lm_mmtLayer.mmt_pixU, lm_mmtLayer.mmt_pixV, lm_mmtLayer.mmt_slTotalSize);
  }
...
ULONG CLayerMaker::MakeOneShadowMaskMip(INDEX iMip) {
  ...
  if (lm_plsLight->ls_ulFlags & LSF_DIRECTIONAL) {
    ...
    prProjection.ScreenBBoxL()
      = FLOATaabbox2D(FLOAT2D(pixLayerMinU, pixLayerMinV), FLOAT2D(pixLayerMinU + pixLayerSizeU, pixLayerMinV + pixLayerSizeV));
    prProjection.AspectRatioL() = 1.0f;
    prProjection.NearClipDistanceL() = 0.00f;
    prProjection.pr_vZoomFactors(1) = prProjection.pr_vZoomFactors(2) = 1024.0f / (1 << iMipLevel);

    FLOAT3D vDirection;
    AnglesToDirectionVector(lm_plsLight->ls_penEntity->GetPlacement().pl_OrientationAngle, vDirection);
    ...
    plCenter.pl_PositionVector = vO + vStepU * (FLOAT(pixLayerSizeU) / 2 - 0.5f) // !!!!
    ...
    ulLighted &= RenderShadows(*lm_pwoWorld, *(CEntity *)NULL, apr, lm_pbpoPolygon->bpo_boxBoundingBox, pubLayer, pixLayerSizeU,
                               pixLayerSizeV, lm_plsLight->ls_ubPolygonalMask);
    ...
    if (!(lm_pbpoPolygon->bpo_ulFlags & BPOF_DARKCORNERS)) {
      prProjection.ScreenBBoxL()
        = FLOATaabbox2D(FLOAT2D(pixLayerMinU - fpixHotU + 1, pixLayerMinV - fpixHotV + 1),
                        FLOAT2D(pixLayerMinU + pixLayerSizeU - fpixHotU + 1, pixLayerMinV + pixLayerSizeV - fpixHotV + 1));
    } else {
      prProjection.ScreenBBoxL()
        = FLOATaabbox2D(FLOAT2D(pixLayerMinU - fpixHotU, pixLayerMinV - fpixHotV),
                        FLOAT2D(pixLayerMinU + pixLayerSizeU - fpixHotU, pixLayerMinV + pixLayerSizeV - fpixHotV));
    }
    prProjection.AspectRatioL() = 1.0f;
    ...
    if (lm_plsLight->ls_ubPolygonalMask == 0) {
      ...
      ulLighted &= RenderShadows(*lm_pwoWorld, *lm_plsLight->ls_penEntity, apr, FLOATaabbox3D(), pubLayer, pixLayerSizeU,
                                 pixLayerSizeV, lm_plsLight->ls_ubPolygonalMask);
      ...
      ulLighted &= RenderShadows(*lm_pwoWorld, *(CEntity *)NULL, apr, box, pubLayer, pixLayerSizeU, pixLayerSizeV,
...
BOOL CLayerMaker::CreateLayers(CBrushPolygon &bpo, CWorld &woWorld, BOOL bDoDirectionalLights) {
  ...
  FORDELETELIST(CBrushShadowLayer, bsl_lnInShadowMap, lm_pbsmShadowMap->bsm_lhLayers, itbsl) {
    ...
    if (itbsl->bsl_ulFlags & BSLF_CALCULATED || itbsl->bsl_plsLightSource->ls_ulFlags & LSF_DYNAMIC) {
      // skip it
      continue;
    }
    // if we are not doing directional lights and it is directional
    if (!bDoDirectionalLights && (itbsl->bsl_plsLightSource->ls_ulFlags & LSF_DIRECTIONAL)) {
      // skip it
