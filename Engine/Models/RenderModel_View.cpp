...
static void GetNeighbourTriangleVertices(Triangle *ptri, INDEX &i1, INDEX &i2) {
  ...
  for (INDEX itri = 0; itri < _atri.Count(); itri++) {
    ...
    i1 = triA.i1;
    i2 = triA.i2;
    if (i2 == triB.i0 && i1 == triB.i1)
      return;
    if (i2 == triB.i1 && i1 == triB.i2)
      return;
    if (i2 == triB.i2 && i1 == triB.i0)
      return;
    triA.Rotate();
    i1 = triA.i1;
    i2 = triA.i2;
...
static void PrepareSurfaceElements(ModelMipInfo &mmi, MappingSurface &ms) {
  ...
  FOREVER {
    ...
    if (ctTrianglesDone >= ctTriangles)
      break;
...
static int qsort_CompareSurfaceDiffuseTypes(const void *pSrf1, const void *pSrf2) {
  ...
  if (srf1.ms_aiPolygons.Count() == 0 || (srf1.ms_ulRenderingFlags & SRF_INVISIBLE)
      || srf1.ms_sttTranslucencyType == STT_ALPHAGOURAUD)
    return +1;
  if (srf2.ms_aiPolygons.Count() == 0 || (srf2.ms_ulRenderingFlags & SRF_INVISIBLE)
      || srf2.ms_sttTranslucencyType == STT_ALPHAGOURAUD)
    return -1;
  ...
  if (srf1.ms_sttTranslucencyType == srf2.ms_sttTranslucencyType) {
    ...
    if (bSrf1Spec && !bSrf2Spec)
      return -1;
    if (!bSrf1Spec && bSrf2Spec)
      return +1;
    if (bSrf1Refl && !bSrf2Refl)
      return -1;
    if (!bSrf1Refl && bSrf2Refl)
      return +1;
    // identical surfaces
  ...
  if (srf1.ms_sttTranslucencyType == STT_TRANSLUCENT) {
    if (srf2.ms_sttTranslucencyType == STT_OPAQUE || srf2.ms_sttTranslucencyType == STT_TRANSPARENT)
      return +1;
    return -1;
  }
  if (srf2.ms_sttTranslucencyType == STT_TRANSLUCENT) {
    if (srf1.ms_sttTranslucencyType == STT_OPAQUE || srf1.ms_sttTranslucencyType == STT_TRANSPARENT)
      return -1;
    return +1;
  }
  // ... then additive ...
  if (srf1.ms_sttTranslucencyType == STT_ADD) {
    if (srf2.ms_sttTranslucencyType == STT_MULTIPLY)
      return -1;
    return +1;
  }
  if (srf2.ms_sttTranslucencyType == STT_ADD) {
    if (srf1.ms_sttTranslucencyType == STT_MULTIPLY)
      return +1;
    return -1;
  }
  // ... then multiplicative.
...
extern void PrepareModelForRendering(CModelData &md) {
  ...
  if (md.md_bPreparedForRendering)
    return;
  ...
  for (INDEX iMip = 0; iMip < md.md_MipCt; iMip++)
    PrepareModelMipForRendering(md, iMip);
  // mark as prepared
...
COLOR _acol[]
  = {C_BLACK,   C_WHITE,   C_dGRAY,  C_GRAY,    C_lGRAY,  C_dRED,  C_RED,      C_lRED,    C_dGREEN,   C_GREEN,   C_lGREEN,
     C_dBLUE,   C_BLUE,    C_lBLUE,  C_dCYAN,   C_CYAN,   C_lCYAN, C_dMAGENTA, C_MAGENTA, C_lMAGENTA, C_dYELLOW, C_YELLOW,
     C_lYELLOW, C_dORANGE, C_ORANGE, C_lORANGE, C_dBROWN, C_BROWN, C_lBROWN,   C_dPINK,   C_PINK,     C_lPINK};
...
static void FlushElements(INDEX ctElem, INDEX *pai) {
  ...
  if (_bMultiPlayer)
    mdl_bShowStrips = 0; // don't allow in multiplayer mode!
  if (!mdl_bShowStrips) {
    ...
    if (_bMultiPlayer)
      mdl_bShowTriangles = 0; // don't allow in multiplayer mode!
...
static void RenderColors(CRenderModel &rm) {
  // only if required
  if (rm.rm_rtRenderType & RT_NO_POLYGON_FILL)
    return;
  _icol = 0;
  ...
  FOREACHINSTATICARRAY(mmi.mmpi_MappingSurfaces, MappingSurface, itms) {
    ...
    if ((ms.ms_ulRenderingFlags & SRF_INVISIBLE) || ms.ms_ctSrfVx == 0 || !(_ulColorMask & ms.ms_ulOnColor)
        || (_ulColorMask & ms.ms_ulOffColor)) {
      if (ctElements > 0)
        FlushElements(ctElements, &mmi.mmpi_aiElements[iStartElem]);
      iStartElem += ctElements + ms.ms_ctSrfEl;
    ...
    for (INDEX iSrfVx = 0; iSrfVx < ms.ms_ctSrfVx; iSrfVx++)
      pcolSrfBase[iSrfVx] = glcol;
    ctElements += ms.ms_ctSrfEl;
  }
  // all done
  if (ctElements > 0)
...
static void RenderWireframe(CRenderModel &rm) {
  // only if required
  if (!(rm.rm_rtRenderType & RT_WIRE_ON) && !(rm.rm_rtRenderType & RT_HIDDEN_LINES))
    return;
  _icol = 0;
...
vtxNext16L:
      ...
      for (INDEX iMipVx = 0; iMipVx < _ctAllMipVx; iMipVx++) {
        ...
        const FLOAT fSinH0 = pfSinTable[mfv0.mfv_ubNormH];
        const FLOAT fSinH1 = pfSinTable[mfv1.mfv_ubNormH];
        const FLOAT fSinP0 = pfSinTable[mfv0.mfv_ubNormP];
        const FLOAT fSinP1 = pfSinTable[mfv1.mfv_ubNormP];
        const FLOAT fCosH0 = pfCosTable[mfv0.mfv_ubNormH];
        const FLOAT fCosH1 = pfCosTable[mfv1.mfv_ubNormH];
        const FLOAT fCosP0 = pfCosTable[mfv0.mfv_ubNormP];
        const FLOAT fCosP1 = pfCosTable[mfv1.mfv_ubNormP];
...
vtxNext8:
    ...
    // if lerping
      ...
      fStretchX *= 0.00390625f;
      fOffsetX *= 256.0f;
      fStretchY *= 0.00390625f;
      fOffsetY *= 256.0f;
      fStretchZ *= 0.00390625f;
      fOffsetZ *= 256.0f;
      // for each vertex in mip
      __asm {
        mov     edi,D [pvtxMipBase]
...
void CModelObject::RenderModel_View(CRenderModel &rm) {
  ...
  ASSERT(_avtxMipBase.Count() == 0);
  _avtxMipBase.Push(_ctAllMipVx);
  ASSERT(_atexMipBase.Count() == 0);
  _atexMipBase.Push(_ctAllMipVx);
  ASSERT(_acolMipBase.Count() == 0);
  _acolMipBase.Push(_ctAllMipVx);
  ASSERT(_anorMipBase.Count() == 0);
  _anorMipBase.Push(_ctAllMipVx);

  ASSERT(_atexMipFogy.Count() == 0);
  _atexMipFogy.Push(_ctAllMipVx);
  ASSERT(_ashdMipFogy.Count() == 0);
  _ashdMipFogy.Push(_ctAllMipVx);
  ASSERT(_atx1MipHaze.Count() == 0);
  _atx1MipHaze.Push(_ctAllMipVx);
  ASSERT(_ashdMipHaze.Count() == 0);
  _ashdMipHaze.Push(_ctAllMipVx);

  ASSERT(_avtxSrfBase.Count() == 0);
  _avtxSrfBase.Push(_ctAllSrfVx);
  ASSERT(_atexSrfBase.Count() == 0);
  _atexSrfBase.Push(_ctAllSrfVx);
  ASSERT(_acolSrfBase.Count() == 0);
  _acolSrfBase.Push(_ctAllSrfVx);
  ...
  if (rm.rm_ulFlags & RMF_HAZE) {
    ...
    // get viewer offset
    // _fHazeAdd = (_vViewer%(rm.rm_vObjectPosition-_aprProjection->pr_vViewerPosition)) - _haze_hp.hp_fNear; // might cause a BUG
    // in compiler ????
  ...
  if (bModelSetupTimer)
    _sfStats.StopTimer(CStatForm::STI_MODELSETUP);
  ...
  if ((_ulMipLayerFlags & SRF_BUMP) && mdl_bRenderBump && ptdBump != NULL && bAllLayers && _eAPI == GAT_OGL) {
    ...
    FOREACHINSTATICARRAY(mmi.mmpi_MappingSurfaces, MappingSurface, itms) {
      ...
      if (!(ms.ms_ulRenderingFlags & SRF_BUMP))
        continue; // skip non-bump or empty surface
  ...
  if ((_ulMipLayerFlags & (SRF_DETAIL)) && mdl_bRenderDetail && ptdBump != NULL && ulTransAlpha > 192 && bAllLayers
      && !_bHasBump) {
    ...
    FOREACHINSTATICARRAY(mmi.mmpi_MappingSurfaces, MappingSurface, itms) {
      ...
      if ((ms.ms_ulRenderingFlags & SRF_INVISIBLE) || ctSrfVx == 0)
        break; // done if found invisible or empty surface
      if (!(ms.ms_ulRenderingFlags & SRF_DETAIL))
        continue; // skip non-detail surface
      // cache surface pointers
      pvTexCoord = &mmi.mmpi_avmexTexCoord[iSrfVx0];
      puwSrfToMip = &mmi.mmpi_auwSrfToMip[iSrfVx0];
      ptexSrfBase = &_atexSrfBase[iSrfVx0];
...
reflMipLoop:
...
#else
    // for each mip vertex
    for (INDEX iMipVx = 0; iMipVx < _ctAllMipVx; iMipVx++) { // get normal in absolute space
    ...
    FOREACHINSTATICARRAY(mmi.mmpi_MappingSurfaces, MappingSurface, itms) {
      ...
      if ((ms.ms_ulRenderingFlags & SRF_INVISIBLE) || ctSrfVx == 0)
        break; // done if found invisible or empty surface
      if (!(ms.ms_ulRenderingFlags & SRF_REFLECTIONS))
        continue; // skip non-reflection surface
      // cache surface pointers
      puwSrfToMip = &mmi.mmpi_auwSrfToMip[iSrfVx0];
      ptexSrfBase = &_atexSrfBase[iSrfVx0];
      pcolSrfBase = &_acolSrfBase[iSrfVx0];
...
specMipLoop:
...
#else
    // for each mip vertex
    for (INDEX iMipVx = 0; iMipVx < _ctAllMipVx; iMipVx++) { // reflect light vector around vertex normal in object space
    ...
    FOREACHINSTATICARRAY(mmi.mmpi_MappingSurfaces, MappingSurface, itms) {
      ...
      if ((ms.ms_ulRenderingFlags & SRF_INVISIBLE) || ctSrfVx == 0)
        break; // done if found invisible or empty surface
      if (!(ms.ms_ulRenderingFlags & SRF_SPECULAR))
        continue; // skip non-specular surface
      // cache surface pointers
      puwSrfToMip = &mmi.mmpi_auwSrfToMip[iSrfVx0];
      ptexSrfBase = &_atexSrfBase[iSrfVx0];
      pcolSrfBase = &_acolSrfBase[iSrfVx0];
      ...
      for (INDEX iSrfVx = 0; iSrfVx < ctSrfVx; iSrfVx++) {
        ...
        pcolSrfBase[iSrfVx].abgr = (((colSrfSpec.r) * slShade) >> 8) | (((colSrfSpec.g) * slShade) & 0x0000FF00)
  ...
  if ((rm.rm_ulFlags & RMF_HAZE) && !(_ulMipLayerFlags & MMI_TRANSLUCENT)) {
    ...
    FOREACHINSTATICARRAY(mmi.mmpi_MappingSurfaces, MappingSurface, itms) {
      ...
      if ((ms.ms_ulRenderingFlags & SRF_INVISIBLE) || ctSrfVx == 0)
        break; // done if found invisible or empty surface
      if (!(ms.ms_ulRenderingFlags & SRF_OPAQUE))
        continue; // skip not-solid or empty surfaces
      // cache surface pointers
      puwSrfToMip = &mmi.mmpi_auwSrfToMip[iSrfVx0];
      ptexSrfBase = &_atexSrfBase[iSrfVx0];
      pcolSrfBase = &_acolSrfBase[iSrfVx0];
...
void CModelObject::RenderPatches_View(CRenderModel &rm) {
  ...
  for (INDEX iMaskBit = 0; iMaskBit < MAX_TEXTUREPATCHES; iMaskBit++) {
    ...
    if (mo_PatchMask & ((1UL) << iMaskBit)) {
      ...
      if (ppp.ppp_auwElements.Count() == 0)
        continue;
...
void CModelObject::RenderShadow_View(CRenderModel &rm, const CPlacement3D &plLight, const FLOAT fFallOff, const FLOAT fHotSpot,
                                     const FLOAT fIntensity, const FLOATplane3D &plShadowPlane) {
  // no shadow, if projection is not perspective or no textures
  if (!_aprProjection.IsPerspective() || !(rm.rm_rtRenderType & RT_TEXTURE))
    return;
  ...
  const FLOAT3D vRef = plShadowPlaneObj.ProjectPoint(FLOAT3D(0.0f, 0.0f, 0.0f)) * rm.rm_mObjectToView + rm.rm_vObjectToView;
  ...
  ASSERT(_acolMipBase.Count() == 0);
  _acolMipBase.Push(_ctAllMipVx);
  ASSERT(_aooqMipShad.Count() == 0);
  _aooqMipShad.Push(_ctAllMipVx);
  ASSERT(_avtxSrfBase.Count() == 0);
  _avtxSrfBase.Push(_ctAllSrfVx);
  ASSERT(_acolSrfBase.Count() == 0);
  _acolSrfBase.Push(_ctAllSrfVx);
  ASSERT(_atx4SrfShad.Count() == 0);
  _atx4SrfShad.Push(_ctAllSrfVx);
  ...
  // for each mip vertex
  {
    for (INDEX iMipVx = 0; iMipVx < _ctAllMipVx; iMipVx++) {
      ...
      FLOAT fDt
        = plShadowPlaneObj(1) * vtx.x + plShadowPlaneObj(2) * vtx.y + plShadowPlaneObj(3) * vtx.z - plShadowPlaneObj.Distance();
      ...
      // if vertex between shadow plane and light
        ...
        if (fDls < fHotSpot)
          pubsMipBase[iMipVx] = iLightMax;
        else if (fDls > fFallOff)
          pubsMipBase[iMipVx] = 0;
        else
          pubsMipBase[iMipVx] = FloatToInt((fFallOff - fDls) * fLightStep);
      }
    }
  }
...
void CModelObject::AddSimpleShadow_View(CRenderModel &rm, const FLOAT fIntensity, const FLOATplane3D &plShadowPlane) {
  ...
  const FLOAT3D v00
    = plShadowPlaneObj.ProjectPoint(FLOAT3D(vMin(1), vMin(2), vMin(3))) * rm.rm_mObjectToView + rm.rm_vObjectToView;
  const FLOAT3D v01
    = plShadowPlaneObj.ProjectPoint(FLOAT3D(vMin(1), vMin(2), vMax(3))) * rm.rm_mObjectToView + rm.rm_vObjectToView;
  const FLOAT3D v10
    = plShadowPlaneObj.ProjectPoint(FLOAT3D(vMax(1), vMin(2), vMin(3))) * rm.rm_mObjectToView + rm.rm_vObjectToView;
  const FLOAT3D v11
    = plShadowPlaneObj.ProjectPoint(FLOAT3D(vMax(1), vMin(2), vMax(3))) * rm.rm_mObjectToView + rm.rm_vObjectToView;
  // calc done
  ...
  // if this model has fog
  if (rm.rm_ulFlags & RMF_FOG) { // for each vertex in shadow quad
    GFXTexCoord tex;
    for (INDEX i = 0; i < 4; i++) {
  ...
  // if this model has haze
  if (rm.rm_ulFlags & RMF_HAZE) { // for each vertex in shadow quad
    for (INDEX i = 0; i < 4; i++) {
