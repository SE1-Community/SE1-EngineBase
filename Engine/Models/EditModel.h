...
class ENGINE_API CEditModel : public CSerial {
  ...
  public:
    ...
    void DefaultMapping(INDEX iCurrentMip,
                        INDEX iSurface = -1); // sets default mapping for given mip-model and surface (or all surfaces -1)
    ...
    void DrawWireSurface(CDrawPort *pDP, INDEX iCurrentMip, INDEX iCurrentSurface, FLOAT fMagnifyFactor, PIX offx, PIX offy,
                         COLOR clrVisible, COLOR clrInvisible); // draws given surface in wire frame
    void DrawFilledSurface(CDrawPort *pDP, INDEX iCurrentMip, INDEX iCurrentSurface, FLOAT fMagnifyFactor, PIX offx, PIX offy,
                           COLOR clrVisible, COLOR clrInvisible); // fills given surface with color
    void PrintSurfaceNumbers(CDrawPort *pDP, CFontData *pFont, INDEX iCurrentMip, FLOAT fMagnifyFactor, PIX offx, PIX offy,
                             COLOR clrInk); // prints surface numbers
    void ExportSurfaceNumbersAndNames(CTFileName fnFile);
    void GetScreenSurfaceCenter(CDrawPort *pDP, INDEX iCurrentMip, INDEX iCurrentSurface, FLOAT fMagnifyFactor, PIX offx, PIX offy,
    ...
    BOOL ChangeSurfacePositionRelative(INDEX iCurrentMip, INDEX iCurrentSurface, FLOAT fDZoom, FLOAT dX, FLOAT dY, PIX dTH,
                                       PIX dTV); // if it can, sets surface position with given offseted values
    BOOL ChangeSurfacePositionAbsolute(INDEX iCurrentMip, INDEX iCurrentSurface, FLOAT newZoom, FLOAT3D newCenter,
                                       FLOAT3D newAngle);               // if it can, sets surface position to given values
    void RememberSurfaceUndo(INDEX iCurrentMip, INDEX iCurrentSurface); // remember undo position
    ModelTextureVertex *FindClosestMappingVertex(INDEX iCurrentMip, INDEX iCurrentSurface, PIX ptx, PIX pty, FLOAT fMagnifyFactor,
                                                 PIX offx, PIX offy); // finds closest point and returns it
    void MoveVertex(ModelTextureVertex *pmtvClosestVertex, PIX dx, PIX dy, INDEX iCurrentMip,
                    INDEX iCurrentSurface);                         // try new vertex position
