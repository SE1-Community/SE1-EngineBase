...
struct VertexNeighbors {
  CStaticStackArray<INDEX> vp_aiNeighbors;
};
...
void CEditModel::LoadModelAnimationData_t(CTStream *pFile, const FLOATmatrix3D &mStretch) // throw char *
{
  try {
    ...
    if (edm_md.md_Stretch(1) == 0.0f)
      edm_md.md_Stretch(1) = 1.0f;
    if (edm_md.md_Stretch(2) == 0.0f)
      edm_md.md_Stretch(2) = 1.0f;
    if (edm_md.md_Stretch(3) == 0.0f)
      edm_md.md_Stretch(3) = 1.0f;
    ...
    for (INDEX iFr = 0; iFr < edm_md.md_FramesCt; iFr++) {
      ...
      for (INDEX iVtx = 0; iVtx < edm_md.md_VerticesCt; iVtx++) // for all vertices in one frame
      {
        ...
        if (edm_md.md_Flags & MF_COMPRESSED_16BIT) {
          edm_md.md_FrameVertices16[iFVtx].mfv_SWPoint
            = SWPOINT3D(FloatToInt((vVtx(1) - vCenter(1)) * f1oStretchX), FloatToInt((vVtx(2) - vCenter(2)) * f1oStretchY),
                        FloatToInt((vVtx(3) - vCenter(3)) * f1oStretchZ));
        } else {
          edm_md.md_FrameVertices8[iFVtx].mfv_SBPoint
            = SBPOINT3D(FloatToInt((vVtx(1) - vCenter(1)) * f1oStretchX), FloatToInt((vVtx(2) - vCenter(2)) * f1oStretchY),
                        FloatToInt((vVtx(3) - vCenter(3)) * f1oStretchZ));
        }
        ...
        if (edm_md.md_Flags & MF_COMPRESSED_16BIT) {
          CompressNormal_HQ(vSum, edm_md.md_FrameVertices16[iFVtx].mfv_ubNormH, edm_md.md_FrameVertices16[iFVtx].mfv_ubNormP);
        } else {
...
void CEditModel::SaveIncludeFile_t(CTFileName fnFileName, CTString strDefinePrefix) // throw char *
{
  ...
  for (INDEX iCollisionBox = 0; iCollisionBox < edm_md.md_acbCollisionBox.Count(); iCollisionBox++) {
    ...
    sprintf(line, "#define %s_COLLISION_BOX_%s %d\n", strDefinePrefix, GetCollisionBoxName(iCollisionBox), iCollisionBox);
    strmHFile.Write_t(line, strlen(line));
  }
  ...
  for (INDEX iSound = 0; iSound < edm_aasAttachedSounds.Count(); iSound++) {
    if (edm_aasAttachedSounds[iSound].as_fnAttachedSound != "") {
      CTString strLooping;
      if (edm_aasAttachedSounds[iSound].as_bLooping)
        strLooping = "L";
      else
        strLooping = "NL";
      ...
      sprintf(line, "//sound SOUND_%s_%-16s %-32s // %s, %s, %s\n", strDefinePrefix, aiInfo.ai_AnimName, strWithQuotes,
              strAnimationPrefix + aiInfo.ai_AnimName, strLooping, strDelay);
      strmHFile.Write_t(line, strlen(line));
    }
  }
...
CTextureDataInfo *CEditModel::AddTexture_t(const CTFileName &fnFileName, const MEX mexWidth, const MEX mexHeight) {
  ...
  try {
    ...
    FOREVER {
      ...
      // Key-word "SIZE" defines stretch factor
        ...
        FLOATmatrix3D mTran;
        mTran.Diagonal(1.0f);
        sscanf(ld_line, "TRANSFORM %g %g %g %g %g %g %g %g %g", &mTran(1, 1), &mTran(1, 2), &mTran(1, 3), &mTran(2, 1),
               &mTran(2, 2), &mTran(2, 3), &mTran(3, 1), &mTran(3, 2), &mTran(3, 3));
        mStretch *= mTran;
      }
    ...
    if (bAnimationsFound != TRUE)
      throw("There are no animations defined for this model, and that can't be. Probable cause: script missing key-word "
            "\"ANIM_START\".");
    ...
    // import mapping
    if ((fnImportMapping != "") || ((fnClosed != "") && (fnOpened != "") && (fnUnwrapped != ""))) {
      ...
      if ((ctModelPolygons != ctClosedPolygons) || (ctModelPolygons != ctOpenedPolygons)
          || (ctModelPolygons != ctUnwrappedPolygons)) {
        ThrowF_t(
          "ERROR: Object used to create model and some of objects used to define mapping don't have same number of polygons!");
      }
...
void CEditModel::NewModel(CObject3D *pO3D) {
  ...
  for (INDEX i = 0; i < edm_md.md_VerticesCt; i++) {
    ...
    edm_md.md_MainMipVertices[i] = DOUBLEtoFLOAT(pO3D->ob_aoscSectors[0].osc_aovxVertices[i]);
...
void CEditModel::AddMipModel(CObject3D *pO3D) {
  ...
  for (i = 0; i < o3dvct; i++) {
    ...
    if (same_found == FALSE) // if no vertice close enough is found, we have error
    {
      ThrowF_t("Vertex from mip model %d with number %d, coordinates (%f,%f,%f), can't be found in main mip model.\n"
               ...
               edm_md.md_MipCt, i, pO3D->ob_aoscSectors[0].osc_aovxVertices[i](1), pO3D->ob_aoscSectors[0].osc_aovxVertices[i](2),
               pO3D->ob_aoscSectors[0].osc_aovxVertices[i](3));
    }
  }
  ...
  for (i = 0; i < pO3D->ob_aoscSectors[0].osc_aomtMaterials.Count(); i++) {
    ...
    ms.ms_colColor = pO3D->ob_aoscSectors[0].osc_aomtMaterials[i].omt_Color | CT_OPAQUE; // copy surface color, set no alpha
  ...
  for (i = 0; i < pmmpi->mmpi_PolygonsCt; i++) // loop all model polygons
  {
    ...
    pmp->mp_Surface = pO3D->ob_aoscSectors[0].osc_aomtMaterials.Index(
      pO3D->ob_aoscSectors[0].osc_aopoPolygons[i].opo_Material); // copy surface index
...
void CEditModel::CalculateUnwrappedMapping(CObject3D &o3dClosed, CObject3D &o3dOpened, CObject3D &o3dUnwrapped) {
  ...
  for (INDEX iSurface = 0; iSurface < pMMI->mmpi_MappingSurfaces.Count(); iSurface++) {
    ...
    for (INDEX iSurfaceTextureVertex = 0; iSurfaceTextureVertex < pmsSurface->ms_aiTextureVertices.Count();
         iSurfaceTextureVertex++) {
      ...
      for (INDEX iOpenedPolygon = 0; iOpenedPolygon < o3dOpened.ob_aoscSectors[0].osc_aopoPolygons.Count(); iOpenedPolygon++) {
        ...
        if (popoOpenedPolygon->opo_Material->omt_Name != pmsSurface->ms_Name)
          continue;
...
//----------------------------------------------------------------------------------------------
void CEditModel::CreateMipModels_t(CObject3D &objRestFrame, CObject3D &objMipSourceFrame, INDEX iVertexRemoveRate,
      ...
      if (EQUAL_SUB_STR("SIZE")) {
        ...
        for (i = 0; i < no_of_mip_models - 1; i++) {
          ...
          if (edm_md.md_VerticesCt < O3D.ob_aoscSectors[0].osc_aovxVertices.Count()) {
            ThrowF_t("It is unlikely that mip-model \"%s\" is valid.\n"
          ...
          if (edm_md.md_MipCt < MAX_MODELMIPS) {
            ...
            ThrowF_t("There are too many mip-models defined in script file. Maximum of %d mip-models allowed.",
                     MAX_MODELMIPS - 1);
          }
        }
      } else if (EQUAL_SUB_STR("ORIGIN_TRI")) {
 ...
 */
void CEditModel::DrawWireSurface(CDrawPort *pDP, INDEX iCurrentMip, INDEX iCurrentSurface, FLOAT fMagnifyFactor, PIX offx,
                                 PIX offy, COLOR clrVisible, COLOR clrInvisible) {
  ...
  for (INDEX iPoly = 0; iPoly < edm_md.md_MipInfos[iCurrentMip].mmpi_PolygonsCt; iPoly++) {
    ...
    if (pPoly->mp_Surface == iCurrentSurface) { // readout poly vertices
 ...
 */
void CEditModel::DrawFilledSurface(CDrawPort *pDP, INDEX iCurrentMip, INDEX iCurrentSurface, FLOAT fMagnifyFactor, PIX offx,
                                   PIX offy, COLOR clrVisible, COLOR clrInvisible) {
  FLOAT3D f3dTr0, f3dTr1, f3dTr2;
  ...
  for (INDEX iPoly = 0; iPoly < edm_md.md_MipInfos[iCurrentMip].mmpi_PolygonsCt; iPoly++) {
    ...
    if (pPoly->mp_Surface == iCurrentSurface) { // readout poly vertices
      ...
      FLOAT3D f3dNormal = (f3dTr2 - f3dTr1) * (f3dTr0 - f3dTr1);
      if (f3dNormal(3) < 0)
        clrFill = clrVisible | 0xFF;
      else
        clrFill = clrInvisible | 0xFF;
 ...
 */
void CEditModel::PrintSurfaceNumbers(CDrawPort *pDP, CFontData *pFont, INDEX iCurrentMip, FLOAT fMagnifyFactor, PIX offx,
                                     PIX offy, COLOR clrInk) {
  char achrLine[256];
      ...
      for (INDEX iPatch = 0; iPatch < MAX_TEXTUREPATCHES; iPatch++) {
        ...
        if (edm_md.md_mpPatches[iPatch].mp_toTexture.GetName() != "") {
          ...
          MEXaabbox2D boxPatch = MEXaabbox2D(
            mex2dPosition, MEX2D(mex2dPosition(1) + pTD->GetWidth() * fStretch, mex2dPosition(2) + pTD->GetHeight() * fStretch));
          // for each polygon
          ...
          if (ctOccupiedPolygons != 0) {
            ...
            for (INDEX iOccupied = 0; iOccupied < ctOccupiedPolygons; iOccupied++) {
              pMMI->mmpi_aPolygonsPerPatch[iExistingPatch].ppp_iPolygons[iOccupied] = aiPolygons[iOccupied];
            }
          }
          // count existing patches
          iExistingPatch++;
...
void CEditModel::CorrectCollisionBoxSize(void) {
  ...
  if (vCorrectedDiagonale(1) < 0.1f)
    vCorrectedDiagonale(1) = 0.01f;
  if (vCorrectedDiagonale(2) < 0.1f)
    vCorrectedDiagonale(2) = 0.01f;
  if (vCorrectedDiagonale(3) < 0.1f)
    vCorrectedDiagonale(3) = 0.01f;
...
void CEditModel::SetCollisionBoxDimensionEquality(INDEX iNewDimEqType) {
  edm_md.md_acbCollisionBox.Lock();
  edm_md.md_acbCollisionBox[edm_iActiveCollisionBox].mcb_iCollisionBoxDimensionEquality = iNewDimEqType;
  edm_md.md_acbCollisionBox.Unlock();
  CorrectCollisionBoxSize();
};
