...
struct ENGINE_API ModelMipInfo {
  ...
  CStaticArray<struct PolygonsPerPatch>
    mmpi_aPolygonsPerPatch;                        // for each patch info telling which polygons are occupied by that patch
...
class ENGINE_API CModelData : public CAnimData {
  public:
    ...
    CStaticArray<FLOAT3D> md_MainMipVertices; // global array with coordinates of all vertices of main mip-model, used for creating
                                              // mip-models [VerticesCt]
    CStaticArray<struct TransformedVertexData>
      md_TransformedVertices;                 // buffer with i,j,k, rotated and stretched gouraud normal [VerticesCt]
    ...
    struct ModelMipInfo
      md_MipInfos[MAX_MODELMIPS]; // contains how many polygons and which contains which vertices (different for each mip-model)
