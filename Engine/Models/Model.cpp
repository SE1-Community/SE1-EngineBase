...
CModelData::CModelData(const CModelData &c) {
  ASSERT(FALSE);
};
CModelData &CModelData::operator=(const CModelData &c) {
  ASSERT(FALSE);
  return *this;
};
...
COLOR PaletteColorValues[] = {
  C_RED,     C_GREEN,   C_BLUE,    C_CYAN,    C_MAGENTA, C_YELLOW, C_ORANGE, C_BROWN, C_PINK,  C_dGRAY,    C_GRAY,
  C_lGRAY,   C_dRED,    C_lRED,    C_dGREEN,  C_lGREEN,  C_dBLUE,  C_lBLUE,  C_dCYAN, C_lCYAN, C_dMAGENTA, C_lMAGENTA,
  C_dYELLOW, C_lYELLOW, C_dORANGE, C_lORANGE, C_dBROWN,  C_lBROWN, C_dPINK,  C_lPINK, C_WHITE, C_BLACK,
};
...
void MappingSurface::SetRenderingParameters(ULONG ulOldFlags) {
  ...
  if (ulOldFlags & PCF_NOSHADING) {
    ...
    switch (ulOldFlags & PS_MASK) {
      case PS_MATTE: ms_sstShadingType = SST_MATTE; break;
      case PS_SHINY: ms_sstShadingType = SST_MATTE; break;
      case PS_METAL: ms_sstShadingType = SST_MATTE; break;
      default: ms_sstShadingType = SST_MATTE;
    }
  }
 ...
 */
void ModelMipInfo::Read_t(CTStream *pFile, BOOL bReadPolygonalPatches, BOOL bReadPolygonsPerSurface, BOOL bReadSurfaceColors) {
  INDEX iMembersCt;
...
void CModelCollisionBox::Read_t(CTStream *istrFile) {
  ...
  } else if ((mcb_vCollisionBoxMax(3) - mcb_vCollisionBoxMin(3)) == (mcb_vCollisionBoxMax(1) - mcb_vCollisionBoxMin(1))) {
    mcb_iCollisionBoxDimensionEquality = LENGTH_EQ_WIDTH;
  } else if ((mcb_vCollisionBoxMax(3) - mcb_vCollisionBoxMin(3)) == (mcb_vCollisionBoxMax(2) - mcb_vCollisionBoxMin(2))) {
...
void CModelData::Write_t(CTStream *pFile) // throw char *
{
  ...
  if (md_Flags & MF_COMPRESSED_16BIT) {
    pFile->WriteFullChunk_t(CChunkID("AV17"), &md_FrameVertices16[0],
                            md_VerticesCt * md_FramesCt * sizeof(struct ModelFrameVertex16));
  } else {
    pFile->WriteFullChunk_t(CChunkID("AFVX"), &md_FrameVertices8[0],
                            md_VerticesCt * md_FramesCt * sizeof(struct ModelFrameVertex8));
  }
...
void CModelData::Read_t(CTStream *pFile) // throw char *
{
  ...
  if (md_Flags & MF_COMPRESSED_16BIT) {
    ...
    // load new 16-bit compressed format (normals use 2 byte) model
      ...
      pFile->ReadFullChunk_t(CChunkID("AV17"), &md_FrameVertices16[0],
                             md_VerticesCt * md_FramesCt * sizeof(struct ModelFrameVertex16));
    } else {
    ...
    pFile->ReadFullChunk_t(CChunkID("AFVX"), &md_FrameVertices8[0],
                           md_VerticesCt * md_FramesCt * sizeof(struct ModelFrameVertex8));
  }
...
CTString CModelData::GetDescription(void) {
  ...
  str.PrintF("%d mips, %d anims, %d frames, %d vtx, %d svx, %d tri", md_MipCt, ad_NumberOfAnims, md_FramesCt, mmi0.mmpi_ctMipVx,
             mmi0.mmpi_ctSrfVx, mmi0.mmpi_ctTriangles);
...
void CModelObject::GetModelInfo(CModelInfo &miInfo) {
  ...
  for (INDEX i = 0; i < pMD->md_MipCt; i++) {
    ...
    for (INDEX iPolygon = 0; iPolygon < pMD->md_MipInfos[i].mmpi_PolygonsCt; iPolygon++) {
      miInfo.mi_MipInfos[i].mi_TrianglesCt += pMD->md_MipInfos[i].mmpi_Polygons[iPolygon].mp_PolygonVertices.Count() - 2;
    }
...
COLOR CModelObject::GetSurfaceColor(INDEX iCurrentMip, INDEX iCurrentSurface) {
  ...
  CModelData *pMD = (CModelData *)GetData();
  if ((iCurrentMip >= pMD->md_MipCt) || (iCurrentSurface >= pMD->md_MipInfos[iCurrentMip].mmpi_MappingSurfaces.Count())) {
    return -1;
  }
 ...
 */
void CModelObject::GetSurfaceRenderFlags(INDEX iCurrentMip, INDEX iCurrentSurface, enum SurfaceShadingType &sstShading,
                                         enum SurfaceTranslucencyType &sttTranslucency, ULONG &ulRenderingFlags) {
  CModelData *pMD = (CModelData *)GetData();
  if ((iCurrentMip >= pMD->md_MipCt) || (iCurrentSurface >= pMD->md_MipInfos[iCurrentMip].mmpi_MappingSurfaces.Count())) {
    return;
  }
 ...
 */
void CModelObject::SetSurfaceRenderFlags(INDEX iCurrentMip, INDEX iCurrentSurface, enum SurfaceShadingType sstShading,
                                         enum SurfaceTranslucencyType sttTranslucency, ULONG ulRenderingFlags) {
  CModelData *pMD = (CModelData *)GetData();
  if ((iCurrentMip >= pMD->md_MipCt) || (iCurrentSurface >= pMD->md_MipInfos[iCurrentMip].mmpi_MappingSurfaces.Count())) {
    return;
 ...
 */
void CModelObject::ApplySurfaceToPolygonsInRegion(CDrawPort *pDP, CProjection3D *pProjection, PIXaabbox2D box, INDEX iSurface,
 ...
 * Find hitted polygon
 */
struct ModelPolygon *CModelObject::PolygonHit(CPlacement3D plRay, CPlacement3D plObject, INDEX iCurrentMip, FLOAT &fHitDistance) {
