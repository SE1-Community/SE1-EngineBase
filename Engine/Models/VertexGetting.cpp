...
void CModelObject::GetModelVertices(CStaticStackArray<FLOAT3D> &avVertices, FLOATmatrix3D &mRotation, FLOAT3D &vPosition,
                                    FLOAT fNormalOffset, FLOAT fMipFactor) {
  FLOAT3D f3dVertex;
  ...
  FOREACHINLIST(CAttachmentModelObject, amo_lnInMain, mo_lhAttachments, itamo) {
    ...
    ASSERT(pmd != NULL);
    if (pmd == NULL || pmd->md_Flags & (MF_FACE_FORWARD | MF_HALF_FACE_FORWARD))
      continue;
...
void CModelObject::GetAttachmentTransformations(INDEX iAttachment, FLOATmatrix3D &mRotation, FLOAT3D &vPosition,
