...
class ENGINE_API CModelObject : public CAnimObject {
  private:
    ...
    void RenderShadow_View(CRenderModel &rm, const CPlacement3D &plLight, const FLOAT fFallOff, const FLOAT fHotSpot,
  ...
  public:
    ...
    void GetSurfaceRenderFlags(INDEX iCurrentMip, INDEX iCurrentSurface, enum SurfaceShadingType &sstShading,
                               enum SurfaceTranslucencyType &sttTranslucency, ULONG &ulRenderingFlags);
    void SetSurfaceRenderFlags(INDEX iCurrentMip, INDEX iCurrentSurface, enum SurfaceShadingType sstShading,
                               enum SurfaceTranslucencyType sttTranslucency, ULONG ulRenderingFlags);
    ...
    void RenderShadow(CRenderModel &rm, const CPlacement3D &plLight, const FLOAT fFallOff, const FLOAT fHotSpot,
                      const FLOAT fIntensity, const FLOATplane3D &plShadowPlane);

    // Get model vertices in absoulte space
    void GetModelVertices(CStaticStackArray<FLOAT3D> &avVertices, FLOATmatrix3D &mRotation, FLOAT3D &vPosition, FLOAT fNormalOffset,
                          FLOAT fMipFactor);
    ...
    void ColorizeRegion(CDrawPort *pDP, CProjection3D *projection, PIXaabbox2D box, INDEX iChoosedColor,
                        BOOL bOnColorMode); // colorizes polygons touching given box
    void ColorizePolygon(CDrawPort *pDP, CProjection3D *projection, PIX x1, PIX y1, INDEX iChoosedColor,
                         BOOL bOnColorMode); // colorizes hitted polygon
    void ApplySurfaceToPolygon(CDrawPort *pDP, CProjection3D *projection, PIX x1, PIX y1, INDEX iSurface, COLOR colSurfaceColor);
    void ApplySurfaceToPolygonsInRegion(CDrawPort *pDP, CProjection3D *projection, PIXaabbox2D box, INDEX iSurface,
    ...
    struct ModelPolygon *PolygonHit(CPlacement3D plRay, CPlacement3D plObject, INDEX iCurrentMip,
                                    FLOAT &fHitDistance); // returns ptr to hitted polygon (NULL if none)
    struct ModelPolygon *PolygonHitModelData(CModelData *pMD, CPlacement3D plRay, CPlacement3D plObject, INDEX iCurrentMip,
                                             FLOAT &fHitDistance);
    void PickPolyColor(CDrawPort *pDP, CProjection3D *projection, PIX x1, PIX y1, INDEX &iPickedColorNo,
                       BOOL bOnColorMode);                                            // picks color from hitted polygon
    INDEX PickPolySurface(CDrawPort *pDP, CProjection3D *projection, PIX x1, PIX y1); // picks surface from hitted polygon
    INDEX PickVertexIndex(CDrawPort *pDP, CProjection3D *projection, PIX x1, PIX y1,
                          FLOAT3D &vClosestVertex); // obtains index of closest vertex
