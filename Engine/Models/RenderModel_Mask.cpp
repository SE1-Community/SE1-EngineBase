...
static void RenderOneSide(CRenderModel &rm, const INDEX iVisibility) {
  ...
  FOREACHINSTATICARRAY(mmiMip.mmpi_MappingSurfaces, MappingSurface, itms) {
    ...
    if (iVisibility == VISIBLE_BACK && !(ulFlags & SRF_DOUBLESIDED))
      continue;
    ...
    for (INDEX iipo = 0; iipo < ms.ms_aiPolygons.Count(); iipo++) {
      ...
      if (mpPolygon.mp_slVisibility != iVisibility)
        continue;
...
void CModelObject::RenderModel_Mask(CRenderModel &rm) {
  ...
  bPerspective = TRUE;
  if (!_aprProjection.IsPerspective())
    bPerspective = FALSE;
  ...
  if (ptd == NULL)
    rm.rm_rtRenderType = (rm.rm_rtRenderType & ~RT_TEXTURE_MASK) | RT_WHITE_TEXTURE;
  ...
  for (INDEX ivx = 0; ivx < rm.rm_pmdModelData->md_VerticesCt; ivx++) {
    ...
    if (tvd.tvd_pv2.pv2_fI < 0 || tvd.tvd_pv2.pv2_fI >= pixWidth || tvd.tvd_fZ > fFrontClipDistance
        || (fBackClipDistance < 0 && tvd.tvd_fZ < fBackClipDistance)) {
  ...
  FOREACHINSTATICARRAY(mmiMip.mmpi_Polygons, ModelPolygon, itmp) {
    ...
    if (bPerspective) {
      ...
      fVisible
        = fNX * prParallel.pr_vViewDirection(1) + fNY * prParallel.pr_vViewDirection(2) + fNZ * prParallel.pr_vViewDirection(3);
    }
