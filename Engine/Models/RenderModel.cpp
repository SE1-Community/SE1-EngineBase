...
void BeginModelRenderingView(CAnyProjection3D &prProjection, CDrawPort *pdp) {
  ...
  if (prProjection->pr_bMirror)
    prProjection->pr_plMirrorView.pl_distance -= 0.06f; // -0.06 is because entire projection is offseted by +0.05
...
static void CalculateBoundingBox(CModelObject *pmo, CRenderModel &rm) {
  ...
  rm.rm_vObjectMinBB(1) *= pmo->mo_Stretch(1);
  rm.rm_vObjectMaxBB(1) *= pmo->mo_Stretch(1);
  rm.rm_vObjectMinBB(2) *= pmo->mo_Stretch(2);
  rm.rm_vObjectMaxBB(2) *= pmo->mo_Stretch(2);
  rm.rm_vObjectMinBB(3) *= pmo->mo_Stretch(3);
  rm.rm_vObjectMaxBB(3) *= pmo->mo_Stretch(3);
...
BOOL CModelObject::CreateAttachment(CRenderModel &rmMain, CAttachmentModelObject &amo) {
  ...
  const FLOATobbox3D boxEntity(FLOATaabbox3D(rmAttached.rm_vObjectMinBB, rmAttached.rm_vObjectMaxBB), vHandle,
                               _aprProjection->pr_ViewerRotationMatrix * mOrientation);
  // frustum test?
  if (gap_iOptimizeClipping > 1) {
    ...
    if (iFrustumTest > 0)
      rmAttached.rm_ulFlags |= RMF_INSIDE;
  ...
  if (_aprProjection->pr_bMirror || _aprProjection->pr_bWarp) {
    ...
    if (iMirrorPlaneTest > 0)
      rmAttached.rm_ulFlags |= RMF_INMIRROR;
...
void CModelObject::RenderModel(CRenderModel &rm) {
  ...
  if (mo_Stretch == FLOAT3D(0.0f, 0.0f, 0.0f))
    return;
  ...
  if (!(rm.rm_ulFlags & RMF_SPECTATOR)
      && (!(rm.rm_rtRenderType & RT_NO_POLYGON_FILL) || (rm.rm_rtRenderType & RT_WIRE_ON)
          || (rm.rm_rtRenderType & RT_HIDDEN_LINES))) {
    // eventually calculate projection model bounding box in object space (needed for fog/haze trivial rejection)
    if (rm.rm_ulFlags & (RMF_FOG | RMF_HAZE))
  ...
  FOREACHINLIST(CAttachmentModelObject, amo_lnInMain, mo_lhAttachments, itamo) {
    ...
    if (pamo->amo_prm == NULL)
      continue; // skip view-rejected attachments
    _pfModelProfile.StopTimer(CModelProfile::PTI_RENDERMODEL);
 ...
 */
//--------------------------------------------------------------------------------------------
void CModelObject::RenderShadow(CRenderModel &rm, const CPlacement3D &plLight, const FLOAT fFallOff, const FLOAT fHotSpot,
  ...
  if (!HasShadow(rm.rm_iMipLevel) || (rm.rm_pmdModelData->md_Flags & (MF_FACE_FORWARD | MF_HALF_FACE_FORWARD)))
    return;
  ASSERT(_iRenderingType == 1);
...
void CModelObject::AddSimpleShadow(CRenderModel &rm, const FLOAT fIntensity, const FLOATplane3D &plShadowPlane) {
  ...
  if (!HasShadow(rm.rm_iMipLevel) || fIntensity < 0.01f || !_aprProjection.IsPerspective()
      || (rm.rm_pmdModelData->md_Flags & (MF_FACE_FORWARD | MF_HALF_FACE_FORWARD)))
    return;
  ASSERT(_iRenderingType == 1);
  ASSERT(fIntensity > 0 && fIntensity <= 1);
  // do some rendering
