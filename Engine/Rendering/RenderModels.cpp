...
BOOL CRenderer::FindModelLights(CEntity &en, const CPlacement3D &plModel, COLOR &colLight, COLOR &colAmbient,
  ...
  if (en.en_psiShadingInfo == NULL /* || en.en_psiShadingInfo->si_pbpoPolygon == NULL*/) { // no shadow
  ...
  // if there is valid shading info
    ...
    if (en.en_psiShadingInfo->si_ptrTerrain != NULL) {
      ...
      CTextureBlending &tbShadow
        = re_pwoWorld->wo_atbTextureBlendings[en.en_psiShadingInfo->si_pbpoPolygon->bpo_bppProperties.bpp_ubShadowBlend];
 ...
 */
void CRenderer::RenderOneModel(CEntity &en, CModelObject &moModel, const CPlacement3D &plModel, const FLOAT fDistanceFactor,
                               BOOL bRenderShadow, ULONG ulDMFlags) {
  // skip invisible models
  if (moModel.mo_Stretch == FLOAT3D(0.0f, 0.0f, 0.0f))
    return;

  // do nothing, if rendering shadows and this model doesn't cast cluster shadows
  if (re_bRenderingShadows && !(en.en_ulFlags & ENF_CLUSTERSHADOWS))
    return;
  ...
  if (!re_bRenderingShadows) {
    // find model lights
    bRenderModelShadow
      = FindModelLights(en, plModel, colLight, colAmbient, fTotalShadowIntensity, vTotalLightDirection, plFloorPlane);
  }
  ...
  if (ulDMFlags & DMF_FOG)
    rm.rm_ulFlags |= RMF_FOG;
  if (ulDMFlags & DMF_HAZE)
    rm.rm_ulFlags |= RMF_HAZE;
  if (ulDMFlags & DMF_INSIDE)
    rm.rm_ulFlags |= RMF_INSIDE;
  if (ulDMFlags & DMF_INMIRROR)
  ...
  if (bRenderModelShadow && !(en.en_ulFlags & ENF_CLUSTERSHADOWS) && moModel.HasShadow(rm.rm_iMipLevel)) {
    ...
    // if full shadows
      ...
      for (INDEX iLight = 0; iLight < _amlLights.Count(); iLight++) {
        ...
        if (!(ml.ml_plsLight->ls_ulFlags & LSF_CASTSHADOWS))
          continue;
 ...
 */
void CRenderer::RenderOneSkaModel(CEntity &en, const CPlacement3D &plModel, const FLOAT fDistanceFactor, BOOL bRenderShadow,
                                  ULONG ulDMFlags) {
  // skip invisible models
  if (en.GetModelInstance()->mi_vStretch == FLOAT3D(0.0f, 0.0f, 0.0f))
    return;
  ...
  if (ulDMFlags & DMF_HAZE)
    ulRenFlags |= RMF_HAZE;
  if (ulDMFlags & DMF_INSIDE)
    ulRenFlags |= RMF_INSIDE;
  if (ulDMFlags & DMF_INMIRROR)
    ulRenFlags |= RMF_INMIRROR;
...
void CRenderer::RenderModels(BOOL bBackground) {
  if (_bMultiPlayer)
    gfx_bRenderModels = 1; // must render in multiplayer mode!
  if (!gfx_bRenderModels && !re_bRenderingShadows)
    return;

  _pfRenderProfile.StartTimer(CRenderProfile::PTI_RENDERMODELS);

  // sort all the delayed models by distance
  qsort(re_admDelayedModels.GetArrayOfPointers(), re_admDelayedModels.Count(), sizeof(CDelayedModel *),
        qsort_CompareDelayedModels);
  ...
  for (INDEX iModel = 0; iModel < re_admDelayedModels.Count(); iModel++) {
    ...
    if ((bBackground && !bIsBackground) || (!bBackground && bIsBackground) || !(dm.dm_ulFlags & DMF_VISIBLE))
      continue;
    ...
    if (en.en_RenderType == CEntity::RT_SKAMODEL || en.en_RenderType == CEntity::RT_SKAEDITORMODEL) {
      ...
      if (!re_bRenderingShadows && _wrpWorldRenderPrefs.wrp_stSelection == CWorldRenderPrefs::ST_ENTITIES
          && _wrpWorldRenderPrefs.wrp_pmoSelectedEntity != NULL
          && en.IsSelected(ENF_SELECTED)) { // get bounding box of current frame
...
void CRenderer::RenderParticles(BOOL bBackground) {
  if (_bMultiPlayer)
    gfx_bRenderParticles = 1; // must render in multiplayer mode!
  if (re_bRenderingShadows || !gfx_bRenderParticles)
    return;
...
void CRenderer::RenderLensFlares(void) {
  ...
  if (gfx_iLensFlareQuality == 0 || re_alfiLensFlares.Count() == 0)
    return;
  ...
  // for each lens flare of this drawport
  {
    for (INDEX iFlare = 0; iFlare < re_alfiLensFlares.Count(); iFlare++) {
      ...
      if (lfi.lfi_ulDrawPortID != ulDrawPortID && lfi.lfi_iMirrorLevel == 0)
        continue;
  ...
  if (ctFlares == 0)
    re_alfiLensFlares.PopAll();
  else
    re_alfiLensFlares.PopUntil(ctFlares - 1);
...
#define FLAREOUTSPEED (0.1f)
      ...
      // for each flare in the lens flare effect
      {
        for (INDEX iReflection = 0; iReflection < ctReflections; iReflection++) {
          ...
          FLOAT fFadeFactor
            = lfi.lfi_fFadeFactor
              * IntensityAtDistance(lfi.lfi_plsLightSource->ls_rFallOff * olf.oft_fFallOffFactor,
                                    lfi.lfi_plsLightSource->ls_rHotSpot * olf.oft_fFallOffFactor, lfi.lfi_fDistance);
          ...
          if (fIntensityFactor < 0.01f || fSizeI < 2.0f || fSizeJ < 2.0f)
            continue;
          ...
          // render the flare
          re_pdpDrawPort->RenderLensFlare(&olf.olf_toTexture, fLightI + olf.olf_fReflectionPosition * fReflectionDirI,
                                          fLightJ + olf.olf_fReflectionPosition * fReflectionDirJ, fSizeI, fSizeJ,
                                          olf.olf_aRotationFactor * fIPositionFactor, colBlending);
        }
      } // for each flare in the lens flare effect
      ...
      if (gfx_iLensFlareQuality > 2 && _wrpWorldRenderPrefs.wrp_lftLensFlares >= CWorldRenderPrefs::LFT_REFLECTIONS_AND_GLARE
        ...
        FLOAT fIntensity
          = IntensityAtDistance(lfi.lfi_plsLightSource->ls_rFallOff * lft.lft_fGlareFallOffFactor,
                                lfi.lfi_plsLightSource->ls_rHotSpot * lft.lft_fGlareFallOffFactor, lfi.lfi_fDistance);
