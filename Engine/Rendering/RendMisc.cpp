...
static inline PIX PIXCoord(FIX16_16 x) {
  return (PIX)Ceil(x);
};
 ...
 */
static inline void DrawArrowLine(CDrawPort &dp, const FLOAT2D &vPoint0, const FLOAT2D &vPoint1, COLOR color, ULONG ulLineType) {
 ...
 */
static inline void DrawArrow(CDrawPort &dp, PIX i0, PIX j0, PIX i1, PIX j1, COLOR color, ULONG ulLineType) {
 ...
 */
inline BOOL PolygonIsSelected(CBrushPolygon &bpo, CBrush3D &br, CBrushSector &bsc) {
  ...
  if (_wrpWorldRenderPrefs.GetSelectionType() != CWorldRenderPrefs::ST_NONE) {
    ...
    if ((_wrpWorldRenderPrefs.GetSelectionType() == CWorldRenderPrefs::ST_POLYGONS && bpo.IsSelected(BPOF_SELECTED))
        // or sector selection drawing is active and this polygon's sector is selected,
        || (_wrpWorldRenderPrefs.GetSelectionType() == CWorldRenderPrefs::ST_SECTORS && bsc.IsSelected(BSCF_SELECTED))
        // or entity selection drawing is active and this polygon's brush is selected
        || (_wrpWorldRenderPrefs.GetSelectionType() == CWorldRenderPrefs::ST_ENTITIES && (br.br_ulFlags & BRF_DRAWSELECTED))) {
      return TRUE;
    }
  }
  return FALSE;
...
void CRenderer::ProjectClipAndDrawArrow(const FLOAT3D &v0, const FLOAT3D &v1, COLOR colColor) {
  ...
  if (ulClipFlags != LCF_EDGEREMOVED) {
    ...
    DrawArrow(*re_pdpDrawPort, (PIX)v2d0(1), (PIX)v2d0(2), (PIX)v2d1(1), (PIX)v2d1(2), colColor, _FULL_);
  }
}
...
void CRenderer::RenderEntityTargets(void) {
  ...
  FOREACHINDYNAMICCONTAINER(re_cenDrawn, CEntity, iten) {
    ...
    if (enSource.en_penParent != NULL) {
      ...
      ProjectClipAndDrawArrow(enSource.GetLerpedPlacement().pl_PositionVector,
                              enSource.en_penParent->GetLerpedPlacement().pl_PositionVector, C_dBLUE | CT_OPAQUE);
    }

    // for all classes in hierarchy of this entity
    for (CDLLEntityClass *pdecDLLClass = enSource.en_pecClass->ec_pdecDLLClass; pdecDLLClass != NULL;
      ...
      for (INDEX iProperty = 0; iProperty < pdecDLLClass->dec_ctProperties; iProperty++) {
        ...
        if ((epProperty.ep_eptType != CEntityProperty::EPT_ENTITYPTR) || (strlen(epProperty.ep_strName) == 0)) {
          // skip it
          continue;
        }
        // get the target
        ...
        ProjectClipAndDrawArrow(enSource.GetLerpedPlacement().pl_PositionVector,
                                penTarget->GetLerpedPlacement().pl_PositionVector, epProperty.ep_colColor);
      }
    }
  }
}
