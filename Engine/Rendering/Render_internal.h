...
#define ALIGNED_NEW_AND_DELETE(align) \
  void *operator new[](size_t size) { \
    return AllocMemoryAligned(size, align); \
  }; \
...
class CScreenPolygon {
  public:
    CPlanarGradients spo_pgOoK; // planar gradients of depth for this polygon used in sorting
...
class CScreenEdge {
  public:
    ...
    // Top and bottom J coordinates of the edge
    PIX sed_pixTopJ;
    PIX sed_pixBottomJ;
...
class CActiveEdge {
  public:
    ...
    inline CActiveEdge(CScreenEdge *psed) : ace_xI(psed->sed_xI), ace_xIStep(psed->sed_xIStep), ace_psedEdge(psed) {};

    inline void Clear(void) {};
};

// Structure used for sorting edges in add list (size is 16 bytes)
class CAddEdge {
  public:
    ...
    inline CAddEdge(CScreenEdge *psed) : ade_xI(psed->sed_xI), ade_psedEdge(psed) {};
...
// We must declare dummy clear functions for external classes that get stored in dynamic stack arrays
...
class CRenderer {
  public:
    ...
    // For visibility tweaking
    ULONG re_ulVisExclude;
  ...
  public:
    ...
    BOOL FindModelLights(CEntity &en, const CPlacement3D &plModel, COLOR &colLight, COLOR &colAmbient, FLOAT &fTotalShadowIntensity,
                         FLOAT3D &vTotalLightDirection, FLOATplane3D &plFloorPlane);
    // Render a model
    void RenderOneModel(CEntity &en, CModelObject &moModel, const CPlacement3D &plModel, const FLOAT fDistanceFactor, BOOL bRenderShadow, ULONG ulDMFlags);
    ...
    void ProjectClipAndDrawArrow(const FLOAT3D &v0, const FLOAT3D &v1, COLOR colColor);
...
extern ULONG RenderShadows(CWorld &woWorld, CEntity &enViewer, CAnyProjection3D &prProjection, const FLOATaabbox3D &boxViewer,
