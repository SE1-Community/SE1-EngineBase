...
void CRenderer::AddModelEntity(CEntity *penModel) {
  ...
  if (gap_iOptimizeClipping > 0 && (pprProjection->pr_bMirror || pprProjection->pr_bWarp)) {
    ...
    if (fPlaneDistance < -fR)
      iMirrorPlaneTest = -1;
    else if (fPlaneDistance > +fR)
      iMirrorPlaneTest = +1;
  ...
  if (iFrustumTest > 0)
    dm.dm_ulFlags |= DMF_INSIDE; // mark the need for clipping
  if (iMirrorPlaneTest > 0)
    dm.dm_ulFlags |= DMF_INMIRROR; // mark the need for clipping to mirror/warp plane

  // if this is an editor model and editor models are disabled
  if (penModel->en_RenderType == CEntity::RT_EDITORMODEL && !_wrpWorldRenderPrefs.IsEditorModelsOn()) {
    // don't render it
    _pfRenderProfile.StopTimer(CRenderProfile::PTI_ADDMODELENTITY);
    return;
  }
  ...
  if (_pselenSelectOnRender != NULL)
    SelectEntityOnRender(*pprProjection, *penModel);
...
void CRenderer::AddSkaModelEntity(CEntity *penModel) {
  ...
  // if this is an editor model and editor models are disabled
  if (penModel->en_RenderType == CEntity::RT_SKAEDITORMODEL && !_wrpWorldRenderPrefs.IsEditorModelsOn()) {
    // don't render it
    // _pfRenderProfile.StopTimer(CRenderProfile::PTI_ADDMODELENTITY);
    return;
  }
...
void CRenderer::AddLensFlare(CEntity *penLight, CLightSource *pls, CProjection3D *pprProjection, INDEX iMirrorLevel /*=0*/) {
  ...
  const INDEX ctFlares = re_alfiLensFlares.Count();
  {
    for (INDEX iFlare = 0; iFlare < ctFlares; iFlare++) {
      ...
      // if it is this one
      if (lfiOld.lfi_plsLightSource == pls && (lfiOld.lfi_ulDrawPortID == ulDrawPortID || lfiOld.lfi_iMirrorLevel > 0)) {
        // use it
        plfi = &lfiOld;
        break;
      }
  ...
  lfi.lfi_fOoK = (1 - pprProjection->pr_fDepthBufferFactor / vRotated(3)) * pprProjection->pr_fDepthBufferMul
                 + pprProjection->pr_fDepthBufferAdd;
...
void CRenderer::AddNonZoningBrush(CEntity *penBrush, CBrushSector *pbscThatAdds) {
  ...
  if (!(penBrush->en_ulFlags & ENF_ZONING) && !bWireFrame) { // test every brush polygon for it's visibility flag
...
addBrush:
  ...
  CBrushMip *pbm
    = brBrush.GetBrushMipByDistance(_wrpWorldRenderPrefs.GetCurrentMipBrushingFactor(brBrush.br_prProjection->MipFactor()));
  ...
  FOREACHINDYNAMICCONTAINER(re_pwoWorld->wo_cenEntities, CEntity, iten) {
    ...
    if (iten->en_RenderType == CEntity::RT_BRUSH
      ...
      // if it is model, or editor model that should be drawn
               ...
               || (iten->en_RenderType == CEntity::RT_EDITORMODEL && _wrpWorldRenderPrefs.IsEditorModelsOn())) {
      // add it as a model
      AddModelEntity(&iten.Current());
    } else if (iten->en_RenderType == CEntity::RT_SKAMODEL
               || (iten->en_RenderType == CEntity::RT_SKAEDITORMODEL && _wrpWorldRenderPrefs.IsEditorModelsOn())) {
...
void CRenderer::AddEntitiesInBox(const FLOATaabbox3D &boxNear) {
  ...
  FOREACHINDYNAMICCONTAINER(re_pwoWorld->wo_cenEntities, CEntity, iten) {
    ...
    if (iten->en_RenderType == CEntity::RT_BRUSH
      ...
      // if it is model, or editor model that should be drawn
    } else if (iten->en_RenderType == CEntity::RT_MODEL || iten->en_RenderType == CEntity::RT_EDITORMODEL) {
    ...
    } else if (iten->en_RenderType == CEntity::RT_SKAMODEL || iten->en_RenderType == CEntity::RT_SKAEDITORMODEL) {
      // get model's bounding box for current frame
      FLOATaabbox3D boxModel;
...
void CRenderer::AddZoningSectorsAroundBox(const FLOATaabbox3D &boxNear) {
  ...
  FOREACHINDYNAMICCONTAINER(re_pwoWorld->wo_cenEntities, CEntity, iten) {
    ...
    if (iten->en_RenderType == CEntity::RT_FIELDBRUSH && !_wrpWorldRenderPrefs.IsFieldBrushesOn()) {
      // skip it
      continue;
    }
    ...
    */
    ...
    if (pbm != NULL) {
      ...
      FOREACHINDYNAMICARRAY(pbm->bm_abscSectors, CBrushSector, itbsc) {
        ...
        if (itbsc->bsc_boxBoundingBox.HasContactWith(boxNear) && !((itbsc->bsc_ulFlags & BSCF_HIDDEN) && !re_bRenderingShadows)) {
...
void CRenderer::AddMirror(CScreenPolygon &spo) {
  ...
  mi.mi_plPlane = bpo.bpo_pbplPlane->bpl_plRelative * m + plLerped.pl_PositionVector;
