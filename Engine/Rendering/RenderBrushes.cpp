    ...
    for (INDEX itri = 0; itri < cttri; itri++) {
      for (INDEX ied = 0; ied < 3; ied++) {
        ...
        if (ulClipFlags != LCF_EDGEREMOVED) {
          ...
          re_pdpDrawPort->DrawLine((PIX)fI0, (PIX)fJ0, (PIX)fI1, (PIX)fJ1, colorEdges | CT_OPAQUE, ulEdgesLineType);
        }
      }
    }
  }
  ...
  FOREACHINSTATICARRAY(bscSector.bsc_abpoPolygons, CBrushPolygon, itpo) {
    ...
    FOREACHINSTATICARRAY(itpo->bpo_abpePolygonEdges, CBrushPolygonEdge, itpe) {
      ...
      if (ulClipFlags != LCF_EDGEREMOVED) {
        ...
        re_pdpDrawPort->DrawLine((PIX)v2d0(1), (PIX)v2d0(2), (PIX)v2d1(1), (PIX)v2d1(2), colorEdges | CT_OPAQUE, ulEdgesLineType);
      }
    }
  }
}
...
void CRenderer::PrepareBrush(CEntity *penBrush) {
  ...
  if (brBrush.GetFirstMip()->bm_abscSectors.Count() == 0 && _wrpWorldRenderPrefs.IsEditorModelsOn() && wld_bRenderEmptyBrushes) {
...
void CRenderer::RenderWireFrameBrushes(void) {
  BOOL bRenderNonField = _wrpWorldRenderPrefs.wrp_ftEdges != CWorldRenderPrefs::FT_NONE
 ...
 */
static inline int CompareTranslucentPolygons(const CTranslucentPolygon &tp0, const CTranslucentPolygon &tp1) {
  if (tp0.tp_fViewerDistance < tp1.tp_fViewerDistance)
    return -1;
  else if (tp0.tp_fViewerDistance > tp1.tp_fViewerDistance)
    return +1;
  else
    return 0;
}
...
ScenePolygon *CRenderer::SortTranslucentPolygons(ScenePolygon *pspoFirst) {
  ...
  for (ScenePolygon *pspo = pspoFirst; pspo != NULL; pspo = pspo->spo_pspoSucc) {
    ...
    tp.tp_fViewerDistance = ((CBrushPolygon *)pspo->spo_pvPolygon)->bpo_pbplPlane->bpl_pwplWorking->wpl_plView.Distance();
  }

  // sort the container
  qsort(re_atcTranslucentPolygons.GetArrayOfPointers(), re_atcTranslucentPolygons.Count(), sizeof(CTranslucentPolygon *),
        qsort_CompareTranslucentPolygons);
