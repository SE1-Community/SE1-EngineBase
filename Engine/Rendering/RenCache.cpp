  ...
  if (re_pbrCurrent->br_prProjection.IsPerspective()) {
    ...
    for (INDEX ipl = 0; ipl < ctpl; ipl++) {
      ...
      wpl.wpl_plView.Distance()
        = wpl.wpl_plView(1) * v(1) + wpl.wpl_plView(2) * v(2) + wpl.wpl_plView(3) * v(3) + wpl.wpl_plRelative.Distance();
...
void CRenderer::SetupFogAndHaze(void) {
  ...
  if (_bMultiPlayer)
    gfx_bRenderFog = 1; // must render fog in multiplayer mode!
  ...
  if (_wrpWorldRenderPrefs.wrp_bFogOn && gfx_bRenderFog) { // if the sector has fog
    CFogParameters fp;
    if (bsc.bsc_pbmBrushMip->bm_pbrBrush->br_penEntity->GetFog(bsc.GetFogType(), fp)) {
      // activate fog if not already active
      if (!_fog_bActive) {
        StartFog(fp, br.br_prProjection->pr_vViewerPosition, br.br_prProjection->pr_ViewerRotationMatrix);
      }
  ...
  if (_wrpWorldRenderPrefs.wrp_bHazeOn && gfx_bRenderFog) { // if the sector has haze
    CHazeParameters hp;
    FLOAT3D vViewDir;
    ...
    if (bsc.bsc_pbmBrushMip->bm_pbrBrush->br_penEntity->GetHaze(bsc.GetHazeType(), hp, vViewDir)) {
      ...
      if (re_bViewerInHaze || (hp.hp_ulFlags & HPF_VISIBLEFROMOUTSIDE)) {
        ...
        if (!_haze_bActive) {
          StartHaze(hp, br.br_prProjection->pr_vViewerPosition, br.br_prProjection->pr_ViewerRotationMatrix);
        }
...
void CRenderer::AddEdgeToAddAndRemoveLists(CScreenEdge &sed) {
  ...
  while (plnInList->ln_Succ != NULL) {
    slIInList = ((CAddEdge *)((UBYTE *)plnInList - offsetof(CAddEdge, ade_lnInAdd)))->ade_xI.slHolder;
...
inline void CRenderer::MakeScreenEdge(CScreenEdge &sed, FLOAT fI0, FLOAT fJ0, FLOAT fI1, FLOAT fJ1) {
  ...
  ASSERT(sed.sed_xI > FIX16_16(-1.0f) && sed.sed_xI < FIX16_16(re_fbbClipBox.Max()(1) + SENTINELEDGE_EPSILON)
...
void CRenderer::SetOneTextureParameters(CBrushPolygon &bpo, ScenePolygon &spo, INDEX iLayer) {
  ...
  spo.spo_aubTextureFlags[iLayer]
    = (bpo.bpo_abptTextures[iLayer].s.bpt_ubFlags & (BPTF_CLAMPU | BPTF_CLAMPV | BPTF_AFTERSHADOW)) | (tb.tb_ubBlendingType);
  ...
  if (iTransformation == 0) {
    ...
    if ((bpo.bpo_abptTextures[iLayer].s.bpt_ubFlags & (BPTF_CLAMPU | BPTF_CLAMPV))
        == 0) { // make a mapping adjusted for texture wrapping
...
CScreenPolygon *CRenderer::MakeScreenPolygon(CBrushPolygon &bpo) {
  ...
  if (_wrpWorldRenderPrefs.wrp_shtShadows != CWorldRenderPrefs::SHT_NONE && !(bpo.bpo_ulFlags & BPOF_FULLBRIGHT)) {
  ...
  BOOL bBackgroundPolygon
    = re_bBackgroundEnabled && (bpo.bpo_pbscSector->bsc_pbmBrushMip->bm_pbrBrush->br_penEntity->en_ulFlags & ENF_BACKGROUND);
  ...
  if (fOpacity < 1) { // better to hold opacity in integer
  ...
  if (bForceTraslucency || ((bpo.bpo_ulFlags & BPOF_RENDERASPORTAL) && ((bpo.bpo_ulFlags & BPOF_TRANSLUCENT) || bSelected))) {
...
void CRenderer::AddPolygonToScene(CScreenPolygon *pspo) {
  ...
  if (&bpo == NULL
      || ((bpo.bpo_ulFlags & BPOF_OCCLUDER)
          && (!(bpo.bpo_ulFlags & BPOF_SELECTED) || _wrpWorldRenderPrefs.GetSelectionType() != CWorldRenderPrefs::ST_POLYGONS))) {
  ...
  for (INDEX i = 0; i < ctVtx; i++) {
    ...
    if (-v(3) < fNearestZ)
      fNearestZ = -v(3); // inverted because of negative sign
    pvtx[i].x = v(1);
    pvtx[i].y = v(2);
    pvtx[i].z = v(3);
  }
...
void CRenderer::MakeSpan(CScreenPolygon &spo, CScreenEdge *psed0, CScreenEdge *psed1) {
  ...
  ASSERT(!(spo.IsPortal() && (re_ubLightIllumination == 0 || re_ubLightIllumination != spo.spo_ubIllumination)));
...
void CRenderer::AddSpansToScene(void) {
  ...
  for (INDEX iSpan = 0; iSpan < ctSpans; iSpan++) { // get span start and stop I from edges
