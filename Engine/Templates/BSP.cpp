...
template<class Type> inline BOOL EpsilonEq(const Type &a, const Type &b) {
  return Abs(a - b) <= BSP_EPSILON;
};

template<class Type> inline BOOL EpsilonNe(const Type &a, const Type &b) {
  return Abs(a - b) > BSP_EPSILON;
};

// BSPVertex : Assignment operator with coordinates only
...
// BSPVertexContainer : Constructor
template<class Type, int iDimensions> BSPVertexContainer<Type, iDimensions>::BSPVertexContainer(void) {}
...
// BSPVertexContainer : Initialize for a direction
template<class Type, int iDimensions>
...
// BSPVertexContainer : Uninitialize
template<class Type, int iDimensions> void BSPVertexContainer<Type, iDimensions>::Uninitialize(void) {
...
template<class Type, int iDimensions> class CVertexComparator {
  public:
...
// BSPVertexContainer : Sort vertices in this container along the largest axis of container direction
template<class Type, int iDimensions> void BSPVertexContainer<Type, iDimensions>::Sort(void) {
...
// BSPVertexContainer : Eliminate paired vertices
template<class Type, int iDimensions> void BSPVertexContainer<Type, iDimensions>::ElliminatePairedVertices(void) {
  // if there are no vertices, or the container is not line
  if (bvc_aVertices.Count() == 0 || IsPlannar()) {
    // do not attempt to sort
    return;
...
// BSPEdge : Constructor with two vectors
template<class Type, int iDimensions>
BSPEdge<Type, iDimensions>::BSPEdge(const Vector<Type, iDimensions> &vVertex0, const Vector<Type, iDimensions> &vVertex1,
  ULONG ulTag) : bed_vVertex0(vVertex0), bed_vVertex1(vVertex1), bed_ulEdgeTag(ulTag) {}

// BSPEdge : Remove all edges marked for removal
template<class Type, int iDimensions>
...
// BSPEdge : Optimize a polygon made out of BSP edges using tag information
template<class Type, int iDimensions>
...
// BSPPolygon : Add an edge to the polygon
template<class Type, int iDimensions> inline void BSPPolygon<Type, iDimensions>::AddEdge(
  const Vector<Type, iDimensions> &vPoint0, const Vector<Type, iDimensions> &vPoint1, ULONG ulTag)
{
  *bpo_abedPolygonEdges.New() = BSPEdge<Type, iDimensions>(vPoint0, vPoint1, ulTag);
}

// BSPNode : Recursive destructor
template<class Type, int iDimensions> void BSPNode<Type, iDimensions>::DeleteBSPNodeRecursively(void) {
...
// BSPNode : Constructor for a leaf node
template<class Type, int iDimensions> BSPNode<Type, iDimensions>::BSPNode(enum BSPNodeLocation bnl) :
bn_bnlLocation(bnl), bn_pbnFront(NULL), bn_pbnBack(NULL) {
  ASSERT(bnl == BNL_INSIDE || bnl == BNL_OUTSIDE);
}

// BSPNode : Constructor for a branch node
template<class Type, int iDimensions>
...
// BSPNode : Constructor for cloning a bsp (sub)tree
template<class Type, int iDimensions> BSPNode<Type, iDimensions>::BSPNode(BSPNode<Type, iDimensions> &bnRoot) :
...
// BSPNode : Test if a sphere is inside, outside, or intersecting. (Just a trivial rejection test)
template<class Type, int iDimensions>
...
// BSPNode : Test if a box is inside, outside, or intersecting (just a trivial rejection test)
template<class Type, int iDimensions> FLOAT BSPNode<Type, iDimensions>::TestBox(const OBBox<Type> &box) const {
  // if this is an inside node
  if (bn_bnlLocation == BNL_INSIDE) {
    // it is inside
    return 1;
...
// BSPNode : Find minimum/maximum parameters of points on a line that are inside - recursive
template<class Type, int iDimensions>
void BSPNode<Type, iDimensions>::FindLineMinMax(BSPLine<Type, iDimensions> &bl, const Vector<Type, iDimensions> &v0,
                                                const Vector<Type, iDimensions> &v1, Type t0, Type t1) {
  // if this is an inside node
  if (bn_bnlLocation == BNL_INSIDE) {
    // just update min/max
...
// BSPCutter : Constructor for splitting a polygon with a BSP tree
template<class Type, int iDimensions>
...
// BSPCutter : Destructor
template<class Type, int iDimensions> BSPCutter<Type, iDimensions>::~BSPCutter(void) {}

// BSPCutter : Cut a polygon with a BSP tree
template<class Type, int iDimensions>
void BSPCutter<Type, iDimensions>::CutPolygon(BSPPolygon<Type, iDimensions> &bpoPolygon, BSPNode<Type, iDimensions> &bn) {
  ...
  // if this node is a branch
    ...
    // if the polygon is on the split plane
      ...
      BSPPolygon<Type, iDimensions> bpoInsideFront((Plane<Type, iDimensions> &)bpoPolygon, bcFront.bc_abedInside,
                                                   bpoPolygon.bpo_ulPlaneTag);
      ...
      BSPPolygon<Type, iDimensions> bpoOutsideFront((Plane<Type, iDimensions> &)bpoPolygon, bcFront.bc_abedOutside,
                                                    bpoPolygon.bpo_ulPlaneTag);
      // cut them with back part of bsp
...
// BSPCutter : Split a polygon with a plane - returns FALSE if polygon is laying on the plane
template<class Type, int iDimensions>
BOOL BSPCutter<Type, iDimensions>::SplitPolygon(BSPPolygon<Type, iDimensions> &bpoPolygon,
                                                const Plane<Type, iDimensions> &plSplitPlane, ULONG ulPlaneTag,
  ...
  Vector<Type, iDimensions> vSplitDirection = ((Vector<Type, iDimensions> &)plSplitPlane)
                                            * (Vector<Type, iDimensions> &)bpoPolygon;
...
// BSPCutter : Split an edge with a plane
template<class Type, int iDimensions>
void BSPCutter<Type, iDimensions>::SplitEdge(const Vector<Type, iDimensions> &vPoint0, const Vector<Type, iDimensions> &vPoint1,
                                             ULONG ulEdgeTag, const Plane<Type, iDimensions> &plSplitPlane,
                                             BSPPolygon<Type, iDimensions> &bpoFront, BSPPolygon<Type, iDimensions> &bpoBack,
                                             BSPVertexContainer<Type, iDimensions> &bvcFront,
                                             BSPVertexContainer<Type, iDimensions> &bvcBack) {
...
// BSPTree : Constructor
template<class Type, int iDimensions> BSPTree<Type, iDimensions>::BSPTree(void) {
  bt_pbnRoot = NULL;
}
...
// BSPTree : Constructor with array of polygons oriented inwards
template<class Type, int iDimensions>
...
// BSPTree : Create bsp-subtree from array of polygons oriented inwards
template<class Type, int iDimensions> BSPNode<Type, iDimensions> *
BSPTree<Type, iDimensions>::CreateSubTree(CDynamicArray<BSPPolygon<Type, iDimensions>> &abpoPolygons) {
  ...
  {FOREACHINDYNAMICARRAY(abpoPolygons, polygon_t, itbpo) {
    ...
    BOOL bOnPlane = BSPCutter<Type, iDimensions>::SplitPolygon(itbpo.Current(), bpoSplitter, bpoSplitter.bpo_ulPlaneTag, bpoFront, bpoBack);
...
// BSPTree : Create bsp-tree from array of polygons oriented inwards
template<class Type, int iDimensions>
...
// BSPTree : Destroy bsp-tree
template<class Type, int iDimensions> void BSPTree<Type, iDimensions>::Destroy(void) {
  // if tree is in array
...
// BSPTree : Test if a sphere could touch any of inside nodes (just a trivial rejection test)
template<class Type, int iDimensions>
...
// BSPTree : Test if a box is inside, outside, or intersecting (just a trivial rejection test)
template<class Type, int iDimensions> FLOAT BSPTree<Type, iDimensions>::TestBox(const OBBox<Type> &box) const {
  if (bt_pbnRoot == NULL) {
    return FALSE;
  }
...
// BSPTree : Find minimum/maximum parameters of points on a line that are inside
template<class Type, int iDimensions> void BSPTree<Type, iDimensions>::FindLineMinMax(
  const Vector<Type, iDimensions> &v0, const Vector<Type, iDimensions> &v1, Type &tMin, Type &tMax) const
{
  // init line
...
// BSPTree : Move one subtree to array
template<class Type, int iDimensions>
...
// BSPTree : Count nodes in subtree
template<class Type, int iDimensions> INDEX BSPTree<Type, iDimensions>::CountNodes(BSPNode<Type, iDimensions> *pbnSubtree) {
  if (pbnSubtree == NULL) {
    return 0;
...
// BSPTree : Move all nodes to array
template<class Type, int iDimensions> void BSPTree<Type, iDimensions>::MoveNodesToArray(void) {
  // if there is no tree
  if (bt_pbnRoot == NULL) {
    // do nothing
    return;
...
// BSPTree : Read/write entire bsp tree to disk
template<class Type, int iDimensions> void BSPTree<Type, iDimensions>::Read_t(CTStream &strm) {
  // free eventual existing tree
  Destroy();
...
template<class Type, int iDimensions> void BSPTree<Type, iDimensions>::Write_t(CTStream &strm) {
