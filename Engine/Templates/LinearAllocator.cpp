...
// default constructor
template<class Type> CLinearAllocator<Type>::CLinearAllocator(void) {
...
// copy constructor
template<class Type> CLinearAllocator<Type>::CLinearAllocator(CLinearAllocator<Type> &laOriginal) {
  ASSERT(FALSE);
}
...
template<class Type> void CLinearAllocator<Type>::Clear(void) {
...
template<class Type> inline void CLinearAllocator<Type>::SetAllocationStep(INDEX ctStep) {
  la_ctAllocationStep = ctStep;
}

// allocate a new memory block
template<class Type> void CLinearAllocator<Type>::AllocBlock(INDEX iCount) {
...
template<class Type> inline Type &CLinearAllocator<Type>::New(void) {
  if (la_ctFree == 0) {
    // allocate a new memory block
...
template<class Type> inline Type *CLinearAllocator<Type>::New(INDEX ct) {
...
template<class Type> inline void CLinearAllocator<Type>::Reset(void) {
