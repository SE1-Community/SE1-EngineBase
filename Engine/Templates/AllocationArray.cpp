 ...
 * Default constructor.
 */
template<class Type> inline CAllocationArray<Type>::CAllocationArray(void) : CStaticArray<Type>(), aa_aiFreeElements() {
 ...
 * Destructor.
 */
template<class Type> inline CAllocationArray<Type>::~CAllocationArray(void) {};

// Destroy all objects, and reset the array to initial (empty) state.
template<class Type> inline void CAllocationArray<Type>::Clear(void) {
 ...
 */
template<class Type> inline void CAllocationArray<Type>::SetAllocationStep(INDEX ctStep) {
  ASSERT(ctStep > 0);
 ...
 */
template<class Type> inline void CAllocationArray<Type>::New(INDEX iCount) {
  // never call this!
  ASSERT(FALSE);
};
 ...
 * Destroy all objects.
 */
template<class Type> inline void CAllocationArray<Type>::Delete(void) {
  // never call this!
  ASSERT(FALSE);
}

// Alocate a new object.
template<class Type> inline INDEX CAllocationArray<Type>::Allocate(void) {
...
template<class Type> inline void CAllocationArray<Type>::FreeAll(void) {
...
template<class Type> inline BOOL CAllocationArray<Type>::IsAllocated(INDEX i) {
  // must be within pool limits
...
template<class Type> INDEX CAllocationArray<Type>::Count(void) const {
  ASSERT(this != NULL);
...
template<class Type> INDEX CAllocationArray<Type>::Index(Type *ptObject) {
  ASSERT(this != NULL);
...
// Assignment operator.
template<class Type> CAllocationArray<Type> &CAllocationArray<Type>::operator=(const CAllocationArray<Type> &aaOriginal) {
  ASSERT(this != NULL);
