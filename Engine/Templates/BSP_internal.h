...
template<class Type, int iDimensions> class BSPVertex : public Vector<Type, iDimensions> {
  public:
    // Constructor
...
template<class Type, int iDimensions> class BSPPolygon : public Plane<Type, iDimensions> {
  public:
    ...
    inline BSPPolygon(Plane<Type, iDimensions> &plPlane, CDynamicArray<BSPEdge<Type, iDimensions>> abedPolygonEdges, ULONG ulPlaneTag) :
      Plane<Type, iDimensions>(plPlane), bpo_abedPolygonEdges(abedPolygonEdges), bpo_ulPlaneTag(ulPlaneTag) {};

    // Clear the object
    inline void Clear(void) {
      bpo_abedPolygonEdges.Clear();
    };
};

template<class Type, int iDimensions> class BSPLine {
  public:
    Type bl_tMin;
    Type bl_tMax;
};

// Template class for BSP-tree node of arbitrary dimensions and arbitrary type of members (split plane)
template<class Type, int iDimensions> class BSPNode : public Plane<Type, iDimensions> {
  public:
  ...
  public:
    ...
    inline BSPNode(const Plane<Type, iDimensions> &plSplitPlane, ULONG ulPlaneTag, BSPNode<Type, iDimensions> &bnFront,
                   BSPNode<Type, iDimensions> &bnBack);
    ...
    void FindLineMinMax(BSPLine<Type, iDimensions> &bl, const Vector<Type, iDimensions> &v0, const Vector<Type, iDimensions> &v1, Type t0, Type t1);
...
template<class Type, int iDimensions> class BSPCutter {
  public:
    ...
    static inline void SplitEdge(const Vector<Type, iDimensions> &vPoint0, const Vector<Type, iDimensions> &vPoint1,
  ...
  public:
    ...
    static inline BOOL SplitPolygon(BSPPolygon<Type, iDimensions> &bpoPolygon, const Plane<Type, iDimensions> &plPlane,
                                    ULONG ulPlaneTag, BSPPolygon<Type, iDimensions> &bpoFront,
                                    BSPPolygon<Type, iDimensions> &bpoBack);
