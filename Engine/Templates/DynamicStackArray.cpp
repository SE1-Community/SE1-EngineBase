 ...
 * Default constructor.
 */
template<class Type> inline CDynamicStackArray<Type>::CDynamicStackArray(void) : CDynamicArray<Type>() {
  da_ctUsed = 0;
 ...
 * Destructor.
 */
template<class Type> inline CDynamicStackArray<Type>::~CDynamicStackArray(void) {
  // lock the array on destruction
  CDynamicArray<Type>::Unlock();
};

// Destroy all objects, and reset the array to initial (empty) state.
template<class Type> inline void CDynamicStackArray<Type>::Clear(void) {
 ...
 */
template<class Type> inline void CDynamicStackArray<Type>::SetAllocationStep(INDEX ctStep) {
  ASSERT(ctStep > 0);
 ...
 */
template<class Type> inline Type &CDynamicStackArray<Type>::Push(void) {
...
template<class Type> inline Type *CDynamicStackArray<Type>::Push(INDEX ct) {
  // if there are no free elements in the array
 ...
 */
template<class Type> inline void CDynamicStackArray<Type>::PopAll(void) {
 ...
 */
template<class Type> inline Type &CDynamicStackArray<Type>::operator[](INDEX i) {
  ASSERT(this != NULL);
  ASSERT(i < da_ctUsed); // check bounds
  return CDynamicArray<Type>::operator[](i);
}
template<class Type> inline const Type &CDynamicStackArray<Type>::operator[](INDEX i) const {
  ASSERT(this != NULL);
  ASSERT(i < da_ctUsed); // check bounds
  return CDynamicArray<Type>::operator[](i);
}
 ...
 */
template<class Type> INDEX CDynamicStackArray<Type>::Count(void) const {
  ASSERT(this != NULL);
  return da_ctUsed;
}
 ...
 */
template<class Type> INDEX CDynamicStackArray<Type>::Index(Type *ptMember) {
  ASSERT(this != NULL);
 ...
 */
template<class Type> Type **CDynamicStackArray<Type>::GetArrayOfPointers(void) {
  return da_Pointers;
}
 ...
 * Assignment operator.
 */
template<class Type> CDynamicStackArray<Type> &CDynamicStackArray<Type>::operator=(CDynamicStackArray<Type> &arOriginal) {
  ASSERT(this != NULL);
