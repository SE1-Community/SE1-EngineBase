...
// Constructor
template<class Type> CDynamicArray<Type>::CDynamicArray(void) {
...
// Copy constructor
template<class Type> CDynamicArray<Type>::CDynamicArray(CDynamicArray<Type> &daOriginal) {
#if CHECKARRAYLOCKING
  // not locked
  da_LockCt = 0;
#endif
...
template<class Type> void CDynamicArray<Type>::Clear(void) {
  ASSERT(this != NULL);

  // if any pointers are allocated
  if (da_Count != 0) {
    // NOTE: We must explicitly clear objects here,
    // because array deleting does not call object destructors!
...
template<class Type> Type &CDynamicArray<Type>::operator[](INDEX iObject) {
  return *Pointer(iObject);
};

template<class Type> const Type &CDynamicArray<Type>::operator[](INDEX iObject) const {
  return *Pointer(iObject);
};

// Grow pointer array by a given number of members
template<class Type> void CDynamicArray<Type>::GrowPointers(INDEX iCount) {
...
template<class Type> void CDynamicArray<Type>::ShrinkPointers(INDEX iCount) {
  ASSERT(this != NULL && iCount > 0);
...
template<class Type> Type *CDynamicArray<Type>::AllocBlock(INDEX iCount) {
  ASSERT(this != NULL && iCount > 0);
...
template<class Type> Type *CDynamicArray<Type>::New(INDEX iCount) {
  ASSERT(this != NULL && iCount >= 0);
...
template<class Type> void CDynamicArray<Type>::Delete(Type *ptMember) {
  ASSERT(this != NULL);
...
template<class Type> Type *CDynamicArray<Type>::Pointer(INDEX iMember) {
  ASSERT(this != NULL);
...
template<class Type> const Type *CDynamicArray<Type>::Pointer(INDEX iMember) const {
  ASSERT(this != NULL);
...
template<class Type> void CDynamicArray<Type>::Lock(void) {
  ASSERT(this != NULL);
...
template<class Type> void CDynamicArray<Type>::Unlock(void) {
  ASSERT(this != NULL);
...
template<class Type> INDEX CDynamicArray<Type>::Index(Type *ptMember) {
  ASSERT(this != NULL);
...
template<class Type> INDEX CDynamicArray<Type>::GetIndex(Type *ptMember) {
  ASSERT(this != NULL);
...
template<class Type> INDEX CDynamicArray<Type>::Count(void) const {
  ASSERT(this != NULL);
  return da_Count;
}

// Assignment operator
template<class Type> CDynamicArray<Type> &CDynamicArray<Type>::operator=(CDynamicArray<Type> &arOriginal) {
  ASSERT(this != NULL);
...
template<class Type> void CDynamicArray<Type>::MoveArray(CDynamicArray<Type> &arOther) {
...
// Constructor for given array
template<class Type> inline CDynamicArrayIterator<Type>::CDynamicArrayIterator(CDynamicArray<Type> &da) : dai_Array(da) {
  // lock indices
  dai_Array.Lock();
  dai_Index = 0;
}
...
// Destructor
template<class Type> inline CDynamicArrayIterator<Type>::~CDynamicArrayIterator(void) {
  // unlock indices
  dai_Array.Unlock();
  dai_Index = -1;
}
...
// Move to next object
template<class Type> inline void CDynamicArrayIterator<Type>::MoveToNext(void) {
  ASSERT(this != NULL);
  dai_Index++;
}

// Check if finished
template<class Type> inline BOOL CDynamicArrayIterator<Type>::IsPastEnd(void) {
  ASSERT(this != NULL);
