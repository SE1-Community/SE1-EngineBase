...
// Default constructor
template<class Type> CStock<Type>::CStock(void) {
...
template<class Type> Type *CStock<Type>::Obtain_t(const CTFileName &fnmFileName) {
...
template<class Type> void CStock<Type>::Release(Type *ptObject) {
...
// Free all unused elements of the stock
template<class Type> void CStock<Type>::FreeUnused(void) {
  BOOL bAnyRemoved;
...
template<class Type> SLONG CStock<Type>::CalculateUsedMemory(void) {
  SLONG slUsedTotal = 0;
...
template<class Type> void CStock<Type>::DumpMemoryUsage_t(CTStream &strm) {
  CTString strLine;
  SLONG slUsedTotal = 0;
  ...
  {FOREACHINDYNAMICCONTAINER(st_ctObjects, Type, itt) {
    ...
    strLine.PrintF("%7.1fk %s(%d) %s", slUsedByObject / 1024.0f,
                   (const char *)(itt->GetName()), itt->GetUsedCount(), itt->GetDescription());
    strm.PutLine_t(strLine);
  }}
}

// Get number of total elements in stock
template<class Type> INDEX CStock<Type>::GetTotalCount(void) {
  return st_ctObjects.Count();
}

// Get number of used elements in stock
template<class Type> INDEX CStock<Type>::GetUsedCount(void) {
  INDEX ctUsed = 0;
